...
var Popup = _ui.default.inherit({
  ...
  _setAriaLabel() {
    ...
    if (showTitle && !!title) {
      var titleId = new _guid.default();
      this._$title.find(".".concat(TOOLBAR_LABEL_CLASS)).eq(0).attr('id', titleId);
      this.$overlayContent().attr('aria-labelledby', titleId);
    } else {
      this.$overlayContent().attr('aria-label', _message.default.format('dxPopup-defaultTitle'));
    }
  },
