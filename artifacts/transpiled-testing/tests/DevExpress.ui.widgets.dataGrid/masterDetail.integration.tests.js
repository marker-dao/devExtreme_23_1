System.register(["core/utils/type", "core/config", "events/pointer", "core/utils/common", "../../helpers/dataGridHelper.js", "jquery"], function (_export, _context) {
  "use strict";

  var typeUtils, config, pointerEvents, commonUtils, createDataGrid, baseModuleConfig, $, CELL_UPDATED_CLASS;
  return {
    setters: [function (_coreUtilsType) {
    ...
    `;
      ...
      QUnit.module('Master Detail', baseModuleConfig, () => {
        QUnit.test('Column hiding should works with masterDetail and column fixing', function (assert) {
          ...
          this.clock.tick(10);
          dataGrid.expandAdaptiveDetailRow({
            id: 1
          });
          this.clock.tick(10);
          dataGrid.collapseAdaptiveDetailRow({
            id: 1
          });
        ...
        QUnit.test('Master Row - expandRow should resolve its promise only after re-rendering (T880769)', function (assert) {
          ...
          this.clock.tick(10);
          assert.verifySteps(['rowExpanded rowCount: 2, masterRow: true, detailRow: true', 'done rowCount: 2, masterRow: true, detailRow: true']);
        });
              ...
              templates: {
                rowTemplate: {
                  render({
                    ...
                    const markup = '<tr class="my-row">' + '<td>' + data.text + '</td>' + '</tr>';
        ...
        QUnit.testInActiveWindow('DataGrid with inside grid in masterDetail - the invalid message of the datebox should not be removed when focusing cell', function (assert) {
          ...
          const dataGrid = createDataGrid({
            ...
            masterDetail: {
              enabled: true,
              template: function ($container, options) {
                $('<div/>').addClass('inside-grid').dxDataGrid({
                  dataSource: [{
                    name: 'Inside Grid Item'
                  }],
                  columns: [{
                    dataField: 'name',
                    dataType: 'date',
        ...
        QUnit.test('Master grid should scroll its content properly when rows in nested detail grids are expanded (T1010839)', function (assert) {
          ...
          const dataGrid = createDataGrid({
            ...
            masterDetail: {
              enabled: true,
              template: function (container, options) {
                $('<div>').addClass('nested').dxDataGrid({
                  dataSource: getData(),
                  keyExpr: 'id',
                  columnFixing: {
                    enabled: true
