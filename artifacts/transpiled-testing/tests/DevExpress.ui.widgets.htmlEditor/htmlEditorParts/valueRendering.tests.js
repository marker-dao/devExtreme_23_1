System.register(["jquery", "ui/html_editor", "ui/html_editor/converters/markdown", "core/utils/size", "./utils.js", "devextreme-quill"], function (_export, _context) {
  "use strict";

  var $, getOuterHeight, checkLink, prepareEmbedValue, prepareTableValue, Quill, CONTENT_CLASS, HTML_EDITOR_SUBMIT_ELEMENT_CLASS, TABLE_WITH_HEADER_MARKUP, MD_TABLE_WITH_HEADER_MARKUP, EXPECTED_TABLE_MARKUP, test, testModule, moduleConfig;
  ...
  _export("default", function () {
    testModule('Value as HTML markup', moduleConfig, () => {
      ...
      test('render table with header without paragraph', function (assert) {
        ...
        const expectedValue = '<table>' + '<thead>' + '<tr>' + '<th><p>Header1</p></th>' + '<th><p>Header2</p></th>' + '</tr>' + '</thead>' + '<tbody>' + '<tr>' + '<td><p>Data1</p></td>' + '<td><p>Data2</p></td>' + '</tr>' + '</tbody>' + '</table>';
      ...
      test('render table with header and multiple paragraphs', function (assert) {
        ...
        const $element = instance.$element();
        const markup = prepareTableValue($element.find(getSelector(CONTENT_CLASS)).html());
        const expectedValue = '<table>' + '<thead>' + '<tr>' + '<th>' + '<p>Header1</p>' + '<p>Subheader1</p>' + '</th>' + '<th><p>Header2</p></th>' + '</tr>' + '</thead>' + '<tbody>' + '<tr>' + '<td>' + '<p>Data1</p>' + '<p>Data1_1</p>' + '<p>Data1_2</p>' + '</td>' + '<td><p>Data2</p></td>' + '</tr>' + '</tbody>' + '</table>';
        assert.strictEqual(instance.option('value'), value);
        assert.strictEqual(markup, expectedValue);
      });
      test('render transclude content', function (assert) {
        assert.expect(4);
        const instance = $('#htmlEditor').html('<h1>Hi!</h1><p>Test</p>').dxHtmlEditor().dxHtmlEditor('instance');
        const $element = instance.$element();
        const markup = $element.find(getSelector(CONTENT_CLASS)).html();
      ...
      test('render transclude content and predefined value', function (assert) {
        const instance = $('#htmlEditor').html('<h1>Hi!</h1><p>Test</p>').dxHtmlEditor({
      ...
      test('change value by user', function (assert) {
        ...
        instance.$element().find(getSelector(CONTENT_CLASS)).html('<p>Hi! <strong>Test.</strong></p><p>New line</p>');
      });
      ...
      test('editor should preserve break lines', function (assert) {
        ...
        const instance = $('#htmlEditor').html('<p><br></p><p><br></p><h1>Hi!</h1><p>Te</p><p>st</p>').dxHtmlEditor().dxHtmlEditor('instance');
        this.clock.tick(10);
        const $element = instance.$element();
        const markup = $element.find(getSelector(CONTENT_CLASS)).html();
      ...
      test('editor shouldn\'t create unexpected break lines', function (assert) {
        ...
        const instance = $('#htmlEditor').html(htmlMarkup).dxHtmlEditor().dxHtmlEditor('instance');
        this.clock.tick(10);
        const $element = instance.$element();
        const markup = $element.find(getSelector(CONTENT_CLASS)).html();
      ...
      test('clear the HTML value', function (assert) {
        ...
        instance.$element().find(getSelector(CONTENT_CLASS)).html('');
      });
      test('render widget in detached container', function (assert) {
        ...
        const quillMarkup = '<ol><li data-list="bullet"><span class="ql-ui" contenteditable="false">' + '</span>t1</li><li data-list="bullet"><span class="ql-ui" contenteditable="false"></span>t2</li></ol>';
        $container.detach();
    ...
    testModule('Value as Markdown markup', {
      ...
      test('change markdown value by user', function (assert) {
        ...
        instance.$element().find(getSelector(CONTENT_CLASS)).html('<p>Hi! <strong>Test.</strong></p>');
      });
    ...
    testModule('Custom blots rendering', {
      ...
      test('render image', function (assert) {
        ...
        instance.insertEmbed(0, 'extendedImage', {
          src: 'http://test.test/test.jpg',
          width: 100,
          height: 100,
          alt: 'altering'
        });
      ...
      test('render link', function (assert) {
        ...
        instance.insertText(0, 'test', 'link', {
          href: 'http://test.test',
          target: true
        });
      });
      test('render variable', function (assert) {
        ...
        instance.insertEmbed(0, 'variable', {
          escapeChar: '#',
          value: 'Test'
        });
      });
  ...
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_uiHtml_editor) {}, function (_uiHtml_editorConvertersMarkdown) {}, function (_coreUtilsSize) {
      getOuterHeight = _coreUtilsSize.getOuterHeight;
    }, function (_utilsJs) {
      checkLink = _utilsJs.checkLink;
      prepareEmbedValue = _utilsJs.prepareEmbedValue;
      prepareTableValue = _utilsJs.prepareTableValue;
    }, function (_devextremeQuill) {
      Quill = _devextremeQuill.default;
    }],
    execute: function () {
      CONTENT_CLASS = 'dx-htmleditor-content';
      HTML_EDITOR_SUBMIT_ELEMENT_CLASS = 'dx-htmleditor-submit-element';
      TABLE_WITH_HEADER_MARKUP = `
<table>
    <thead>
        <tr>
            <th>Header1</th>
    ...
    <tbody>
      ...
      MD_TABLE_WITH_HEADER_MARKUP = `| Header1  | Header2 |
| ---------| --------|
|Data1    | Data2   |`;
      EXPECTED_TABLE_MARKUP = '<table><thead><tr><th><p>Header1</p></th><th><p>Header2</p></th></tr></thead><tbody><tr><td><p>Data1</p></td><td><p>Data2</p></td></tr></tbody></table>';
      ({
        test,
        module: testModule
      } = QUnit);
