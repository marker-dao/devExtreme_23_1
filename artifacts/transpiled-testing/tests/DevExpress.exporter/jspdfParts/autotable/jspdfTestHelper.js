...
(["testing/tests/DevExpress.exporter/jspdfParts/autotable/jspdfTestHelper.js"], ["core/utils/type"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.exporter/jspdfParts/autotable/jspdfTestHelper.js", ["core/utils/type"], function($__export) {
  "use strict";
  var assert,
      isDefined,
  ...
  return {
    ...
    execute: function() {
      ...
      JSPdfDataGridTestHelper = function() {
        function JSPdfDataGridTestHelper(jsPDFDocument) {
          this.jsPDFDocument = jsPDFDocument;
        }
        return ($traceurRuntime.createClass)(JSPdfDataGridTestHelper, {
          checkCustomizeCell: function(eventArgs, expectedCells, callIndex) {
            var $__4 = eventArgs,
                actualGridCell = $__4.gridCell,
                actualPdfCell = $__4.pdfCell;
            var $__5 = expectedCells[callIndex],
                expectedGridCell = $__5.gridCell,
                expectedPdfCell = $__5.pdfCell;
            ...
            for (var propertyName in actualGridCell) {
          ...
          checkColumnWidths: function(expectedColumnWidths, actualAutoTableOptions) {
            var columnStyles = actualAutoTableOptions.columnStyles;
            var actualWidths = [];
            for (var i = 0; i < expectedColumnWidths.length; i++) {
              var width = columnStyles[i] ? columnStyles[i].cellWidth : 'auto';
              actualWidths.push(width);
            }
            assert.deepEqual(actualWidths, expectedColumnWidths, 'Column widths');
          },
          checkRowAndColumnCount: function(expectedCells, actualAutoTableOptions, rowType) {
            assert.strictEqual(expectedCells[rowType].length, actualAutoTableOptions[rowType].length, ("actual row count of the " + rowType));
            for (var rowIndex = 0; rowIndex < expectedCells[rowType].length; rowIndex++) {
          ...
          checkCellsContent: function(expectedCells, actualAutoTableOptions, rowType) {
            this._iterateCells(expectedCells[rowType], function(cell, rowIndex, columnIndex) {
          ...
          checkCellsStyles: function(expectedCells, actualAutoTableOptions, rowType) {
            this._iterateCells(expectedCells[rowType], function(cell, rowIndex, columnIndex) {
              var expectedPdfCellStyles = cell.pdfCell.styles || {};
              var actualPdfCellStyles = actualAutoTableOptions[rowType][rowIndex][columnIndex].styles;
              ['halign', 'fontStyle', 'cellWidth'].forEach(function(styleName) {
                assert.strictEqual(actualPdfCellStyles[styleName], expectedPdfCellStyles[styleName], ("AutoTable " + rowType + "[" + rowIndex + "][" + columnIndex + "].styles." + styleName));
                assert.strictEqual(isDefined(actualPdfCellStyles[styleName]), Object.prototype.hasOwnProperty.call(actualPdfCellStyles, styleName), ("AutoTable " + rowType + "[" + rowIndex + "][" + columnIndex + "].styles." + styleName + " is defined"));
              });
            });
          },
          checkMergeCells: function(expectedCells, actualAutoTableOptions, rowType) {
            this._iterateCells(expectedCells[rowType], function(expectedCell, rowIndex, columnIndex) {
              var actualPdfCell = actualAutoTableOptions[rowType][rowIndex][columnIndex];
      ...
      $__export("JSPdfDataGridTestHelper", JSPdfDataGridTestHelper);
    }
  };
});
...
//# sourceMappingURL=jspdfTestHelper.js.map