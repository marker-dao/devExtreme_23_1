...
(["testing/tests/DevExpress.exporter/exceljsParts/ExcelJSTestHelper.js"], ["core/utils/type","exporter/exceljs/export"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.exporter/exceljsParts/ExcelJSTestHelper.js", ["core/utils/type", "exporter/exceljs/export"], function($__export) {
  "use strict";
  var isDefined,
      Export,
      assert,
  ...
  return {
    ...
    execute: function() {
      var $__4;
      (($__4 = QUnit, assert = $__4.assert, $__4));
      ExcelJSTestHelper = function() {
        function ExcelJSTestHelper(worksheet) {
          this.worksheet = worksheet;
        }
        return ($traceurRuntime.createClass)(ExcelJSTestHelper, {
          checkCustomizeCell: function(eventArgs, expectedCells, callIndex) {
            var currentRowIndex = Math.floor(callIndex / expectedCells[0].length);
            var currentCellIndex = callIndex % expectedCells[currentRowIndex].length;
            var expectedCell = expectedCells[currentRowIndex][currentCellIndex];
            var expectedAddress = expectedCell.excelCell.address;
          ...
          checkAutoFilter: function(autoFilterEnabled, autoFilter, worksheetViews) {
          ...
          checkValues: function(cellsArray) {
            var $__3 = this;
            this._iterateCells(cellsArray, function(cellArgs) {
              var excelCell = cellArgs.excelCell;
              var $__6 = excelCell.address,
                  row = $__6.row,
                  column = $__6.column;
              assert.deepEqual($__3.worksheet.getCell(row, column).value, excelCell.value, ("this.worksheet.getCell(" + row + ", " + column + ").value"));
            });
          },
          checkColumnWidths: function(expectedWidths, startColumnIndex) {
            var tolerance = 0.2;
            for (var i = 0; i < expectedWidths.length; i++) {
              var actual = this.worksheet.getColumn(startColumnIndex + i).width;
              var expected = expectedWidths[i];
              var message = ("worksheet.getColumns(" + i + ").width");
          ...
          checkOutlineLevel: function(expectedOutlineLevelValues, startRowIndex) {
            ...
            this.worksheet.eachRow({includeEmpty: false}, function(row) {
          ...
          checkCellStyle: function(cellsArray) {
            var $__3 = this;
            this._iterateCells(cellsArray, function(cellArgs) {
              var excelCell = cellArgs.excelCell;
              var $__6 = excelCell.address,
                  row = $__6.row,
                  column = $__6.column;
              var sourceCell = $__3.worksheet.getCell(row, column);
              for (var propertyName in sourceCell.style) {
                assert.deepEqual(sourceCell.style[propertyName], excelCell[propertyName], ("this.worksheet.getCell(" + row + ", " + column + ")." + propertyName));
              }
            });
          },
          checkRowAndColumnCount: function(actual, total, topLeft) {
          ...
          checkCellRange: function(cellRange, actual, topLeft) {
            assert.deepEqual(cellRange.from, topLeft, 'cellRange.from');
            if (actual.row > 0 && actual.column > 0) {
              assert.deepEqual(cellRange.to, {
                row: topLeft.row + actual.row - 1,
                column: topLeft.column + actual.column - 1
              }, 'cellRange.to');
            } else {
              assert.deepEqual(cellRange.to, {
                row: topLeft.row + actual.row,
                column: topLeft.column + actual.column
              }, 'cellRange.to');
            }
          },
          _extendExpectedCells: function(cellsArray, topLeft, callback) {
            this._iterateCells(cellsArray, function(cellArgs, rowIndex, columnIndex) {
          ...
          _iterateCells: function(cellsArray, callback) {
            for (var rowIndex = 0; rowIndex < cellsArray.length; rowIndex++) {
              for (var columnIndex = 0; columnIndex < cellsArray[rowIndex].length; columnIndex++) {
                var currentCell = cellsArray[rowIndex][columnIndex];
                callback(currentCell, rowIndex, columnIndex);
              }
            }
          },
          _iterateWorksheetCells: function(callback) {
            this.worksheet.eachRow({includeEmpty: true}, function(row) {
              row.eachCell({includeEmpty: true}, function(cell) {
                callback(cell);
              });
            });
          },
          checkMergeCells: function(cellsArray, topLeft) {
            var $__3 = this;
            this._iterateWorksheetCells(function(excelCell) {
              if (excelCell.col < topLeft.column) {
                ...
                var expectedExcelCell = cellsArray[excelCell.row - topLeft.row][excelCell.col - topLeft.column].excelCell;
                if (!isDefined(expectedExcelCell.master)) {
                  assert.strictEqual(excelCell.isMerged, false, ("cell: " + excelCell.address + ".isMerged"));
                  assert.strictEqual(excelCell.master, excelCell, ("cell: " + excelCell.address + ".master"));
                } else {
                  var master = $__3.worksheet.getCell(expectedExcelCell.master[0] + topLeft.row - 1, expectedExcelCell.master[1] + topLeft.column - 1);
          ...
          checkCellFormat: function(cellsArray) {
            var $__3 = this;
            this._iterateCells(cellsArray, function(cellArgs) {
              var $__6,
                  $__7;
              var $__5 = cellArgs.excelCell,
                  address = $__5.address,
                  dataType = ($__6 = $__5.dataType) === void 0 ? 'string' : $__6,
                  type = ($__7 = $__5.type) === void 0 ? 3 : $__7,
                  numFmt = $__5.numFmt;
              var $__8 = address,
                  row = $__8.row,
                  column = $__8.column;
              assert.deepEqual($traceurRuntime.typeof($__3.worksheet.getCell(row, column).value), dataType, ("typeof this.worksheet.getCell(" + row + ", " + column + ").value"));
              assert.deepEqual($__3.worksheet.getCell(row, column).type, type, ("this.worksheet.getCell(" + row + ", " + column + ").type"));
              assert.deepEqual($__3.worksheet.getCell(row, column).numFmt, numFmt, ("this.worksheet.getCell(" + row + ", " + column + ").numFmt"));
            });
          }
        }, {});
      }();
      ExcelJSPivotGridTestHelper = function($__super) {
        function ExcelJSPivotGridTestHelper() {
          $traceurRuntime.superConstructor(ExcelJSPivotGridTestHelper).apply(this, arguments);
        }
        return ($traceurRuntime.createClass)(ExcelJSPivotGridTestHelper, {
          checkCustomizeCell: function(eventArgs, expectedCells, callIndex) {
            var pivotCell = eventArgs.pivotCell;
            var expectedCell = $traceurRuntime.superGet(this, ExcelJSPivotGridTestHelper.prototype, "checkCustomizeCell").call(this, eventArgs, expectedCells, callIndex);
            assert.notStrictEqual(pivotCell, undefined, 'pivotCell property exist');
            var $__9 = function(propertyName) {
              if (propertyName === 'path' || propertyName === 'rowPath' || propertyName === 'columnPath') {
                ...
                pivotCell[propertyName].forEach(function(pathValue, index) {
            ...
            for (var propertyName in pivotCell) {
              $__9(propertyName);
            }
          },
          extendExpectedCells: function(cellsArray, topLeft, options) {
            ...
            $traceurRuntime.superGet(this, ExcelJSPivotGridTestHelper.prototype, "_extendExpectedCells").call(this, cellsArray, topLeft, function(cellArgs, rowIndex, columnIndex) {
          ...
          getAllRealColumnWidths: function(component) {
            return $traceurRuntime.spread(this.getRowsAreaColumnWidths(component), this.getColumnsAreaColumnWidths(component));
          },
          getColumnsAreaColumnWidths: function(component) {
            var columnWidths = [];
            var showTotalsPrior = component.option('showTotalsPrior');
            var columnsAreaColumns = component.$element().find('.dx-pivotgrid-horizontal-headers tr')[showTotalsPrior === 'columns' || showTotalsPrior === 'both' ? 'first' : 'last']().find('td');
            columnsAreaColumns.each(function(_, element) {
              return columnWidths.push(element.getBoundingClientRect().width);
            });
            return columnWidths;
          },
          getRowsAreaColumnWidths: function(component) {
            var columnWidths = [];
            var rowsAreaColumns = component.$element().find('.dx-pivotgrid-vertical-headers tr').first().find('td');
            rowsAreaColumns.each(function(_, element) {
              return columnWidths.push(element.getBoundingClientRect().width);
            });
            return columnWidths;
          },
          toExcelWidths: function(widths) {
            return widths.map(function(width) {
              return Math.floor(width / Export.__internals.MAX_DIGIT_WIDTH_IN_PIXELS * 100) / 100;
            });
          }
        }, {}, $__super);
      }(ExcelJSTestHelper);
      ExcelJSDataGridTestHelper = function($__super) {
        function ExcelJSDataGridTestHelper() {
          $traceurRuntime.superConstructor(ExcelJSDataGridTestHelper).apply(this, arguments);
        }
        return ($traceurRuntime.createClass)(ExcelJSDataGridTestHelper, {
          checkCustomizeCell: function(eventArgs, expectedCells, callIndex) {
            var $__8;
            var $__5 = eventArgs,
                gridCell = ($__8 = $__5.gridCell) === void 0 ? {} : $__8;
            var expectedCell = $traceurRuntime.superGet(this, ExcelJSDataGridTestHelper.prototype, "checkCustomizeCell").call(this, eventArgs, expectedCells, callIndex);
            var expectedColumn = expectedCell.gridCell.column;
            var actualColumn = gridCell.column;
            ...
            var gridCellSkipProperties = ['column'];
            for (var propertyName in gridCell) {
          ...
          _extendExpectedCells: function(cellsArray, topLeft) {
            $traceurRuntime.superGet(this, ExcelJSDataGridTestHelper.prototype, "_extendExpectedCells").call(this, cellsArray, topLeft, function(cellArgs, rowIndex, columnIndex) {
              var $__8;
              var $__5 = cellArgs,
                  excelCell = $__5.excelCell,
                  gridCell = ($__8 = $__5.gridCell) === void 0 ? {} : $__8;
      ...
      $__export("ExcelJSPivotGridTestHelper", ExcelJSPivotGridTestHelper), $__export("ExcelJSDataGridTestHelper", ExcelJSDataGridTestHelper);
    }
  };
});
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["core/utils/type","exporter/exceljs/export"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("core/utils/type"), require("exporter/exceljs/export"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=ExcelJSTestHelper.js.map