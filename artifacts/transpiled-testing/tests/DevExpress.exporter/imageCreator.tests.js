...
(["testing/tests/DevExpress.exporter/imageCreator.tests.js"], ["jquery","exporter","core/utils/type","core/utils/svg","core/utils/deferred","core/utils/window"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.exporter/imageCreator.tests.js", ["jquery", "exporter", "core/utils/type", "core/utils/svg", "core/utils/deferred", "core/utils/window"], function($__export) {
  "use strict";
  var $,
      exporter,
      imageCreator,
  ...
  function setupCanvasStub(drawnElements, paths) {
    ...
    sinon.stub(prototype, 'closePath', function() {
      paths[paths.length - 1].push({action: 'Z'});
    });
    ...
    function getFontParam(fontString, paramType) {
      ...
      var matches = patterns[paramType] && fontString.match(new RegExp(patterns[paramType], 'i'));
    ...
    sinon.stub(prototype, 'fillText', function() {
      ...
      var fontParts = tempFont.split('__');
      var style = {
    ...
    sinon.stub(prototype, 'createPattern', function() {
      drawnElements.push({type: 'pattern'});
    });
  ...
  function getData(markup, isFullMode) {
    return imageCreator.getData(markup, {
      width: 500,
      height: 250,
      format: 'png',
      margin: 10
    }, isFullMode !== undefined ? isFullMode : true);
  }
  return {
    setters: [function($__m) {
      $ = $__m.default;
    }, function($__m) {
      exporter = $__m.default;
    }, function($__m) {
      typeUtils = $__m.default;
    }, function($__m) {
      svgUtils = $__m.default;
    }, function($__m) {
    ...
    execute: function() {
      imageCreator = exporter.image.creator;
      ...
      QUnit.test('Canvas size', function(assert) {
        ...
        var imageBlob = exporter.image.getData(testingMarkupStart + testingMarkupEnd, {
          format: 'png',
          width: 500,
          height: 250,
          margin: 10
        ...
        $.when(imageBlob).done(function() {
          var createCanvas = exporter.image.creator._createCanvas;
      ...
      QUnit.test('Canvas size. Scaled screen', function(assert) {
        var done = assert.async();
        var srcPixelRatio = window.devicePixelRatio;
      ...
      QUnit.test('toDataURL ImageQuality', function(assert) {
        var done = assert.async();
        var imageBlob = exporter.image.getData(testingMarkupStart + testingMarkupEnd, {format: 'png'});
        assert.expect(2);
        $.when(imageBlob).done(function() {
          try {
      ...
      QUnit.test('Special symbols drown on canvas correct', function(assert) {
        var that = this;
        var done = assert.async();
        var imageBlob = imageCreator.getData(testingMarkupStart + '<g class=\'dxc-title\' transform=\'translate(0,0)\'><text x=\'0\' y=\'30\' transform=\'translate(160,0)\' text-anchor=\'middle\'>Специальные символы</text></g>' + testingMarkupEnd, {
          width: 500,
          height: 250,
          format: 'png'
        });
      ...
      QUnit.test('Defined background', function(assert) {
        var that = this;
        var done = assert.async();
        var imageBlob = imageCreator.getData(testingMarkupStart + '<polygon points=\'220,10 300,210 170,250 123,234\' style=\'fill:lime;stroke:purple;stroke-width:1\'/>' + testingMarkupEnd, {
          width: 560,
          height: 290,
          margin: 10,
          format: 'png',
      ...
      QUnit.test('Transformation of canvas context args (T892041, T1020859)', function(assert) {
        ...
        var srcPixelRatio = window.devicePixelRatio;
        window.devicePixelRatio = 2;
        var context = window.CanvasRenderingContext2D.prototype;
        var imageBlob = imageCreator.getData(testingMarkupStart + '<polygon points=\'220,10 300,210 170,250 123,234\' style=\'fill:lime;stroke:purple;stroke-width:1\'/>' + testingMarkupEnd, {
          width: 560,
          height: 290,
          format: 'png'
      ...
      QUnit.test('Path', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 L 331 280" stroke-width="2" stroke="#FF0000"></path>' + testingMarkupEnd;
      ...
      QUnit.test('Path with missed L directive', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 200 100 300 200 L 331 280" stroke-width="2" stroke="#FF0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(7);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Path with no spaces between directives and with comma separated coords', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M36,181L184,98,200,100,300,200L331,280" stroke-width="2" stroke="#FF0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(7);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Closed path', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 L 331 280 Z" stroke-width="2" stroke="#FF0000" fill="none"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(2);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Several closed paths (T809807)', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 L 331 280 Z M 36 181 L 184 98 L 331 280 Z" stroke-width="2" stroke="#FF0000" fill="none"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(3);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Filled path', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 L 331 280 Z" stroke-width="0" stroke="none" opacity="0.5" fill="#ff0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(2);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Bezier path', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 61 53 C 61 53 45 53 87 50 C 29 45 71 47 13 26" stroke-width="2" stroke="#FF0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(4);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Path with fill none and parent fill', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<g fill="#FF0000"><path d="M 86 373 C 86 373 124 373 143 369 C 161.66666666666666 364.3746312684366 180.33333333333334 368 199 345 C 217.66666666666666 322 236.33333333333334 267.1666666666667 255 231 C 273.6666666666667 194.83333333333334 292.3333333333333 150.13569321533922 311 128 C 330 105.46902654867256 368 97 368 97" fill="none" stroke="#955f71" stroke-width="2"></path></g>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(3);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Arc path', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 10 10 L 20 20 16 28 A 15 15 0 0 0 14 15 L 99 15 A 0 0 0 0 1 99 15" stroke-width="1" stroke="#ffffff" stroke-linejoin="round" fill="#ff0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(15);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Arc path (relative coords)', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 10 10 L 20 20 16 28 a 15 15 0 0 0 -2 -13 L 99 15 a 0 0 0 0 1 0 0" stroke-width="1" stroke="#ffffff" stroke-linejoin="round" fill="#ff0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(12);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Skip unknown command', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<path d="M 36 181 L 184 98 L 331 280 V8 Z" stroke-width="2" stroke="#FF0000"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(9);
        $.when(imageBlob).done(function() {
          try {
            ...
            assert.deepEqual(that.paths[0][3], {action: 'Z'}, 'Third component of path');
          } finally {
            done();
          }
        });
      ...
      QUnit.test('Rect', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="81" y="24" width="118" height="168" transform="translate(250,230)" stroke-dasharray="8,6" opacity="0.5" stroke-width="12" stroke="#955f71" fill="#955f71"></rect>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var context = window.CanvasRenderingContext2D.prototype;
        assert.expect(10);
        $.when(imageBlob).done(function(blob) {
      ...
      QUnit.test('Rect with cornerRadius', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="21" y="100" rx="10" ry="10" width="118" height="168" transform="translate(250,230)" stroke-width="7" stroke="#955f71" fill="#955f71"></rect>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var context = window.CanvasRenderingContext2D.prototype;
        assert.expect(10);
        $.when(imageBlob).done(function(blob) {
      ...
      QUnit.test('Rect width cornerRadius exceed 1/2 height or 1/2 width', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="21" y="100" rx="30" ry="30" width="118" height="30" transform="translate(250,230)" stroke-width="7" stroke="#955f71" fill="#955f71"></rect>' + '<rect x="21" y="100" rx="30" ry="30" width="50" height="100" transform="translate(250,230)" stroke-width="7" stroke="#955f71" fill="#955f71"></rect>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(2);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Stroke-opacity / Fill-opacity', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="21" y="100" width="118" height="30" transform="translate(250,230)" stroke-width="7" stroke-opacity="0.7" fill-opacity="0.5" stroke="#955f71" fill="#955f71"></rect>' + '<rect x="21" y="100" width="118" height="30" transform="translate(250,230)" stroke-width="7" opacity="0.5" stroke="#955f71" fill="#955f71"></rect>' + '<rect x="21" y="100" width="118" height="30" transform="translate(250,230)" stroke-width="7" stroke-opacity="0.7" fill-opacity="0.5" opacity="0.5" stroke="#955f71" fill="#955f71"></rect>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(6);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Filter shadow', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<defs>' + '<filter id="testFilter1" x="-40%" y="-40%" width="180%" height="200%" transform="translate(0,0)"><feGaussianBlur in="SourceGraphic" result="gaussianBlurResult" stdDeviation="1"></feGaussianBlur><feOffset in="gaussianBlurResult" result="offsetResult" dx="0" dy="1"></feOffset><feFlood result="floodResult" flood-color="#223387" flood-opacity="0.2"></feFlood><feComposite in="floodResult" in2="offsetResult" operator="in" result="compositeResult"></feComposite><feComposite in="SourceGraphic" in2="compositeResult" operator="over"></feComposite></filter>' + '<filter id="testFilter1-part-1" x="-40%" y="-40%" width="180%" height="200%" transform="translate(0,0)"><feGaussianBlur in="SourceGraphic" result="gaussianBlurResult" stdDeviation="1"></feGaussianBlur><feOffset in="gaussianBlurResult" result="offsetResult" dx="0" dy="1"></feOffset><feFlood result="floodResult" flood-color="#223387" flood-opacity="0.2"></feFlood><feComposite in="floodResult" in2="offsetResult" operator="in" result="compositeResult"></feComposite><feComposite in="SourceGraphic" in2="compositeResult" operator="over"></feComposite></filter>' + '<filter id="testFilter1-part-2" x="-40%" y="-40%" width="180%" height="200%" transform="translate(0,0)"><feGaussianBlur in="SourceGraphic" result="gaussianBlurResult" stdDeviation="1"></feGaussianBlur><feOffset in="gaussianBlurResult" result="offsetResult" dx="0" dy="1"></feOffset><feFlood result="floodResult" flood-color="#223387" flood-opacity="0.2"></feFlood><feComposite in="floodResult" in2="offsetResult" operator="in" result="compositeResult"></feComposite><feComposite in="SourceGraphic" in2="compositeResult" operator="over"></feComposite></filter>' + '</defs>' + '<circle cx="0" cy="0" r="4" transform="translate(0,0)" filter="none" stroke="none" stroke-width="0" fill="#ffffff" opacity="0.32"></circle>' + '<circle cx="0" cy="0" r="4" transform="translate(0,0)" filter="url(#testFilter1-part-2)" stroke="#ffffff" stroke-width="2" fill="#ba4d51"></circle>' + '<circle cx="20" cy="20" r="4" filter="url(someurlpart#testFilter2-part-1)" fill="#ba4d51"></circle>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(4);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('lineargradient', function(assert) {
        var $__2 = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<defs>' + '<linearGradient id="testlineargradient1">' + '<stop offset="0%" stop-color="red"></stop>' + '<stop offset="100%" stop-color="blue"></stop>' + '</linearGradient>' + '</defs>' + '<path d="M 0 0 C 10 10 20 20 30 20 Z" fill="url(#testlineargradient1)" opacity="0.3"></path>' + '<path d="M 0 0 C 10 10 20 20 30 20 Z" fill="url(#testlineargradient2)" opacity="0.3"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        $.when(imageBlob).done(function() {
          try {
            assert.strictEqual($__2.drawnElements.length, 3, 'Canvas elements count');
            assert.strictEqual($__2.drawnElements[1].type, 'linearGradient', 'First element is linearGradient');
            assert.deepEqual($__2.drawnElements[1].args, {
              x0: 0,
              y0: 0,
              x1: 30,
              y1: 0
            });
            assert.strictEqual($__2.drawnElements[1].addColorStop.callCount, 2);
            assert.deepEqual($__2.drawnElements[1].addColorStop.getCall(0).args, [0, 'red']);
            assert.deepEqual($__2.drawnElements[1].addColorStop.getCall(1).args, [1, 'blue']);
            assert.roughEqual($__2.drawnElements[2].style.globalAlpha, 0.3, 0.1);
            assert.strictEqual($__2.drawnElements[2].style.fillStyle, '#aaaaaa');
            assert.strictEqual($__2.drawnElements[2].type, 'fill', 'fill');
          } finally {
            done();
          }
        });
      ...
      QUnit.test('lineargradient with rotation angle', function(assert) {
        var $__2 = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<defs>' + '<linearGradient id="testlineargradient1" gradientTransform="rotate(45)">' + '<stop offset="0%" stop-color="red"></stop>' + '<stop offset="100%" stop-color="blue"></stop>' + '</linearGradient>' + '</defs>' + '<path d="M 0 0 C 10 10 20 20 30 20 Z" fill="url(#testlineargradient1)" opacity="0.3"></path>' + '<path d="M 0 0 C 10 10 20 20 30 20 Z" fill="url(#testlineargradient2)" opacity="0.3"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var context = window.CanvasRenderingContext2D.prototype;
        $.when(imageBlob).done(function() {
          try {
      ...
      QUnit.test('radialgradient', function(assert) {
        var $__2 = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<defs>' + '<radialGradient id="testradialgradient">' + '<stop offset="0%" stop-color="red"></stop>' + '<stop offset="100%" stop-color="blue"></stop>' + '</radialGradient>' + '</defs>' + '<path d="M 0 0 C 10 10 20 20 30 20 Z" fill="url(#testradialgradient)" opacity="0.3"></path>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        $.when(imageBlob).done(function() {
          try {
            assert.strictEqual($__2.drawnElements.length, 3, 'Canvas elements count');
            assert.strictEqual($__2.drawnElements[1].type, 'radialGradient', 'First element is radialGradient');
            assert.deepEqual($__2.drawnElements[1].args, {
              r0: 0,
              r1: 15,
              x0: 15,
      ...
      QUnit.test('Circle', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<circle cx="0" cy="0" r="140" transform="translate(250,230)" stroke-dasharray="5,10" opacity="0.5" fill="#F05B41" stroke="#955f71"></circle>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(5);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Image with href', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" href="/testing/content/exporterTestsContent/test-image.png" visibility="visible"></image>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(3);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Image with 404 href', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" href="/wrongurl" visibility="visible"></image>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(1);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Export image in group', function(assert) {
        var $__2 = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<g transform="translate(10, 10)"><image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" href="/testing/content/exporterTestsContent/test-image.png" visibility="visible"></image></g>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var context = window.CanvasRenderingContext2D.prototype;
        assert.expect(5);
        $.when(imageBlob).done(function() {
          try {
            assert.equal($__2.drawnElements.length, 2, 'Canvas elements count');
            assert.equal($__2.drawnElements[1].type, 'image');
      ...
      QUnit.test('Text', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="20" y="30" text-anchor="middle" style="font-style: italic; font-size:16px; font-family:\'Segoe UI Light\', \'Helvetica Neue Light\', \'Segoe UI\', \'Helvetica Neue\', \'Trebuchet MS\', Verdana; font-weight:bold; fill:#232323; opacity: 0.3;">Test</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(12);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Defaults of text', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="20" y="30" text-anchor="middle" style="font-style: italic; font-weight:bold; opacity: 0.3;">Test</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(4);
        $.when(imageBlob).done(function(blob) {
          try {
            assert.equal(that.drawnElements.length, 2, 'Canvas elements count');
            var style = that.drawnElements[1].style;
      ...
      QUnit.test('Text with zero opacity in parent group', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<g opacity="0"><text x="20" y="30">Test</text></g>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        $.when(imageBlob).done(function() {
          try {
            assert.equal(that.drawnElements.length, 2, 'Canvas elements count');
      ...
      QUnit.test('Text offset position calculation with start alignment', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="10" y="30" style="font-size:28px; fill:#232323; font-family: sans-serif;">' + '<tspan x="10" y="30">World</tspan><tspan>Population</tspan><tspan>by Decade</tspan></text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var canvas = $('<canvas width=\'1000\' height=\'1000\'>').appendTo('#qunit-fixture')[0];
        ...
        $.when(imageBlob).done(function(blob) {
          try {
            ...
            var part2Width = ctx.measureText('Population').width;
            assert.equal(that.drawnElements.length, 4, 'Canvas elements count');
      ...
      QUnit.test('Text with default coordinates', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text text-anchor="middle" style="font-size:28px; fill:#232323; font-family: sans-serif;">Text</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(2);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('tspan element inside other tspan', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="100" y="100" text-anchor="middle" style="font-size:28px; fill:#232323; font-family: sans-serif;">' + '<tspan x="0" y="30" text-anchor="start">' + '<tspan>Imports, Dec 2014 - May 2015</tspan>' + '<tspan style="font-size:13px;" dy="-5">(Mod)</tspan>' + '</tspan>' + '<tspan x="0" y="100" text-anchor="start">' + '<tspan>Second Line</tspan>' + '</tspan>' + '</text > ' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(6);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Text offset position calculation with center alignment', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="10" y="30" text-anchor="middle" style="font-size:28px; fill:#232323; font-family: sans-serif;">' + '<tspan x="10" y="30">World</tspan><tspan>Population</tspan><tspan>by Decade</tspan></text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var canvas = $('<canvas width=\'1000\' height=\'1000\'>').appendTo('#qunit-fixture')[0];
        var ctx = canvas.getContext('2d');
        ctx.font = '28px sans-serif';
        ...
        $.when(imageBlob).done(function(blob) {
          try {
            var part1Width = ctx.measureText('World').width;
            var part2Width = ctx.measureText('Population').width;
            var part3Width = ctx.measureText('by Decade').width;
            var center = (part1Width + part2Width + part3Width) / 2;
            assert.equal(that.drawnElements.length, 4, 'Canvas elements count');
      ...
      QUnit.test('Text offset position calculation with end alignment', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="10" y="30" text-anchor="end" style="font-size:28px; fill:#232323; font-family: sans-serif;">' + '<tspan x="10" y="30">World</tspan><tspan>Population</tspan><tspan>by Decade</tspan></text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var canvas = $('<canvas width=\'1000\' height=\'1000\'>').appendTo('#qunit-fixture')[0];
        var ctx = canvas.getContext('2d');
        ctx.font = '28px sans-serif';
        ...
        $.when(imageBlob).done(function(blob) {
          try {
            ...
            var part2Width = ctx.measureText('Population').width;
            var part3Width = ctx.measureText('by Decade').width;
            var commonWidth = (part1Width + part2Width + part3Width);
            assert.equal(that.drawnElements.length, 4, 'Canvas elements count');
      ...
      QUnit.test('Multiline text', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="10" y="30" text-anchor="start" style="font-size:28px; fill:#232323; font-family: sans-serif;">' + '<tspan x="10" y="30">World </tspan>' + '<tspan x="10" dy="28">Populationby</tspan>' + '<tspan x="10" y="30">World </tspan>' + '<tspan x="10" dy="34px">Populationby</tspan>' + '<tspan x="10" dy="16">Populationby</tspan>' + '</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(11);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Stroke text', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="50" y="50" text-anchor="start" stroke-width="5" style="fill:#222; font-family:\'Trebuchet MS\', Verdana; stroke: #F2f2f2; stroke-width: 5px;"><tspan style="font-weight: bold; font-style: italic; " stroke-opacity="0.7">Age</tspan></text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(14);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Multiline text with shadow and stroked texts', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<defs>' + '<filter id="testFilter1" x="-40%" y="-40%" width="180%" height="200%" transform="translate(0,0)"><feGaussianBlur in="SourceGraphic" result="gaussianBlurResult" stdDeviation="1"></feGaussianBlur><feOffset in="gaussianBlurResult" result="offsetResult" dx="0" dy="1"></feOffset><feFlood result="floodResult" flood-color="#223387" flood-opacity="0.2"></feFlood><feComposite in="floodResult" in2="offsetResult" operator="in" result="compositeResult"></feComposite><feComposite in="SourceGraphic" in2="compositeResult" operator="over"></feComposite></filter>' + '</defs>' + '<text filter="url(#testFilter1)" style="white-space: pre; fill: #232323; font-style: italic; font-weight: bold; font-family: \'Trebuchet MS\', Verdana; font-size: 14px; cursor: default;" x="0" y="0" transform="translate(463,51)">' + '<tspan x="0" y="0" stroke="#f2f2f2" stroke-width="1" stroke-opacity="0.3" stroke-linejoin="round">Text1</tspan>' + '<tspan x="0" dy="13px" stroke="#f2f2f2" stroke-width="1" stroke-opacity="0.3" stroke-linejoin="round">Text2</tspan>' + '<tspan x="0" y="0">Text1</tspan>' + '<tspan x="0" dy="13px">Text2</tspan>' + '</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        $.when(imageBlob).done(function(blob) {
          try {
            assert.equal(that.drawnElements.length, 7, 'Canvas elements count');
      ...
      QUnit.test('Text with ellipsis', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="50" y="30" transform="translate(0,0)" text-anchor="middle" style="font-size:28px;font-family:\'Segoe UI Light\', \'Helvetica Neue Light\', \'Segoe UI\', \'Helvetica Neue\', \'Trebuchet MS\', Verdana;font-weight:200;fill:#232323;cursor:default;">Test...<title>Test test</title></text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(5);
        $.when(imageBlob).done(function(blob) {
          try {
            assert.equal(that.drawnElements.filter(function(e) {
              return e.type === 'text';
            }).length, 1, 'Canvas text elements count');
      ...
      QUnit.test('Text with °. On error behavior', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="0" y="0" transform="translate(100,10) rotate(270,100,10)" style="fill:#767676;font-size:16px;font-family:\'Segoe UI\', \'Helvetica Neue\', \'Trebuchet MS\', Verdana;font-weight:400;" text-anchor="middle">Temperature, °C</text>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(3);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Text decoration', function(assert) {
        ...
        var done = assert.async();
        var context = window.CanvasRenderingContext2D.prototype;
        var markup = testingMarkupStart + '<text x="0" y="50" style="font-family:\'Segoe UI Light\'" text-anchor="start"><tspan>Before text... </tspan>' + '<tspan x="500" y="90" text-anchor="end" style="text-decoration:underline; font-size:38px; fill:#23FF23;">Underlined text</tspan>' + '<tspan x="250" y="30" text-anchor="center" style="text-decoration:overline; font-size:24px; fill:#AAFF23;">Overlined text</tspan>' + '<tspan x="0" y="160" text-anchor="start" style="text-decoration:line-through; font-size:14px; fill:#23FFFF;">Line-through text</tspan>' + '<tspan x="250" y="190" text-anchor="middle">After text</tspan>' + '<tspan x="250" y="190" text-anchor="middle" style="text-decoration:line-through;" fill="none" stroke="none">No filled text(no display)</tspan>' + '<tspan x="250" y="190" text-anchor="middle" style="text-decoration:line-through;" fill="none" stroke="#222">No filled text(no display)</tspan>' + '</text>' + testingMarkupEnd;
        context.measureText.withArgs('Underlined text').returns({width: 50});
        var imageBlob = getData(markup, {
          width: 500,
          height: 250,
          format: 'png'
        ...
        $.when(imageBlob).done(function(blob) {
          try {
            ...
            var overlineDecoration = that.drawnElements[6];
            var lineThroughDecoration = that.drawnElements[9];
            var noDisplayDecoration = that.drawnElements[13];
            var noFillDecoration = that.drawnElements[16];
      ...
      QUnit.test('Order of clip and translate', function(assert) {
        var done = assert.async();
        var markup = testingMarkupStart + '<defs><clipPath id="DevExpress_13"><rect x="62" y="78" width="736" height="309" transform="translate(0,0)"></rect></clipPath></defs><g fill="#00ced1" stroke="#00ced1" stroke-width="0" clip-path="url(#DevExpress_13)"><circle cx="0" cy="0" r="28" opacity="0.5" transform="translate(404,245)"></circle></g>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        var context = window.CanvasRenderingContext2D.prototype;
        assert.expect(13);
        $.when(imageBlob).done(function(blob) {
      ...
      QUnit.test('Elements with visibility', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<g visibility="hidden"><rect x="20" y="20" width="200" height="200" fill="#FF0000"></rect><rect x="50" y="50" width="200" height="200" fill="#00FF00" visibility="visible"></rect></g>' + testingMarkupEnd;
        var imageBlob = getData(markup);
        assert.expect(3);
        $.when(imageBlob).done(function(blob) {
          try {
      ...
      QUnit.test('Do not export elements with \'hidden-for-export\' attribute', function(assert) {
        var $__2 = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<g hidden-for-export="true"><rect x="20" y="20" width="200" height="200" fill="#FF0000"></rect></g><rect x="50" y="50" width="200" height="200" fill="#00FF00"></rect>' + testingMarkupEnd;
        assert.expect(3);
        getData(markup).then(function() {
          try {
            assert.equal($__2.drawnElements.length, 3, 'Canvas elements count');
            assert.deepEqual($__2.drawnElements[1].args, {
              x: 50,
              y: 50,
              width: 200,
            ...
            assert.deepEqual($__2.drawnElements[2].style, {
              fillStyle: '#00ff00',
              globalAlpha: 1
            }, 'Rect style');
          } finally {
      ...
      QUnit.test('Defined background color', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="50" y="50" width="200" height="200" fill="#00FF00" visibility="visible"></rect>' + testingMarkupEnd;
      ...
      QUnit.test('Export.color option', function(assert) {
        ...
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="50" y="50" width="200" height="200" fill="#00FF00" visibility="visible"></rect>' + testingMarkupEnd;
        var imageBlob = getData(markup, {color: '#aaaaaa'});
        assert.expect(1);
        $.when(imageBlob).done(function(blob) {
          try {
            assert.deepEqual(that.drawnElements[0].style, {
      ...
      QUnit.test('Read computed style of elements if export target is attached element', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<text x="20" y="30" style="font-style: italic; font-size:16px; font-family:\'Segoe UI Light\', \'Helvetica Neue Light\', \'Segoe UI\', \'Helvetica Neue\', \'Trebuchet MS\', Verdana; font-weight:bold; fill:#232323; opacity: 0.3;">Test</text>' + testingMarkupEnd;
        var element = new window.DOMParser().parseFromString(markup, 'image/svg+xml').childNodes[0];
        $('#qunit-fixture').append(element);
        this.stubGetComputedStyle(element.childNodes[0], {
          fill: '#ff0000',
          'font-size': '25px',
          'font-style': ''
        });
      ...
      QUnit.test('Read computed style of elements. Ignore default opacity', function(assert) {
        var that = this;
        var done = assert.async();
        var markup = testingMarkupStart + '<rect x="20" y="30" width="100" height="200" fill="#232323" stroke="#454545" stroke-width="1" stroke-opacity="0.5" fill-opacity="0.5" opacity="0.3" ></rect>' + testingMarkupEnd;
        var element = new window.DOMParser().parseFromString(markup, 'image/svg+xml').childNodes[0];
        $('#qunit-fixture').append(element);
        this.stubGetComputedStyle(element.childNodes[0], {
          fill: '#ff0000',
      ...
      QUnit.test('Processing of inner SVG', function(assert) {
        var done = assert.async();
        var markup = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="820px" height="420px"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"><rect x="1" y="2" width="10" height="30"></rect></svg></svg>';
        var context = window.CanvasRenderingContext2D.prototype;
        var imageBlob = getData(markup);
        assert.expect(1);
        $.when(imageBlob).done(function() {
      ...
      QUnit.test('getElementOptions should work correctly with empty attributs element', function(assert) {
        ...
        var imageBlob = imageCreator.getData(markup, {__parseAttributesFn: function(attributes) {
      ...
      QUnit.test('Custom parser can be asynchronous', function(assert) {
        var done = assert.async();
        var markup = testingMarkupStart + testingMarkupEnd;
      ...
      QUnit.test('Large array items. All items sync', function(assert) {
        ...
        var asyncEachDeferred = exporter.image.asyncEach(new Array(arraySize), callbackSpy);
      ...
      QUnit.module('Tests with private API usage', {
        ...
        QUnit.test('getData returns Blob when it supported by Browser', function(assert) {
          ...
          var done = assert.async();
          var markup = '<svg></svg>';
          var deferred = imageCreator.getData(markup, {backgroundColor: '#aaa'});
          assert.expect(1);
        ...
        QUnit.test('data has base64 format if useBase64 is true(T1136337)', function(assert) {
          var done = assert.async();
          var markup = '<svg></svg>';
          var deferred = imageCreator.getData(markup, {useBase64: true});
          assert.expect(1);
          $.when(deferred).done(function(data) {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","exporter","core/utils/type","core/utils/svg","core/utils/deferred","core/utils/window"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("exporter"), require("core/utils/type"), require("core/utils/svg"), require("core/utils/deferred"), require("core/utils/window"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=imageCreator.tests.js.map