...
(["testing/tests/DevExpress.ui.widgets.editors/textEditorParts/markup.tests.js"], ["jquery","ui/text_box/ui.text_editor","core/utils/deferred","core/devices"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.editors/textEditorParts/markup.tests.js", ["jquery", "ui/text_box/ui.text_editor", "core/utils/deferred", "core/devices"], function($__export) {
  "use strict";
  var $,
      Deferred,
      devices,
    ...
    execute: function() {
      ...
      module('Basic markup', function() {
        ...
        test('init with placeholder in the input container', function(assert) {
          var element = $('#texteditor').dxTextEditor({placeholder: 'enter value'});
          var $inputContainer = element.find(("." + TEXTEDITOR_INPUT_CONTAINER_CLASS));
          assert.strictEqual($inputContainer.length, 1, 'input container is rendered');
          var placeholder = $inputContainer.find(("." + PLACEHOLDER_CLASS));
        ...
        test('value === 0 should be rendered on init', function(assert) {
          var $element = $('#texteditor').dxTextEditor({value: 0});
          var input = $element.find('.' + INPUT_CLASS);
        ...
        test('renderValue should return a promise that resolves after render input value', function(assert) {
          ...
          var editor = $('#texteditor').dxTextEditor({value: 'test'}).dxTextEditor('instance');
          var renderValueStub = sinon.spy(editor, '_renderValue');
          sinon.stub(editor, '_renderInputValue').returns(deferred.promise());
          editor.repaint();
          var promise = renderValueStub.getCall(0).returnValue;
        ...
        [true, false].forEach(function(isMac) {
          test(("\"placeholder\" attribute should be defined for iOS and mac devices (T898735,T964073). IsMac = " + isMac), function(assert) {
            ...
            try {
              ...
              var $editor = $('#texteditor').dxTextEditor();
              var isIos = devices.real().ios;
              var expectedPlaceholder = (isIos || isMac) ? ' ' : '';
              var placeholder = $editor.find(("." + INPUT_CLASS)).attr('placeholder') || '';
      ...
      module('the \'name\' option', function() {
        test('widget input should get the \'name\' attribute with a correct value', function(assert) {
          ...
          var $element = $('#texteditor').dxTextEditor({name: expectedName});
          var $input = $element.find(("." + INPUT_CLASS));
          assert.equal($input.attr('name'), expectedName, 'the input \'name\' attribute has correct value');
        });
      });
      module('basic options changing', {beforeEach: function() {
        ...
        test('the "inputAttr" option', function(assert) {
          var $element1 = $('<div>', {id: 'testDiv1'}).appendTo('#qunit-fixture');
          var $element2 = $('<div>', {id: 'testDiv1'}).appendTo('#qunit-fixture');
          var instance1 = $element1.dxTextEditor({inputAttr: {'data-test': 'test'}}).dxTextEditor('instance');
          var instance2 = $element2.dxTextEditor().dxTextEditor('instance');
          var $input1 = $element1.find('input');
          assert.ok($traceurRuntime.typeof((instance2.option('inputAttr'))) === 'object' && $.isEmptyObject(instance2.option('inputAttr')), 'Option is {} by default');
          ...
          instance1.option('inputAttr', {
            'data-test': 'changedValue',
            'data-anyattr': 'anyvalue'
          });
        ...
        test('partial update of the "inputAttr" option should not replace other attibutes', function(assert) {
          try {
            this.instance.option('inputAttr', {
              autocomplete: 'on',
              id: 'test-id-attr'
            });
            this.instance.option('inputAttr.title', 'new title');
        ...
        test('texteditor is clear when option \'value\' changed to null', function(assert) {
          var instance = $('#texteditor').dxTextEditor({value: 'test'}).dxTextEditor('instance');
        ...
        test('texteditor should trigger "text" option change event once after clear the value', function(assert) {
          var textChangedCounter = 0;
          var latestTextValue;
          this.instance.option('value', 'test');
          this.instance.on('optionChanged', function($__3) {
            var $__4 = $__3,
                name = $__4.name,
                value = $__4.value;
            if (name === 'text') {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","ui/text_box/ui.text_editor","core/utils/deferred","core/devices"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("ui/text_box/ui.text_editor"), require("core/utils/deferred"), require("core/devices"));
  else
