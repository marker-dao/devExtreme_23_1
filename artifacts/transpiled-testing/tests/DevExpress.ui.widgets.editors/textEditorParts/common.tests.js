System.register(["jquery", "events/core/events_engine", "core/utils/dom", "core/devices", "../../../helpers/pointerMock.js", "../../../helpers/keyboardMock.js", "./caretWorkaround.js", "ui/themes", "core/config", "core/utils/console", "events/utils/index", "core/utils/size", "ui/text_box/ui.text_editor", "ui/text_box/ui.text_editor.label.js"], function (_export, _context) {
  "use strict";

  var $, eventsEngine, domUtils, devices, pointerMock, keyboardMock, caretWorkaround, themes, config, consoleUtils, normalizeKeyName, getWidth, TextEditor, TextEditorLabel, TEXTEDITOR_CLASS, INPUT_CLASS, CONTAINER_CLASS, DISABLED_CLASS, STATE_FOCUSED_CLASS, EMPTY_INPUT_CLASS, CLEAR_BUTTON_SELECTOR, PLACEHOLDER_CLASS, LABEL_CLASS, INVISIBLE_STATE_CLASS, BUTTONS_CONTAINER_CLASS, EVENTS, moduleConfig, prepareEvent;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_eventsCoreEvents_engine) {
      eventsEngine = _eventsCoreEvents_engine.default;
    }, function (_coreUtilsDom) {
      domUtils = _coreUtilsDom.default;
    ...
    }, function (_caretWorkaroundJs) {
      caretWorkaround = _caretWorkaroundJs.default;
    }, function (_uiThemes) {
      themes = _uiThemes.default;
    }, function (_coreConfig) {
      config = _coreConfig.default;
    }, function (_coreUtilsConsole) {
      consoleUtils = _coreUtilsConsole.default;
    ...
    }, function (_uiText_boxUiText_editor) {
      TextEditor = _uiText_boxUiText_editor.default;
    }, function (_uiText_boxUiText_editorLabelJs) {
      TextEditorLabel = _uiText_boxUiText_editorLabelJs.TextEditorLabel;
    }],
    execute: function () {
      ...
      LABEL_CLASS = 'dx-texteditor-label';
      INVISIBLE_STATE_CLASS = 'dx-state-invisible';
      BUTTONS_CONTAINER_CLASS = 'dx-texteditor-buttons-container';
      EVENTS = ['FocusIn', 'FocusOut', 'KeyDown', 'KeyUp', 'Change', 'Cut', 'Copy', 'Paste', 'Input'];
      moduleConfig = {
        beforeEach: function () {
      ...
      QUnit.module('general', {}, () => {
        ...
        QUnit.test('T220209 - the \'displayValueFormatter\' option when value is changed using keyboard', function (assert) {
          ...
          const $input = $textEditor.find('.dx-texteditor-input');
          keyboardMock($input).press('end').type('2');
      ...
      QUnit.module('label integration', {
        beforeEach: function () {
          ...
          this.init = (options = {}) => {
            this.textEditor = this.$textEditor.dxTextEditor($.extend(initialOptions, options)).dxTextEditor('instance');
      ...
      QUnit.module('options changing', moduleConfig, () => {
        ...
        QUnit.test('the "value" option should changed by pressing Ctrl+Enter', function (assert) {
          ...
          this.keyboard.type('123').triggerEvent('keydown', {
            key: 'Enter',
            ctrlKey: true
          });
          ...
          this.keyboard.triggerEvent('keydown', {
            key: 'Enter',
            ctrlKey: true
          });
      ...
      QUnit.module('regressions', moduleConfig, () => {
        QUnit.test('event handlers are not set', function (assert) {
          assert.expect(0);
          const EVENTS = ['focusIn', 'focusOut', 'keyDown', 'keyUp', 'change'];
          const input = this.input;
        ...
        QUnit.test('B233277 dxNumberbox/dxTextbox - cursor jump over the right digit, impossible to remove all digits after the cursor moving', function (assert) {
          this.instance.option('valueChangeEvent', 'keyup');
          keyboardMock(this.element.find('.' + INPUT_CLASS)).type('123').press('left').type('4').press('backspace');
          assert.equal(this.instance.option('value'), '123');
        });
        QUnit.test('Text editor should propagate keyboard events to the document', function (assert) {
          ...
          keyboardMock(this.input).keyDown('enter').keyDown('space').keyDown('left').keyDown('right');
        ...
        QUnit.test('Enter key event raising (B238135)', function (assert) {
          ...
          $('#texteditor input').trigger($.Event('keyup', {
            key: 'Enter'
          }));
