...
(["testing/tests/DevExpress.ui.widgets.editors/numberBoxParts/mask.tests.js"], ["jquery","core/utils/browser","core/config","core/devices","../../../helpers/keyboardMock.js","localization/number","ui/text_box/ui.text_editor"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.editors/numberBoxParts/mask.tests.js", ["jquery", "core/utils/browser", "core/config", "core/devices", "../../../helpers/keyboardMock.js", "localization/number", "ui/text_box/ui.text_editor"], function($__export) {
  "use strict";
  var $,
      browser,
      config,
  ...
  return {
    ...
    execute: function() {
      ...
      QUnit.module('format: api value changing', moduleConfig, function() {
        ...
        QUnit.test('number type of input should be converted to tel on mobile device when inputMode is unsupported', function(assert) {
          ...
          try {
            ...
            var instance = $element.dxNumberBox({
              useMaskBehavior: true,
              format: '#',
              mode: 'number'
      ...
      QUnit.module('format: sign and minus button', moduleConfig, function() {
        ...
        QUnit.test('pressing numpad minus button should revert the number', function(assert) {
          ...
          this.keyboard.caret(3).keyDown(keyName).keyPress(keyName).keyUp(keyName).input();
          assert.equal(this.input.val(), '-123.456', 'value is correct');
          assert.equal(this.instance.option('value'), 123.456, 'value should not be changed before valueChange event');
          assert.deepEqual(this.keyboard.caret(), {
            start: 4,
        ...
        [{
          ...
          var $__5 = $__4,
              format = $__5.format,
              text = $__5.text,
              expectedCaretPosition = $__5.expectedCaretPosition;
          QUnit.test(("NumberBox should not reset the negativ value after valueChange event, format: " + format + " (T1092593)"), function(assert) {
            ...
            this.input.focus();
            this.keyboard.type('5').type('-').change();
      ...
      QUnit.module('format: fixed point format', moduleConfig, function() {
        ...
        QUnit.test('Value should be formatted after change on focusout if mask behavior is used (T942114)', function(assert) {
          ...
          this.keyboard.type('1.111').change();
        ...
        QUnit.test('should ignore backspace/delete key down when the caret in the start/end of input (T713045)', function(assert) {
          ...
          this.keyboard.caret(5).press('delete');
          assert.strictEqual(this.input.val(), '1,234');
          this.keyboard.caret(0).press('backspace');
          assert.strictEqual(this.input.val(), '1,234');
        });
        ...
        QUnit.test('value is not changed when using the big float part', function(assert) {
          ...
          this.keyboard.caret(0).type('3.4').press('enter').change();
      ...
      QUnit.module('format: minimum and maximum', moduleConfig, function() {
        ...
        QUnit.test('value removing should be possible when min is specified and stubs are in the format', function(assert) {
          ...
          this.keyboard.caret(1).press('backspace').type('7').press('enter');
      ...
      QUnit.module('format: arabic digit shaping', {
        ...
        QUnit.test('mask should work with arabic digit shaping', function(assert) {
          this.keyboard.type('١٢٣٤٥').press('backspace').change();
      ...
      QUnit.module('format: text input', moduleConfig, function() {
        ...
        QUnit.test('decimal point should move the caret before float part only', function(assert) {
          ...
          }, 'caret was moved to the float part');
        });
        ...
        QUnit.testInActiveWindow('caret position is not changed when the focus out event has occurred', function(assert) {
          ...
          var caretIsUpdatedOnFocusOut;
          var $input = $(this.instance.element()).find('.dx-texteditor-input');
          this.instance._caret = function(newCaret) {
            var result = _caret.apply($__3.instance, newCaret);
        ...
        QUnit.testInActiveWindow('caret position should be updated correctly after setting the "format" options runtime and "valueChangeEvent" is "input"', function(assert) {
          var $element = $('#widget');
          var instance = $element.dxNumberBox({valueChangeEvent: 'input'}).dxNumberBox('instance');
          instance.option('format', '$###,##0');
          var keyboard = keyboardMock($element.find(("." + INPUT_CLASS)), true);
          keyboard.caret(2).type('123456');
          assert.deepEqual(keyboard.caret(), {
            start: 8,
            end: 8
          }, 'caret position is OK');
          assert.strictEqual(instance.option('value'), 123456, 'Widget has the expected value');
        });
      });
      QUnit.module('format: incomplete value', moduleConfig, function() {
        ...
        QUnit.test('minus sign typed to the end of the incomplete value should revert sign', function(assert) {
          ...
          this.keyboard.caret(1).type('.0').type('-');
          assert.equal(this.input.val(), '-0', 'value has been reformatted after incorrect sign');
        });
      });
      QUnit.module('format: percent format', moduleConfig, function() {
      ...
      QUnit.module('format: removing', moduleConfig, function() {
        QUnit.test('delete key', function(assert) {
          ...
          }, 'caret should be moved throug the point');
        ...
        QUnit.test('removing with group separators using delete key', function(assert) {
          ...
          }, 'caret is good after selection removing');
        });
        ...
        QUnit.test('removing all digits with backspace should be possible when required zeros are in the end', function(assert) {
          ...
          this.keyboard.caret(5).press('backspace').press('backspace').press('backspace').press('backspace');
          assert.equal(this.input.val(), '0.00', 'value is correct');
        });
        QUnit.test('removing all digits should change value to 0 even if it\'s negative (T892552)', function(assert) {
          ...
          }).press('backspace').input('backspace').change();
        ...
        QUnit.test('removing decimal separator should be possible if float part is not required', function(assert) {
          ...
          this.keyboard.caret(4).press('backspace').press('backspace');
      ...
      QUnit.module('format: caret boundaries', moduleConfig, function() {
        ...
        QUnit.testInActiveWindow('caret should be before decimal separator on focusin', function(assert) {
          ...
          }, 'caret is just before decimal separator');
        });
        QUnit.test('caret should be moved to the integer part end on input click if format contains stub in the end (T996477)', function(assert) {
          ...
          }, 'caret is on integer part end');
        });
      });
      QUnit.module('format: custom parser and formatter', moduleConfig, function() {
        ...
        QUnit.test('editor should create a related LDML pattern for custom formatter only once', function(assert) {
          ...
          this.instance.option({
            value: null,
            format: {formatter: function(val) {
                counter++;
                return String(val);
              }}
          });
          var callCountAfterCreateLdmlPattern = counter;
          this.keyboard.type('1');
        ...
        QUnit.test('custom formatter can be passed right to "format" property (T1010539)', function(assert) {
          var callCount = 0;
          this.instance.option({format: function() {
              callCount++;
              return '';
            }});
          var callCountAfterCreateLdmlPattern = callCount;
          this.keyboard.type('1');
      ...
      QUnit.module('stubs', moduleConfig, function() {
        [{
          ...
          format: '0,###.##;abc(0,###.##)',
          expectedText: 'abc(1,234.56)',
          expectedValue: -1234.56
        }, {
          format: '#,##0\'.\'\'0\'\'0\'',
          expectedText: '1,234.00',
          expectedValue: 1234
        }, {
          format: '\'-\'#,###.00',
          expectedText: '-1.00',
          ...
          format: '\'plus\' 0;\'minus\' 0',
          expectedText: 'minus 1',
          typedText: '1-',
          expectedValue: -1
        }, {
          format: '\'plus\' 0;\'minus\' 0',
          ...
          var $__5 = $__4,
              format = $__5.format,
              expectedText = $__5.expectedText,
              typedText = $__5.typedText,
              expectedValue = $__5.expectedValue;
          QUnit.test(("widget should correctly apply format=\"" + format + "\", value=\"" + (typedText || expectedValue) + "\""), function(assert) {
            this.instance.option({
              value: null,
              format: format
            });
            this.keyboard.type(typedText || expectedValue.toString()).press('enter');
      ...
      QUnit.module('symbol with dot in format', {
        ...
        QUnit.test('should not change max precision (T972648)', function(assert) {
          this.keyboard.caret(4).type('89').change();
        ...
        QUnit.test('should not affect integer part length calculation', function(assert) {
          ...
          this.keyboard.caret(4).type('22').change();
        ...
        QUnit.test('should not prevent value formatting on every input', function(assert) {
          this.instance.option({format: '\'.\' 00.00'});
          this.keyboard.caret(4).type('2233445567');
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/browser","core/config","core/devices","../../../helpers/keyboardMock.js","localization/number","ui/text_box/ui.text_editor"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/browser"), require("core/config"), require("core/devices"), require("../../../helpers/keyboardMock.js"), require("localization/number"), require("ui/text_box/ui.text_editor"));
  else
