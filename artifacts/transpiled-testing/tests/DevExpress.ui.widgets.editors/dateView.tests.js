...
(["testing/tests/DevExpress.ui.widgets.editors/dateView.tests.js"], ["animation/fx","animation/translator","core/devices","events/visibility_change","generic_light.css!","jquery","localization/date","ui/date_box/ui.date_view","ui/date_box/ui.date_view_roller","../../helpers/executeAsyncMock.js","../../helpers/pointerMock.js","../DevExpress.ui.widgets/scrollableParts/scrollable.constants.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.editors/dateView.tests.js", ["animation/fx", "animation/translator", "core/devices", "events/visibility_change", "generic_light.css!", "jquery", "localization/date", "ui/date_box/ui.date_view", "ui/date_box/ui.date_view_roller", "../../helpers/executeAsyncMock.js", "../../helpers/pointerMock.js", "../DevExpress.ui.widgets/scrollableParts/scrollable.constants.js"], function($__export) {
  "use strict";
  var fx,
      translator,
      devices,
      ...
      RESIZE_WAIT_TIMEOUT,
      DATEVIEW_CLASS,
      DATEVIEW_WRAPPER_CLASS,
      DATEVIEW_ROLLER_CONTAINER_CLASS,
      DATEVIEW_ROLLER_CLASS,
      DATEVIEW_ROLLER_CURRENT_CLASS,
      DATEVIEW_ROLLER_ITEM_CLASS,
      DATEVIEW_ROLLER_ITEM_SELECTED_CLASS,
      DATEVIEW_ROLLER_ITEM_SELECTED_FRAME_CLASS,
      DATEVIEW_ROLLER_ITEM_SELECTED_BORDER_CLASS,
      commonConfig;
  return {
    ...
    execute: function() {
      ...
      QUnit.module('dateViewRoller', {
        beforeEach: function() {
          ...
          this.element = $('#roller').wrap('<div class=\'dx-dateview-wrapper\'>').wrap('<div class=\'dx-dateview-rollers\'>');
        ...
        QUnit.test('selected item should be changed by item click', function(assert) {
          ...
          var instance = $element.dxDateViewRoller('instance');
          var $items = $element.find('.' + DATEVIEW_ROLLER_ITEM_CLASS);
        ...
        QUnit.test('change selected item by option', function(assert) {
          ...
          var instance = element.dxDateViewRoller('instance');
          var content = element.find('.dx-scrollable-content');
          var itemHeight = element.find('.' + DATEVIEW_ROLLER_ITEM_CLASS).eq(0).outerHeight(true);
        ...
        QUnit.test('selected item active class', function(assert) {
          ...
          var instance = element.dxDateViewRoller('instance');
          var content = element.find('.dx-scrollable-content');
          var items = content.find('.' + DATEVIEW_ROLLER_ITEM_CLASS);
          var selected = 2;
        ...
        QUnit.test('change selected item by UI', function(assert) {
          ...
          var content = element.find('.dx-scrollable-content');
          var itemHeight = element.find('.' + DATEVIEW_ROLLER_ITEM_SELECTED_CLASS).eq(0).outerHeight(true);
        ...
        QUnit.test('scrolling by step (UI)', function(assert) {
          ...
          var content = element.find('.dx-scrollable-content');
          var itemHeight = element.find('.' + DATEVIEW_ROLLER_ITEM_SELECTED_CLASS).eq(0).outerHeight(true);
          var pointer = pointerMock(instance.container());
        ...
        QUnit.test('Content should not be have a transform css property after complete', function(assert) {
          ...
          var instance = element.dxDateViewRoller('instance');
          var $content = element.find('.dx-scrollable-content');
          var $container = element.find('.dx-scrollable-container');
          var itemHeight = element.find('.' + DATEVIEW_ROLLER_ITEM_SELECTED_CLASS).eq(0).outerHeight(true);
          var pointer = pointerMock(instance.container());
          pointer.start().down().move(0, -itemHeight * 0.7).wait(500).up();
          assert.deepEqual(translator.locate($content), {
            top: 0,
            left: 0
          });
      ...
      QUnit.module('dateView', {
        ...
        QUnit.test('active roller class', function(assert) {
          ...
          setTimeout(function() {
            triggerShownEvent('#qunit-fixture');
            setTimeout(function() {
              $.each($__3.instance._rollers, function(type) {
                var pointer = pointerMock(this.container());
        ...
        QUnit.test('min and max date should take hours into account', function(assert) {
          ...
          var min = new Date(2000, 5, 4, 8, 0, 0);
          var max = new Date(2000, 5, 6, 12, 0, 0);
          this.instance.option({
            type: 'datetime',
            value: value,
            minDate: min,
          ...
          var rollers = this.instance._rollers;
          var hourItems = rollers.hours.option('items');
        ...
        QUnit.test('it should be impossible to select date out of range via rollers', function(assert) {
          var date = new Date(2012, 9, 10);
          var minDate = new Date(2012, 7, 11);
          var maxDate = new Date(2012, 9, 11);
          this.instance.option({
            value: date,
            maxDate: maxDate,
            minDate: minDate
        ...
        QUnit.test('Dateview have all days in previous month with option minDate', function(assert) {
          var date = new Date(2012, 7, 31);
          var minDate = new Date(2005, 10, 15);
          this.instance.option({
            value: date,
            minDate: minDate
          });
        ...
        QUnit.test('rollers order accordingly date type', function(assert) {
          ...
          dateLocalization.inject({getFormatParts: function() {
        ...
        QUnit.test('regression: each date component has right count', function(assert) {
          ...
          var months = components.eq(0).find('.dx-dateview-item');
          var days = components.eq(1).find('.dx-dateview-item');
          var years = components.eq(2).find('.dx-dateview-item');
          var hours = components.eq(3).find('.dx-dateview-item');
          var minutes = components.eq(4).find('.dx-dateview-item');
        ...
        QUnit.test('min date should be configured correctly', function(assert) {
          ...
          var minDate = new Date(2013, 7, 20);
          this.instance.option({
            minDate: minDate,
            value: date
          });
        ...
        QUnit.test('max date should be configured correctly', function(assert) {
          var date = new Date(2013, 7, 20);
          var maxDate = new Date(2013, 7, 20);
          this.instance.option({
            maxDate: maxDate,
            value: date
          });
        ...
        QUnit.test('T211302 - items number should be correct when min and max months are the same if value is out of range', function(assert) {
          ...
          var max = new Date(2015, 3, 20);
          var daysNumber = max.getDate() - min.getDate() + 1;
          var $element = $('#customDateView').dxDateView({
            value: new Date(2015, 4, 15),
            minDate: min,
            maxDate: max
          });
          var instance = $element.dxDateView('instance');
          var $dayRoller = $element.find('.' + DATEVIEW_ROLLER_CLASS + '-day');
          var $dayRollerItems = $dayRoller.find('.' + DATEVIEW_ROLLER_ITEM_CLASS);
          var $monthRoller = $element.find('.' + DATEVIEW_ROLLER_CLASS + '-month');
          var $monthRollerItems = $monthRoller.find('.' + DATEVIEW_ROLLER_ITEM_CLASS);
        ...
        QUnit.test('time of maximum date should be 23:59:59 by default (T249446)', function(assert) {
          var $element = $('#customDateView').dxDateView({});
          var instance = $element.dxDateView('instance');
          var maxDate = instance.option('maxDate');
          var maxTime = maxDate.getHours() + ':' + maxDate.getMinutes() + ':' + maxDate.getSeconds();
        ...
        QUnit.test('value should be updated after day scroll if it is out of range', function(assert) {
          ...
          var instance = $element.dxDateView('instance');
          var dayRoller = instance._rollers.day;
        ...
        QUnit.test('click on non-selected roller item should change dateView value', function(assert) {
          var $element = $('#customDateView').dxDateView({value: new Date(2015, 4, 12)});
          var instance = $element.dxDateView('instance');
          $element.find('.' + DATEVIEW_ROLLER_CLASS + '-day').eq(0).find('.' + DATEVIEW_ROLLER_ITEM_CLASS).eq(9).trigger('dxclick');
          assert.deepEqual(instance.option('value'), new Date(2015, 4, 10), 'value is changed correctly');
          $element.find('.' + DATEVIEW_ROLLER_CLASS + '-day').eq(0).find('.' + DATEVIEW_ROLLER_ITEM_CLASS).eq(7).trigger('dxclick');
        ...
        QUnit.test('should not fail if null is passed as value', function(assert) {
          assert.expect(0);
          $('#customDateView').dxDateView({value: null});
        });
        QUnit.test('the next value after null should have zero time components when type = \'date\' (T407518)', function(assert) {
          var instance = $('#customDateView').dxDateView({value: null}).dxDateView('instance');
          instance._rollers.day.option('selectedIndex', 1);
        ...
        QUnit.test('time component should be preserved after value is changed by rollers', function(assert) {
          ...
          var instance = $('#customDateView').dxDateView({value: date}).dxDateView('instance');
          instance._rollers.day.option('selectedIndex', 0);
          var newValue = instance.option('value');
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["animation/fx","animation/translator","core/devices","events/visibility_change","generic_light.css!","jquery","localization/date","ui/date_box/ui.date_view","ui/date_box/ui.date_view_roller","../../helpers/executeAsyncMock.js","../../helpers/pointerMock.js","../DevExpress.ui.widgets/scrollableParts/scrollable.constants.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("animation/fx"), require("animation/translator"), require("core/devices"), require("events/visibility_change"), require("generic_light.css!"), require("jquery"), require("localization/date"), require("ui/date_box/ui.date_view"), require("ui/date_box/ui.date_view_roller"), require("../../helpers/executeAsyncMock.js"), require("../../helpers/pointerMock.js"), require("../DevExpress.ui.widgets/scrollableParts/scrollable.constants.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=dateView.tests.js.map