System.register(["jquery", "./widgets.json!", "renovation/components", "ui/button", "ui/check_box", "ui/pager", "ui/data_grid"], function (_export, _context) {
  "use strict";

  var $, widgetsMeta, publicWidgets, Button, PRIVATE_JQUERY_WIDGETS, CUSTOM_ROOT_WIDGET_CLASS, widgetsInBundle, isRenovation, widgets;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_widgetsJson) {
      widgetsMeta = _widgetsJson.default;
    }, function (_renovationComponents) {
      publicWidgets = _renovationComponents.default;
    }, function (_uiButton) {
      Button = _uiButton.default;
    }, function (_uiCheck_box) {}, function (_uiPager) {}, function (_uiData_grid) {}],
    execute: function () {
      /**
       * List of registered jQuery widgets which were created only to be used from old DevExtreme code
       */
      PRIVATE_JQUERY_WIDGETS = ['Widget', 'ResizableContainer', 'TimePanelTableLayout', 'GroupPanel', 'HeaderPanelLayout', 'TimelineHeaderPanelLayout', 'DateTableLayoutBase', 'AllDayPanelLayout', 'AllDayPanelTitle', 'MonthDateTableLayout', 'AppointmentLayout', 'GridPager', 'Scrollable', 'DraggableContainer', 'Droppable', 'Editor'];
      CUSTOM_ROOT_WIDGET_CLASS = {
        'dxGridPager': 'datagrid-pager',
        'dxDataGrid': 'widget'
      };
      widgetsInBundle = publicWidgets.filter(item => !item.inProgress).map(widget => widget.name);
      isRenovation = !!Button.IS_RENOVATED_WIDGET;
      widgets = isRenovation ? widgetsMeta.filter(meta => {
        return PRIVATE_JQUERY_WIDGETS.indexOf(meta.name) === -1 && widgetsInBundle.indexOf(meta.name) !== -1;
      }) : [];
      QUnit.module('Check components registration', () => {
        widgets.forEach(meta => {
          QUnit.test(`${`dx${meta.name}`} is in bundle`, function (assert) {
            const message = 'You should add your widget to the bundle.' + 'See "bundles/modules/parts/renovation"';
      ...
      QUnit.module('Mandatory component setup', {
        ...
        widgets.filter(m => m.props.allProps.indexOf('width') !== -1 && m.props.allProps.indexOf('width') !== -1).forEach(meta => {
        ...
        widgets.filter(m => m.props.template.length).forEach(meta => {
