System.register(["../../../../artifacts/js/vectormap-utils/dx.vectormaputils.js", "jquery"], function (_export, _context) {
  "use strict";

  var parse, $, CONTROLLER_URL, TEST_DATA_URL, testData;
  ...
  return {
    setters: [function (_artifactsJsVectormapUtilsDxVectormaputilsJs) {
      parse = _artifactsJsVectormapUtilsDxVectormaputilsJs.parse;
    }, function (_jquery) {
      $ = _jquery.default;
    }],
    execute: function () {
      /* global ROOT_URL */
      CONTROLLER_URL = ROOT_URL + 'TestVectorMapData/';
      ...
      QUnit.module('data loader', {
        before: async function () {
            ...
            testData.forEach(function (testDataItem) {
              QUnit.test(testDataItem.name, function (assert) {
                ...
                $.when(loadBinaryData(TEST_DATA_URL + testDataItem.name + '.shp'), loadBinaryData(TEST_DATA_URL + testDataItem.name + '.dbf')).then(function (shapeData, dataBaseFileData) {
                  ...
                  const func = parse({
                    'shp': shapeData,
                    'dbf': dataBaseFileData
                  }, function (data_, errors_) {
                    data = data_;
                    errors = errors_;
                  });
          ...
          QUnit.test('process single file', function (assert) {
            const done = assert.async();
            $.getJSON(CONTROLLER_URL + 'ExecuteConsoleApp', {
              file: 'Point.shp'
            }, function (response) {
