...
(["testing/tests/DevExpress.viz.core/legend.tests.js"], ["jquery","core/utils/common","core/utils/extend","../../helpers/vizMocks.js","viz/components/legend","viz/core/title"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core/legend.tests.js", ["jquery", "core/utils/common", "core/utils/extend", "../../helpers/vizMocks.js", "viz/components/legend", "viz/core/title"], function($__export) {
  "use strict";
  var $,
      noop,
      extend,
  ...
  return {
    ...
    execute: function() {
      ...
      environment = {
        ...
        createSimpleLegend: function() {
          ...
          return this.createLegend({
            ...
            widget: {_getTemplate: function(f) {
                this.template = {render: sinon.spy(function(arg) {
        ...
        getRenderedElements: function() {
          ...
          var items = $.map(insideLegendGroup.children, function(item) {
            return {
              group: item,
        ...
        _checkMarkup: function(assert, itemsCount) {
          ...
          var titleCorrection = this.options.title.text ? 1 : 0;
          var subtitleCorrection = titleCorrection;
          var borderCorrection = this.borderCorrection();
          assert.equal(markersGroup.children.length, itemsCount, 'Legend created the correct items count.');
          for (i = 0; i < itemsCount; i++) {
            var itemGroup = markersGroup.children[i];
            var markerGroup = itemGroup.children[0].element;
        ...
        _checkCreatingLabel: function(assert, indexItem) {
          var titleCorrection = this.options.title.text ? 1 : 0;
          var subtitleCorrection = titleCorrection;
          var label = this.renderer.text.getCall(indexItem + titleCorrection + subtitleCorrection);
          assert.equal(label.args[1], 0);
          assert.equal(label.args[2], 0);
        },
        _checkMarker: function(assert, markerAttr, index) {
          var marker = this.renderer.rect.getCall(index + this.borderCorrection()).returnValue;
          var settings = marker._stored_settings;
          var markerGroup = this.findMarkersGroup().children[index].children[0];
        ...
        _checkLabel: function(assert, labelAttr, index) {
          ...
          var settings = label._stored_settings;
      ...
      QUnit.test('Creates correct types of objects for series', function(assert) {
        ...
        var elements = this.getRenderedElements();
        ...
        for (var i = 0; i < this.data.length; i++) {
          ...
          assert.deepEqual(marker.attr.firstCall.args[0], {
            'fill': this.data[i].states.normal.fill,
            opacity: 1,
            filter: undefined
          }, 'Rect element not found for series ' + i);
      ...
      QUnit.test('Draw with Title', function(assert) {
        ...
        this.createAndDrawLegend(200, 200);
        var itemGroups = this.findMarkersGroup().children;
        for (var i = 0; i < this.data.length; i++) {
      ...
      QUnit.test('Create legend, hover fill is "none"', function(assert) {
        ...
        var legend = this.createSimpleLegend();
        legend.draw(200, 200);
        states = $.map(legend._items, function(item) {
          return item.states;
        });
      ...
      QUnit.test('Update', function(assert) {
        ...
        var legend = this.createSimpleLegend().draw(200, 200).update([{
          text: 'newText',
          id: 0,
          states: {
            hover: {
      ...
      QUnit.test('Clear group if the size is too small. Width', function(assert) {
        this.createSimpleLegend().draw(4, 200);
      ...
      QUnit.test('Vertical orientation. Transform must be rounded', function(assert) {
        ...
        this.markerBBoxes = [{
          ...
          width: 13,
          height: 13
        }, defaultMarkerBBox, defaultMarkerBBox];
        this.createSimpleLegend().draw(200, 200);
        this.checkItems(assert, [{
          id: 0,
          marker: {
      ...
      QUnit.test('Custom label format', function(assert) {
        this.options.customizeText = function() {
          return this.text + ' ' + this.itemIndex;
        };
        var legend = this.createSimpleLegend();
        var formatResult = legend._options.customizeText.call({
          text: 'test name',
          itemIndex: 1
        }, {
          text: 'test name',
      ...
      QUnit.test('Custom label title format', function(assert) {
        ...
        var legend = this.createSimpleLegend();
        var formatResult = legend._options.customizeHint.call({
          text: 'test name',
          itemIndex: 1
        }, {
          text: 'test name',
      ...
      QUnit.test('Draw legend with custom format label', function(assert) {
        this.data = [{
          text: 'First',
          id: 0,
          states: $.extend(true, getDefaultStates(), {normal: {fill: 'red'}}),
          visible: true
        }];
        this.options.customizeText = function() {
          return this.seriesName + ':' + this.seriesIndex + ':' + this.seriesColor;
        };
        this.nameField = 'seriesName';
      ...
      QUnit.test('Draw background rect.Legend with border', function(assert) {
        ...
        this.options = getLegendOptions(options);
        var legend = this.createSimpleLegend();
        legend._legendGroup.stub('getBBox') && (legend._legendGroup.getBBox = function() {
          return legendMockBBox;
        });
        legend.draw(200, 200);
      ...
      QUnit.test('Legend with incorrect margin number', function(assert) {
        ...
        var legend = this.createSimpleLegend();
        assert.deepEqual(legend._options.margin, {
          left: 10,
          right: 10,
          top: 10,
          bottom: 10
        }, 'Margin should have default margin');
      });
      ...
      QUnit.test('Legend with incorrect margin object', function(assert) {
        this.options = getLegendOptions({margin: {
            top: undefined,
            bottom: NaN,
            left: -10,
            right: 'abs'
      ...
      QUnit.test('shift legend', function(assert) {
        ...
        assert.deepEqual(legend._insideLegendGroup.attr.firstCall.args[0], {
          translateX: 17,
          translateY: 20
        });
      });
      ...
      QUnit.test('coordsIn after erase', function(assert) {
        ...
        var legend = this.createSimpleLegend().draw(200, 200).shift(10, 15).erase();
        assert.ok(!legend.coordsIn(10, 30));
        assert.ok(!legend.coordsIn(20, 30));
        assert.ok(!legend.coordsIn(30, 30));
        assert.ok(!legend.coordsIn(20, 15));
      ...
      QUnit.test('getItemByCoord', function(assert) {
        ...
        var legend = this.createSimpleLegend().draw(200, 200).shift(10, 15);
        var element1 = {
          bottom: 18,
          id: 0,
          left: -4,
      ...
      QUnit.test('Pass color & opacity to markers on create', function(assert) {
        ...
        this.data = [{
          text: 'text_0',
          id: 0,
          states: $.extend(true, {}, states, {normal: {
              fill: 'color_0',
              opacity: 0.1
            }}),
          visible: true
        ...
        assert.deepEqual(this.renderer.rect.getCall(2).returnValue.attr.getCall(0).args[0], {
          fill: 'color_2',
          opacity: 0.2,
          filter: undefined
        });
      ...
      QUnit.test('Pass color & opacity to markers on update', function(assert) {
        ...
        var legend = this.createSimpleLegend(legendData);
        legend.draw(200, 200);
        legend.update([{
          text: 'text_0',
          id: 0,
        ...
        legend.draw();
        assert.deepEqual(this.renderer.rect.lastCall.returnValue.attr.getCall(0).args[0], {
          fill: 'new_color_0',
          opacity: 0.5,
          filter: undefined
        });
      ...
      QUnit.test('move', function(assert) {
        ...
        assert.deepEqual(this.renderer.g.returnValues[1].attr.args[0][0], {
          translateX: 17,
          translateY: 20
        });
      });
      ...
      QUnit.test('Legend should not redraw if the options is not updated', function(assert) {
        ...
        legend._items.forEach(function(item) {
      ...
      QUnit.module('Life cycle', $.extend({}, environment, {beforeEach: function() {
      ...
      QUnit.module('States', $.extend({}, environment, {beforeEach: function() {
      ...
      QUnit.test('Change state - leads to redraw marker', function(assert) {
        ...
        var marker = this.renderer.rect.lastCall.returnValue;
      ...
      QUnit.test('Check groups order', function(assert) {
        ...
        var titleGroup = this.renderer.g.firstCall.returnValue;
        var markersGroup = this.findMarkersGroup();
        var rect = this.renderer.rect.firstCall.returnValue;
      ...
      QUnit.test('Colors', function(assert) {
        ...
        $.each(this.data, function(i, data) {
          assert.deepEqual(createMarker.getCall(i).returnValue.attr.getCall(0).args, [{
            fill: data.states.normal.fill,
            opacity: 1,
            filter: undefined
          }], String(i));
        });
      ...
      QUnit.test('markers centering(partial markers sizes).', function(assert) {
        ...
        var createMarker = this.createMarker;
        $.each(this.data, function(i) {
          assert.deepEqual(createMarker.getCall(i).returnValue.attr.lastCall.args, [{
            fill: 'color-' + (1 + i),
            opacity: 1,
            filter: undefined
          }]);
      ...
      titleEnvironment = $.extend({}, environment, {
        beforeEach: function() {
          var that = this;
          ...
          module.Title = function(params) {
            ...
            that.title.getLayoutOptions.returns((function() {
              if (that.options.title) {
                var margin = that.options.title.margin || {
                  left: 0,
                  right: 0
                };
        ...
        checkTitle: function(assert, titleDescription) {
          var titleGroup = this.renderer.g.firstCall.returnValue;
          var titleText = this.options.title.text ? 1 : 0;
          var subtitleText = (this.options.title.subtitle || {}).text && titleText ? 1 : 0;
          var insideLegendGroup = this.renderer.g.secondCall.returnValue;
          ...
          var marker = this.findMarkersGroup().children[0].children[0];
      ...
      QUnit.test('Simple title; legend options by default', function(assert) {
        ...
        this.createSimpleLegend().draw(200, 200);
        this.checkTitle(assert, {expectedOffset: {
            x: 0,
            y: 0
          }});
        this.checkItems(assert, [{
      ...
      QUnit.test('Title width more than markers width; legend horizontalAlignment = \'center\'', function(assert) {
        ...
        legend.shift(0, 0);
        var markersGroup = this.findMarkersGroup();
        assert.deepEqual(markersGroup.attr.lastCall.args[0], {
          translateX: 5,
          translateY: 17
        }, 'markers must moved');
      ...
      QUnit.module('Template', extend({}, environment, {beforeEach: function() {
          environment.beforeEach.call(this);
          this.options.markerTemplate = sinon.spy();
          this.data = this.createData(1);
          this.data[0].states.hover = {
            fill: 'hover-color',
            opacity: 0.2
          };
          this.data[0].states.selection = {
      ...
      QUnit.test('getTemplatesGroups', function(assert) {
        var legend = this.createSimpleLegend();
        legend.draw(200, 200);
        var groups = legend.getTemplatesGroups();
      ...
      QUnit.test('getTemplatesDef', function(assert) {
        var legend = this.createSimpleLegend();
        legend.draw(200, 200);
        var defs = legend.getTemplatesDef();
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","core/utils/extend","../../helpers/vizMocks.js","viz/components/legend","viz/core/title"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("core/utils/extend"), require("../../helpers/vizMocks.js"), require("viz/components/legend"), require("viz/core/title"));
  else
