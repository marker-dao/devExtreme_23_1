...
(["testing/tests/DevExpress.viz.core/xyAxes.tests.js"], ["jquery","../../helpers/vizMocks.js","viz/axes/tick_generator","viz/translators/translator2d","viz/translators/range","viz/axes/base_axis","../../helpers/chartMocks.js","viz/core/utils"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core/xyAxes.tests.js", ["jquery", "../../helpers/vizMocks.js", "viz/axes/tick_generator", "viz/translators/translator2d", "viz/translators/range", "viz/axes/base_axis", "../../helpers/chartMocks.js", "viz/core/utils"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      tickGeneratorModule,
  ...
  return {
    setters: [function($__m) {
      ...
      rangeModule = $__m.default;
    }, function($__m) {
      Axis = $__m.Axis;
    }, function($__m) {
      MockSeries = $__m.MockSeries;
    }, function($__m) {
      patchFontOptions = $__m.patchFontOptions;
    }],
    execute: function() {
      ...
      environment = {
        beforeEach: function() {
          ...
          this.renderSettings = {
            ...
            getTemplate: sinon.spy(function() {
              return {render: $__4.templateRender};
            })
          };
        ...
        createSimpleAxis: function(options) {
          ...
          var axis = this.createAxis(this.renderSettings, options);
          axis.validate();
      ...
      environment2DTranslator = $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          this.translator = getStub2DTranslatorWithSettings();
          this.translator.getCanvasVisibleArea.returns({
            min: 0,
            max: 1000
          });
      ...
      overlappingEnvironment = $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          var that = this;
      ...
      QUnit.test('Linear axis updates translator on option changed', function(assert) {
        ...
        assert.strictEqual(translator.update.lastCall.args[2].shiftZeroValue, false);
      });
      ...
      QUnit.module('Ticks skipping. Normal axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          var that = this;
      ...
      QUnit.module('Semidiscrete axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          var that = this;
      ...
      QUnit.test('translates coordinates with tickInterval info', function(assert) {
        ...
        assert.equal(this.translator.translate.callCount, 8);
      ...
      QUnit.test('axis without shifting', function(assert) {
        ...
        var labelsGroup = this.renderer.g.getCall(9);
      ...
      QUnit.test('measuring label, label creation', function(assert) {
        ...
        var axis = this.createSimpleAxis({label: {
            visible: true,
        ...
        assert.deepEqual(text.returnValues[0].attr.args[0][0], {
          opacity: undefined,
          align: 'center',
          'class': undefined
        }, 'text options');
      ...
      QUnit.test('Datetime, no custom format - use auto format based on estimated tickInterval', function(assert) {
        this.generatedTicks = [new Date(2010, 7, 1), new Date(2010, 8, 1), new Date(2010, 9, 1)];
      ...
      QUnit.test('Datetime, custom format - use provided format', function(assert) {
        this.generatedTicks = [new Date(2010, 7, 1), new Date(2010, 8, 10), new Date(2010, 9, 25)];
        this.generatedTickInterval = 'month';
        var axis = this.createSimpleAxis({
          valueType: 'datetime',
          label: {
      ...
      QUnit.test('Not valid mode, default is hide', function(assert) {
        ...
        this.drawAxisWithOptions({
          min: 1,
          max: 10,
          label: {overlappingBehavior: 'mode'}
        });
        assert.equal(this.renderer.text.callCount, 5);
        assert.deepEqual(this.arrayRemovedElements, ['3', '5', '9']);
      });
      ...
      QUnit.test('frequent ticks', function(assert) {
        ...
        this.renderer.text = spyRendererText.call(this, markersBBoxes);
        this.drawAxisWithOptions({label: {overlappingBehavior: 'hide'}});
      ...
      QUnit.test('custom alignment for labels', function(assert) {
        this.translator.translate.withArgs(1).returns(0);
        this.translator.translate.withArgs(3).returns(15);
        this.translator.translate.withArgs(5).returns(20);
        this.translator.translate.withArgs(7).returns(45);
      ...
      QUnit.test('Do not update removed label position on update size', function(assert) {
        ...
        var removedLabel = this.renderer.text.getCall(1).returnValue;
      ...
      QUnit.test('Labels overlap and rotate -45 degrees', function(assert) {
        ...
        this.drawAxisWithOptions({
          min: 1,
          max: 10,
          label: {overlappingBehavior: 'auto'}
        });
        var texts = this.renderer.text;
        assert.deepEqual(this.arrayRemovedElements, [], 'labels shouldn\'t decimated');
        for (i = 0; i < texts.callCount; i++) {
      ...
      QUnit.test('Estimate draws title text and remove it', function(assert) {
        ...
        assert.deepEqual(textElement.css.lastCall.args[0], {'font-weight': 'bold'}, 'lebel style');
      ...
      QUnit.test('Check title overflow on draw', function(assert) {
        ...
        var textElement = this.renderer.text.firstCall.returnValue;
        assert.deepEqual(textElement.setMaxSize.callCount, 1);
        assert.deepEqual(textElement.setMaxSize.firstCall.args, [10, undefined, {
          textOverflow: 'none',
          wordWrap: 'breakWord'
        }]);
      });
      ...
      QUnit.test('Estimate top/bottom margin. Axis with title', function(assert) {
        ...
        this.bBoxes.push({height: 14});
        this.bBoxes.push({height: 44});
        var margins = axis.estimateMargins(this.canvas);
      ...
      QUnit.test('Estimate draws constant lines with outside labels', function(assert) {
        ...
        assert.deepEqual(textElement.attr.lastCall.args[0], {align: 'center'}, 'lebel settings');
        assert.deepEqual(textElement.css.lastCall.args[0], {'font-weight': 'bold'}, 'lebel style');
      ...
      QUnit.test('no custom format - use auto format based on estimated tickInterval', function(assert) {
        this.generatedTicks = [new Date(2009, 11, 1), new Date(2010, 0, 1), new Date(2010, 1, 1)];
        this.generatedTickInterval = 'month';
        var axis = this.createSimpleAxis({
          isHorizontal: true,
          type: 'continuous',
      ...
      QUnit.test('Horizontal axis. Shift outside constant line groups vertically', function(assert) {
        ...
        var groupUnderSeries = {
          topGroup: this.renderer.g.getCall(16).returnValue,
          bottomGroup: this.renderer.g.getCall(17).returnValue
        };
        assert.deepEqual(groupAboveSeries.topGroup.attr.lastCall.args, [{
          translateY: -(64 + 5),
          translateX: 0
        }]);
        assert.deepEqual(groupUnderSeries.topGroup.attr.lastCall.args, [{
          translateY: -(64 + 5),
          translateX: 0
        }]);
        assert.deepEqual(groupAboveSeries.bottomGroup.attr.lastCall.args, [{
          translateY: 45 + 5,
          translateX: 0
        }]);
        assert.deepEqual(groupUnderSeries.bottomGroup.attr.lastCall.args, [{
          translateY: 45 + 5,
          translateX: 0
        }]);
      });
      QUnit.test('Vertical axis. Shift outside constant line groups horizontally', function(assert) {
        ...
        assert.deepEqual(groupAboveSeries.leftGroup.attr.lastCall.args, [{
          translateX: -(50 + 5),
          translateY: 0
        }]);
        assert.deepEqual(groupUnderSeries.leftGroup.attr.lastCall.args, [{
          translateX: -(50 + 5),
          translateY: 0
        }]);
        assert.deepEqual(groupAboveSeries.rightGroup.attr.lastCall.args, [{
          translateX: 76 + 5,
          translateY: 0
        }]);
        assert.deepEqual(groupUnderSeries.rightGroup.attr.lastCall.args, [{
          translateX: 76 + 5,
          translateY: 0
        }]);
      });
      ...
      QUnit.module('Scale breaks', $.extend({}, environment2DTranslator, {
        ...
        updateOptions: function(opt) {
          this.axis.updateOptions($.extend(true, this.options, {breakStyle: {width: 0}}, opt));
          this.axis.setBusinessRange({
            min: opt.min,
            max: opt.max
          });
      ...
      QUnit.test('Scale breaks with the viewport passed to the translator', function(assert) {
        ...
        var userBreaks = this.translator.updateBusinessRange.lastCall.args[0].userBreaks;
        assert.deepEqual(breaks, [{
          from: 250,
          to: 300,
          cumulativeWidth: 10
      ...
      QUnit.test('Remove groups on disposing', function(assert) {
        ...
        var axis = this.createSimpleAxis(this.renderSettings, $.extend(true, this.options, {
          breaks: [{
            startValue: 50,
            endValue: 100
          }, {
        ...
        this.translator.getBusinessRange.returns(new rangeModule.Range({breaks: [{
            startValue: 50,
            endValue: 100
          }, {
            startValue: 70,
      ...
      QUnit.test('T889259. Scale breaks should be into account in the translator after update', function(assert) {
        ...
        this.axis.createTicks(this.canvas);
        var updateBusinessRange = this.translator.updateBusinessRange;
        for (var i = 1; i < updateBusinessRange.callCount; i++) {
          assert.deepEqual(updateBusinessRange.args[i][0].breaks, [{
            from: 300,
            to: 400,
            cumulativeWidth: 0
          }]);
      ...
      QUnit.module('Datetime scale breaks. Weekends and holidays', $.extend({}, environment2DTranslator, {
        beforeEach: function() {
          ...
          that.axis = that.createSimpleAxis({dataType: 'datetime'});
        },
        updateOptions: function(opt) {
          ...
          this.axis.setBusinessRange({
            min: opt.min || new Date(2017, 8, 4, 8),
            max: opt.max || new Date(2017, 8, 11)
          });
        },
        afterEach: environment2DTranslator.afterEach
      }));
      ...
      QUnit.test('Generate weekend breaks', function(assert) {
        this.updateOptions({breakStyle: {width: 10}});
        this.axis.createTicks(this.canvas);
        var breaks = this.tickGeneratorSpy.lastCall.args[7];
        assert.deepEqual(breaks, [{
      ...
      QUnit.test('Do not generate weekend breaks if workdaysOnly is set to false', function(assert) {
        this.updateOptions({workdaysOnly: false});
        this.axis.createTicks(this.canvas);
        var breaks = this.tickGeneratorSpy.lastCall.args[7];
        assert.deepEqual(breaks, []);
      });
      ...
      QUnit.test('Generate two breaks when two days off on week', function(assert) {
        this.updateOptions({workWeek: [1, 2, 4, 5]});
        this.axis.createTicks(this.canvas);
        var breaks = this.tickGeneratorSpy.lastCall.args[7];
        assert.deepEqual(breaks, [{
      ...
      QUnit.test('Concatenate generated breaks with user breaks', function(assert) {
        this.updateOptions({
          breaks: [{
            startValue: new Date(2017, 8, 12),
            endValue: new Date(2017, 8, 13)
          }],
          min: new Date(2017, 8, 4),
          max: new Date(2017, 8, 13)
        });
      ...
      QUnit.test('Several series with not sorted values', function(assert) {
        this.series = [this.stubSeries([[3, 10, 100, 40], []]), this.stubSeries([[80, 120, 40], []])];
      ...
      QUnit.test('Very big difference beetwen the values', function(assert) {
        this.series = [this.stubSeries([[5500, 5100, 300, 5], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          min: 2,
          max: 6000
      ...
      QUnit.test('Small difference beetween the values, breaks are not generated', function(assert) {
        this.series = [this.stubSeries([[2, 3, 4, 5, 6, 7, 8, 9, 10], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          min: 2,
          max: 10
      ...
      QUnit.test('Argument axis. Without breaks', function(assert) {
        this.series = [this.stubSeries([[5500, 5100, 300, 5], []])];
        var axis = this.createSimpleAxis({isArgumentAxis: true});
        axis.updateOptions({
          autoBreaksEnabled: true,
          maxAutoBreakCount: 2,
          isHorizontal: true,
      ...
      QUnit.test('Two values and range is equal to this values', function(assert) {
        this.series = [this.stubSeries([[3, 100], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          maxAutoBreakCount: 1,
          min: 3,
      ...
      QUnit.test('Logarithmic axis', function(assert) {
        this.series = [this.stubSeries([[0.1, 1, 10, 100, 1000, 10000000], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          type: 'logarithmic',
          logarithmBase: 10,
      ...
      QUnit.test('Breaks length should be less than visible range. T569737', function(assert) {
        this.series = [this.stubSeries([[0.001, 0.267, 19, 23, 145, 1009], []])];
        this.axis.setGroupSeries(this.series);
        this.options.maxAutoBreakCount = undefined;
        this.updateOptions({
          type: 'logarithmic',
      ...
      QUnit.test('Auto scale breaks with large range', function(assert) {
        this.series = [this.stubSeries([[1970, 1972, 1974, 5330000000000], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          type: 'numeric',
          min: 0,
      ...
      QUnit.test('Concatenate auto breaks with user breaks', function(assert) {
        this.series = [this.stubSeries([[3, 10, 35, 43, 45, 100, 40], []])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          breaks: [{
            startValue: 36,
      ...
      QUnit.test('Recalculate scale breaks on zoom', function(assert) {
        this.axis.setGroupSeries([this.stubSeries([[3, 10, 40], []]), this.stubSeries([[80, 120, 40], []])]);
        this.updateOptions({
          min: 2,
          max: 120
        });
        this.axis.createTicks(this.canvas);
        this.axis.setGroupSeries([this.stubSeries([[3, 10], []]), this.stubSeries([[80, 120, 40], []])]);
        this.axis.visualRange(50, 100);
        this.axis.createTicks(this.canvas);
        assert.deepEqual(this.tickGeneratorSpy.lastCall.args[7], [{
          from: 80,
      ...
      QUnit.test('Generate the breaks take into account the edge points that out of the range', function(assert) {
        this.series = [this.stubSeries([[14, 50, 100], [20, 43]])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          min: 0,
          max: 100
      ...
      QUnit.test('Generate the breaks take into account the edge points that out of the range, min range === min edge value', function(assert) {
        this.series = [this.stubSeries([[30, 100], [90, 2]])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          min: 2,
          max: 100
      ...
      QUnit.test('Generate the breaks take into account the edge points that out of the range, edge point equal to point in the range', function(assert) {
        this.series = [this.stubSeries([[2, 30, 100], [30, 90]])];
        this.axis.setGroupSeries(this.series);
        this.updateOptions({
          min: 2,
          max: 100
      ...
      QUnit.module('XY axes margin calculation', {
        ...
        testMargins: function(assert, data) {
          ...
          this.generatedBreaks = (data.options.breaks || []).map(function(b) {
            return {
              from: b.startValue,
              to: b.endValue,
      ...
      QUnit.test('T746896. Take into account scale breaks on tick calculation', function(assert) {
        var axis = this.createAxis(this.renderSettings, {
          valueMarginsEnabled: true,
          breakStyle: {width: 0},
          minValueMargin: 0.1,
          maxValueMargin: 0.1,
      ...
      QUnit.test('Calculate common range from series on adjust', function(assert) {
        ...
        this.translator.updateBusinessRange.reset();
        this.axis.createTicks(this.canvas);
        var $__5 = this.translator.updateBusinessRange.lastCall.args[0],
            min = $__5.min,
            max = $__5.max,
      ...
      QUnit.test('Adjusting with constant lines', function(assert) {
        this.updateOptions({constantLines: [{
            value: 90,
            extendAxis: true
          }, {value: 210}]});
        this.series[0].getViewport.returns({
      ...
      QUnit.module('Custom positioning', {
        ...
        drawOrthogonalAxes: function(horizontalAxisOptions, verticalAxisOptions) {
          var horizontalAxis = this.createSimpleAxis($.extend(true, {
            isArgumentAxis: false,
            argumentType: 'numeric'
          }, horizontalAxisOptions));
          var verticalAxis = this.createSimpleAxis($.extend(true, {
            isArgumentAxis: false,
            isHorizontal: false,
            valueType: 'numeric'
          }, verticalAxisOptions));
          horizontalAxis.getOrthogonalAxis = function() {
            return verticalAxis;
          };
          verticalAxis.getOrthogonalAxis = function() {
      ...
      QUnit.test('Set predefined axis position by \'position\' option', function(assert) {
        var $__5 = this.drawOrthogonalAxes({position: 'top'}, {position: 'right'}),
            horizontalAxis = $__5.horizontalAxis,
            verticalAxis = $__5.verticalAxis;
      ...
      QUnit.test('Set custom position', function(assert) {
        var $__5 = this.drawOrthogonalAxes({customPosition: 50}, {customPosition: 75}),
            horizontalAxis = $__5.horizontalAxis,
            verticalAxis = $__5.verticalAxis;
        ...
        assert.deepEqual(horizontalAxis._majorTicks[0].label.attr.getCall(5).args[0], {
          x: 109,
          y: 305
        });
        assert.deepEqual(verticalAxis._majorTicks[0].label.attr.getCall(1).args[0], {
          x: 688,
          y: 541
        });
      });
      QUnit.test('Shift axis by offset option', function(assert) {
        var $__5 = this.drawOrthogonalAxes({
          customPosition: 50,
          offset: -150
        }, {offset: 200}),
            horizontalAxis = $__5.horizontalAxis,
      ...
      QUnit.test('Set predefined axis position by \'customPosition\' option', function(assert) {
        var $__5 = this.drawOrthogonalAxes({customPosition: 0}, {customPosition: 200}),
            horizontalAxis = $__5.horizontalAxis,
            verticalAxis = $__5.verticalAxis;
      ...
      QUnit.test('Validate \'customPosition\' option', function(assert) {
        var horizontalAxis = this.drawOrthogonalAxes({customPosition: 'abcd'}, {}).horizontalAxis;
        assert.equal(horizontalAxis.getAxisPosition(), 600);
        assert.ok(horizontalAxis.customPositionIsAvailable());
        assert.notOk(horizontalAxis.hasNonBoundaryPosition());
        assert.ok(horizontalAxis.customPositionIsBoundary());
      ...
      QUnit.test('Check boundary side margin is equal 0 when the \'customPosition\' option exists and position of axis is not a boundary', function(assert) {
        var verticalAxis = this.drawOrthogonalAxes({}, {customPosition: 200}).verticalAxis;
        verticalAxis._customBoundaryPosition = 200;
        var margins = verticalAxis.getMargins();
      ...
      QUnit.test('Check boundary side margin is cut when the \'offset\' option exists and position of axis is a boundary (position left, offset > 0)', function(assert) {
        var verticalAxis = this.drawOrthogonalAxes({}, {offset: 10}).verticalAxis;
        verticalAxis._customBoundaryPosition = 'left';
        var margins = verticalAxis.getMargins();
      ...
      QUnit.test('Axis template was removed before rendered', function(assert) {
        ...
        var count = this.templateRender.callCount;
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","viz/axes/tick_generator","viz/translators/translator2d","viz/translators/range","viz/axes/base_axis","../../helpers/chartMocks.js","viz/core/utils"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("viz/axes/tick_generator"), require("viz/translators/translator2d"), require("viz/translators/range"), require("viz/axes/base_axis"), require("../../helpers/chartMocks.js"), require("viz/core/utils"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=xyAxes.tests.js.map