...
(["testing/tests/DevExpress.viz.core/polarAxes.tests.js"], ["jquery","../../helpers/vizMocks.js","viz/translators/translator2d","viz/axes/tick_generator","viz/translators/range","viz/axes/base_axis","core/utils/extend"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core/polarAxes.tests.js", ["jquery", "../../helpers/vizMocks.js", "viz/translators/translator2d", "viz/axes/tick_generator", "viz/translators/range", "viz/axes/base_axis", "core/utils/extend"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      translator2DModule,
  ...
  return {
    ...
    execute: function() {
      ...
      QUnit.module('Ticks skipping. Polar axes', $.extend({}, environment, {beforeEach: function() {
      ...
      QUnit.test('Linear axis. axisDivisionMode is betweenLabels, valueMarginsEnabled true', function(assert) {
        ...
        assert.equal(this.translator.translate.callCount, 10);
      ...
      QUnit.module('Circular axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
      ...
      QUnit.test('draw', function(assert) {
        ...
        assert.deepEqual(this.renderer.circle.getCall(0).returnValue.attr.getCall(0).args[0], {
          cx: 20,
          cy: 50,
          r: 20
        });
        assert.deepEqual(this.renderer.circle.getCall(0).returnValue.attr.getCall(1).args[0], {
          'stroke-width': 1,
          stroke: 'red',
          'stroke-opacity': 1
        });
      ...
      QUnit.test('draw ticks. Shift = 10', function(assert) {
        ...
        for (var i = 0; i < this.renderer.path.callCount; i++) {
          assert.deepEqual(this.renderer.path.getCall(i).returnValue.attr.getCall(1).args[0], {
            points: [40, 50, 60, 50],
            opacity: 1
          });
        }
      ...
      QUnit.test('draw ticks. Orientation = center', function(assert) {
        ...
        for (var i = 0; i < this.renderer.path.callCount; i++) {
          ...
          assert.deepEqual(this.renderer.path.getCall(i).returnValue.attr.getCall(0).args[0], {
            'stroke-width': 1,
            stroke: 'red',
            'stroke-opacity': 1,
            opacity: 1
      ...
      QUnit.test('draw labels', function(assert) {
        ...
        for (var i = 0; i < this.renderer.text.callCount; i++) {
          ...
          assert.deepEqual(this.renderer.text.getCall(i).returnValue.attr.getCall(0).args[0], {
            opacity: 1,
            align: 'center',
            'class': undefined
          });
          ...
          assert.deepEqual(this.renderer.text.getCall(i).returnValue.css.firstCall.args[0], {
            'font-size': 12,
            fill: 'black'
          });
      ...
      QUnit.test('adjust labels on updateSize', function(assert) {
        ...
        var axis = this.createDrawnAxis();
        var texts = this.renderer.text.getCalls().map(function(call) {
          var text = call.returnValue;
          text.attr.reset();
          return text;
        });
      ...
      QUnit.test('Update grid on second draw', function(assert) {
        ...
        var grid = this.renderer.path.lastCall.returnValue;
        grid.attr.reset();
        grid.rotate.reset();
        axis.draw({
          left: 10,
      ...
      QUnit.test('Do not rotated grid for Linear polar axis', function(assert) {
        ...
        var grid = this.renderer.circle.lastCall.returnValue;
        axis.draw({
          left: 10,
          right: 80,
          top: 0,
      ...
      QUnit.test('adjusted strip labels', function(assert) {
        ...
        assert.deepEqual(this.renderer.text.getCall(0).returnValue.attr.lastCall.args, [{translateY: 46}]);
      });
      QUnit.test('create constant lines', function(assert) {
        this.createDrawnAxis({constantLines: [{
            value: 10,
            color: 'green',
            label: {}
          }]});
      ...
      QUnit.test('Update constant line', function(assert) {
        this.options.startAngle = 50;
        var updateCanvas = extend({}, this.canvas);
      ...
      QUnit.test('measure labels with indents', function(assert) {
        ...
        assert.deepEqual(axis.measureLabels(this.canvas, true), {
          width: 34,
          height: 24,
          x: 1,
          y: 2
      ...
      QUnit.test('draw spider web axis', function(assert) {
        ...
        assert.deepEqual(this.renderer.path.getCall(0).returnValue.attr.getCall(0).args[0], {points: [{
            x: 37,
            y: 61
          }, {
            x: 37,
        ...
        assert.deepEqual(this.renderer.path.getCall(0).returnValue.attr.getCall(1).args[0], {
          stroke: 'red',
          'stroke-opacity': 1,
          'stroke-width': 1
        });
      ...
      QUnit.test('shift', function(assert) {
        ...
        var argsForMainGroup = this.renderer.g.getCall(6).returnValue.attr.lastCall.args[0];
        var argsForLabelsGroup = this.renderer.g.getCall(9).returnValue.attr.lastCall.args[0];
      ...
      QUnit.module('Linear Axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
      ...
      QUnit.test('draw', function(assert) {
        this.createDrawnAxis({visible: true});
        var returnedPath = this.renderer.path.getCall(0);
        ...
        assert.deepEqual(returnedPath.returnValue.attr.getCall(1).args[0], {
          'stroke-width': 1,
          stroke: 'red',
          'stroke-opacity': 1
        });
      ...
      QUnit.test('draw spider grid', function(assert) {
        ...
        assert.equal(this.renderer.path.callCount, 3);
        for (var i = 0; i < this.renderer.path.callCount; i++) {
          assert.deepEqual(this.renderer.path.getCall(i).args[0], [{
            x: 20,
            y: 50
          }, {
            x: 20,
      ...
      QUnit.test('create spider strips', function(assert) {
        this.renderSettings.drawingType = 'linearSpider';
        var axis = this.createSimpleAxis({strips: [{
            startValue: 10,
            endValue: 20,
            color: 'red'
          }]});
        ...
        axis.draw(this.canvas);
        assert.ok(this.renderer.path.called);
        assert.deepEqual(this.renderer.path.getCall(0).args, [[[{
          x: 20,
          y: 40
        }, {
          x: 20,
      ...
      QUnit.test('create spider constant line', function(assert) {
        this.renderSettings.drawingType = 'linearSpider';
        var axis = this.createSimpleAxis({constantLines: [{
            value: 10,
            color: 'green',
            label: {}
          }]});
      ...
      QUnit.module('Label overlapping, circular axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
      ...
      QUnit.test('frequent tisks', function(assert) {
        ...
        var text = this.renderer.text = spyRendererText.call(this, markersBBoxes);
        this.createDrawnAxis();
      ...
      QUnit.test('Check bounded ticks overlapping on updateSize - remove overlapped lavels if hide mode', function(assert) {
        ...
        var axis = this.createDrawnAxis();
        var text = this.renderer.text;
        text.getCalls().forEach(function(c) {
          return c.returnValue.stub('remove').reset();
        });
        bBoxes = [{
          x: 0,
          y: 10,
      ...
      QUnit.module('Label overlapping, linear axis', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          var that = this;
      ...
      QUnit.module('Circular polar axis. Set business range', {
        ...
        updateOptions: function(options) {
          options = $.extend(true, {label: {}}, options);
      ...
      QUnit.test('Do not apply margings', function(assert) {
        ...
        axis.draw(this.canvas);
        var $__4 = this.axis.getTranslator().getBusinessRange(),
            min = $__4.min,
            max = $__4.max,
            minVisible = $__4.minVisible,
            maxVisible = $__4.maxVisible;
        assert.equal(min, 0);
        assert.equal(max, 100);
        assert.equal(minVisible, 0);
        assert.equal(maxVisible, 100);
        var tickGeneratorOptions = this.tickGenerator.lastCall.returnValue.lastCall.args[0];
      ...
      QUnit.test('Extend business range to ticks', function(assert) {
        ...
        var axis = this.createAxis({valueMarginsEnabled: false});
        axis.setBusinessRange({
          min: 0,
          max: 100,
          interval: 10
      ...
      QUnit.test('Pass interval to range', function(assert) {
        ...
        axis.draw(this.canvas);
        var interval = this.axis.getTranslator().getBusinessRange().interval;
      ...
      QUnit.test('Apply margings to canvas', function(assert) {
        ...
        axis.draw(this.canvas);
        var $__4 = this.axis.getTranslator()._canvasOptions,
            startPadding = $__4.startPadding,
            startPoint = $__4.startPoint,
            endPadding = $__4.endPadding,
            endPoint = $__4.endPoint;
      ...
      QUnit.module('Circular axis. UpdateSize', $.extend({}, environment, {
        ...
        act: function(bBox) {
          ...
          axis.updateSize(this.canvas);
          var $__4 = this.renderer.text.lastCall.returnValue.attr.lastCall.args[0],
              x = $__4.x,
              y = $__4.y;
          return [x, y];
        }
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","viz/translators/translator2d","viz/axes/tick_generator","viz/translators/range","viz/axes/base_axis","core/utils/extend"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("viz/translators/translator2d"), require("viz/axes/tick_generator"), require("viz/translators/range"), require("viz/axes/base_axis"), require("core/utils/extend"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=polarAxes.tests.js.map