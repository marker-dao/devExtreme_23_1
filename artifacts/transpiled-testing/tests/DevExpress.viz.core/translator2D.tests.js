...
(["testing/tests/DevExpress.viz.core/translator2D.tests.js"], ["jquery","viz/translators/translator2d","core/utils/math"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core/translator2D.tests.js", ["jquery", "viz/translators/translator2d", "core/utils/math"], function($__export) {
  "use strict";
  var $,
      translator2DModule,
      adjust,
  ...
  function createTranslatorWithScaleBreaks(options) {
    ...
    var breaks = prepareScaleBreaks(options.breaks || [{
      from: 150,
      to: 200
    }, {
      from: 350,
    ...
    execute: function() {
      ...
      environment = {
        ...
        createTranslator: function(range, canvas, options) {
          return this._createTranslator($.extend({
            axisType: 'continuous',
            dataType: 'numeric'
          }, range), canvas || {
            width: 2000,
      ...
      QUnit.test('Create vertical translator', function(assert) {
        ...
        var translator = this.createTranslator(range, canvas, {isHorizontal: false});
        assert.ok(translator);
        assert.deepEqual(translator._canvas, {
          width: 610,
          height: 400,
          left: 70,
          top: 10,
        ...
        assert.deepEqual(getObjectData(translator._businessRange), {
          min: 0,
          minVisible: 10,
          max: 100,
          maxVisible: 90,
      ...
      QUnit.test('Create horizontal translator', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(range, canvas, optionsHorizontal);
        assert.ok(translator);
        assert.deepEqual(translator._canvas, {
          width: 610,
          height: 400,
      ...
      QUnit.test('Create horizontal translator with paddings', function(assert) {
        ...
        var translator = this.createTranslator(range, canvas, {isHorizontal: true});
      ...
      QUnit.test('Create numeric translator', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(range, canvas);
        assert.ok(translator);
        assert.deepEqual(translator._canvas, {
          width: 610,
          height: 400,
      ...
      QUnit.test('Create horizontal numeric translator when business range delta = 0, Min = max = minVisible = maxVisible = 0 (T696532)', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(range, canvas, {shiftZeroValue: true});
        assert.ok(translator);
      ...
      QUnit.test('Create datetime translator', function(assert) {
        ...
        assert.deepEqual(getObjectData(translator._businessRange), {
          min: new Date(2012, 9, 1),
          max: new Date(2012, 9, 2),
          minVisible: new Date(2012, 9, 1),
          maxVisible: new Date(2012, 9, 2),
          interval: new Date(2012, 9, 2) - new Date(2012, 9, 1),
          axisType: 'continuous',
          dataType: 'datetime'
        });
      ...
      QUnit.test('Create discrete translator (Stick = false, invert = false)', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(range, canvas, $.extend({stick: false}, optionsHorizontal));
        assert.ok(translator);
        assert.deepEqual(translator._canvas, {
          width: 610,
          height: 400,
      ...
      QUnit.test('Create discrete translator (Stick = true, invert = true)', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(range, canvas, $.extend({stick: true}, optionsHorizontal));
      ...
      QUnit.test('Create discrete translator (Stick = true, addSpiderCategory = true)', function(assert) {
        var canvas = $.extend({}, canvasTemplate);
        var translator = this.createTranslator(discreteRange, canvas, $.extend({stick: false}, optionsHorizontal));
        assert.equal(translator._canvasOptions.interval, 127.5);
        assert.deepEqual(translator._categoriesToPoints, {
          'First': 0,
          'Second': 1,
      ...
      QUnit.test('Can be reinited (recalculate canvas options)', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = new translator2DModule.Translator2D(range, canvas, optionsHorizontal);
        canvas.width = 600;
        canvas.left = 50;
        canvas.right = 150;
        translator.reinit();
      ...
      QUnit.test('Get business range', function(assert) {
        var canvas = $.extend({}, canvasTemplate);
        var translator = new translator2DModule.Translator2D($.extend({
          minVisible: 10,
          maxVisible: 90,
          invert: true
        }, numericRange), canvas, optionsHorizontal);
        var range = translator.getBusinessRange();
        assert.ok(range);
        assert.deepEqual(getObjectData(range), {
          min: 0,
          minVisible: 10,
          max: 100,
          maxVisible: 90,
      ...
      QUnit.test('Translate. not round values', function(assert) {
        var translator = this._createTranslator($.extend({
          axisType: 'continuous',
          dataType: 'numeric',
          interval: 1,
          invert: false
          ...
          isHorizontal: true,
          breaksSize: 0,
          conversionValue: function(value) {
            return value;
          }
        });
      ...
      QUnit.test('Translate. Scale breaks. Inverted axis', function(assert) {
        var translator = createTranslatorWithScaleBreaks.call(this, {invert: true});
      ...
      QUnit.test('getRangeByMinZoomValue', function(assert) {
        ...
        assert.deepEqual(translator.getRangeByMinZoomValue(100, {
          minVisible: 200,
          maxVisible: 250
        }), [200, 300]);
        assert.deepEqual(translator.getRangeByMinZoomValue(100, {
      ...
      QUnit.module('Datetime translator', {beforeEach: function() {
          this.createTranslator = function(range, _, options) {
            return new translator2DModule.Translator2D($.extend({
              axisType: 'continuous',
              dataType: 'datetime',
              interval: 1,
              invert: false
      ...
      QUnit.test('from. With scale breaks. Values not in the breaks and translated. Mix scale breaks and gaps(weekends)', function(assert) {
        var translator = createTranslatorWithScaleBreaks.call(this, {
          ...
          max: new Date(2012, 8, 2),
          breaks: [{
            from: new Date(2012, 8, 1, 10),
            to: new Date(2012, 8, 1, 11)
          }, {
            from: new Date(2012, 8, 1, 20),
            to: new Date(2012, 8, 1, 21),
            gapSize: 'some size'
          }, {
            from: new Date(2012, 8, 1, 22),
            to: new Date(2012, 8, 1, 23)
          }]
        });
      ...
      QUnit.test('from. decimal (T932222)', function(assert) {
        var translator = this.createTranslator({
          min: new Date(2020, 8, 1, 0, 0, 0, 10),
          max: new Date(2020, 8, 1, 0, 0, 0, 18)
        });
      ...
      QUnit.module('Logarithmic translator', {beforeEach: function() {
          this.createTranslator = function(range, _, options) {
            return new translator2DModule.Translator2D($.extend({
              axisType: 'logarithmic',
              dataType: 'numeric',
      ...
      QUnit.module('Logarithmic translator. Allow Negatives', {beforeEach: function() {
          this.createTranslator = function(range, _, options) {
            return new translator2DModule.Translator2D($.extend({
              axisType: 'logarithmic',
              dataType: 'numeric',
      ...
      QUnit.module('Discrete translator', {beforeEach: function() {
      ...
      QUnit.test('Translate. AddSpiderCategory = true. Stick = true.', function(assert) {
        var translator = this.createTranslator(undefined, undefined, {
          addSpiderCategory: true,
          stick: true
        });
        assert.equal(translator.translate('a0'), null, 'BP not in categories (Q522516)');
      ...
      QUnit.test('from. border values.', function(assert) {
        var start = 27;
        var delta = 141;
        var translator = this.createTranslator({categories: ['1q', '2q', '3q', '4q', '5q', '6q', '7q']}, {
          bottom: 22,
          height: 200,
          left: 27,
          right: 27,
      ...
      QUnit.test('discrete zooming. translate. DateTime categories', function(assert) {
        var translator = this.createTranslator({
          categories: [new Date(1356480000000), new Date(1356483600000), new Date(1356519600000), new Date(1356523200000), new Date(1356526800000)],
      ...
      QUnit.test('getRangeByMinZoomValue', function(assert) {
        var translator = this.createTranslator();
        assert.deepEqual(translator.getRangeByMinZoomValue(3, {
          minVisible: 'a3',
          maxVisible: 'a4'
        }), ['a3', 'a5']);
        assert.deepEqual(translator.getRangeByMinZoomValue(3, {
      ...
      QUnit.module('Interval translator', {beforeEach: function() {
      ...
      QUnit.test('Translate. Datetime, interval second', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 0, 19, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 0, 17, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 0, 21, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval minute', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 19, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 17, 50, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 5, 1, 0, 21, 50, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval hour', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 5, 1, 19, 10, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 5, 1, 17, 10, 50, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 5, 1, 21, 10, 50, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval day', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 5, 19, 15, 10, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 5, 17, 15, 10, 50, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 5, 21, 15, 10, 50, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval week', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 5, 25, 15, 10, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 5, 9, 15, 10, 50, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 6, 7, 15, 10, 50, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval month', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 6, 11, 15, 10, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 4, 11, 15, 10, 50, 236)), 601, 'BP on the min');
        assert.equal(translator.translate(new Date(2015, 8, 11, 15, 10, 50, 236)), 1402, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval quarter', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2015, 8, 11, 15, 10, 50, 236)), 998, 'BP inside range');
        assert.equal(translator.translate(new Date(2015, 1, 11, 15, 10, 50, 236)), 598, 'BP on the min');
        assert.equal(translator.translate(new Date(2016, 2, 11, 15, 10, 50, 236)), 1400, 'BP on the max');
      });
      QUnit.test('Translate. Datetime, interval year', function(assert) {
        ...
        assert.equal(translator.translate(new Date(2012, 5, 11, 15, 10, 50, 236)), 1000, 'BP inside range');
        assert.equal(translator.translate(new Date(2010, 5, 11, 15, 10, 50, 236)), 600, 'BP on the min');
        assert.equal(translator.translate(new Date(2014, 5, 11, 15, 10, 50, 236)), 1400, 'BP on the max');
      });
      ...
      QUnit.module('Translate special cases', {beforeEach: function() {
      ...
      QUnit.test('Default value', function(assert) {
        ...
        checkTranslator(500, {
          ...
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = false. Position = horizontal. Range is positive');
        checkTranslator(1500, {
          min: -200,
          max: -100,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = false. Position = horizontal. Range is negative');
        checkTranslator(1000, {
          min: -200,
          max: 200,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = false. Position = horizontal. Range includes zero');
        checkTranslator(1500, {
          min: 100,
          max: 200,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = true. Position = horizontal. Range is positive');
        checkTranslator(500, {
          min: -200,
          max: -100,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = true. Position = horizontal. Range is negative');
        checkTranslator(1000, {
          min: -200,
          max: 200,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = true. Position = horizontal. Range includes zero');
        checkTranslator(1500, {
          min: 100,
          max: 200,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = false. Position = vertical. Range is positive');
        checkTranslator(500, {
          min: -200,
          max: -100,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = false. Position = vertical. Range is negative');
        checkTranslator(1000, {
          min: -200,
          max: 200,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = false. Position = vertical. Range includes zero');
        checkTranslator(500, {
          min: 100,
          max: 200,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = true. Position = vertical. Range is positive');
        checkTranslator(1500, {
          min: -200,
          max: -100,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = true. Position = vertical. Range is negative');
        checkTranslator(1000, {
          min: -200,
          max: 200,
          invert: true,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = true. Position = vertical. Range includes zero');
        checkTranslator(500, {
          min: 180,
          max: 180,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = false. Position = horizontal. Start of the canvas is default');
        checkTranslator(1500, {
          min: new Date(10000),
          max: new Date(10000),
          invert: true,
          axisType: 'continuous',
          dataType: 'datetime'
        }, {}, 'Datetime. Invert = true. Position = horizontal. End of the canvas is default');
        checkTranslator(500, {
          min: 0,
          max: 0,
          axisType: 'continuous',
        ...
        checkTranslator(1500, {
          ...
          axisType: 'continuous',
          dataType: 'numeric'
        }, {isHorizontal: false}, 'Invert = false. Position = vertical. End of the canvas is default');
        checkTranslator(1500, {
          min: -100,
          max: 0,
          axisType: 'continuous',
          dataType: 'numeric'
        }, {}, 'Invert = false. Position = horizontal. End of the canvas is default');
      });
      QUnit.test('Other values', function(assert) {
        var that = this;
        function checkTranslator(value, expected, message, range, options) {
          var translator = that.createTranslator($.extend({
            min: 100,
            max: 1000,
            axisType: 'continuous',
            dataType: 'numeric'
        ...
        checkTranslator('canvas_position_start', 500, 'Start position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_top', 500, 'Top position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_left', 500, 'Left position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_end', 1500, 'End position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_bottom', 1500, 'Bottom position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_right', 1500, 'Right position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_middle', 1000, 'Middle position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        checkTranslator('canvas_position_center', 1000, 'Center position. Range is negative. Invert = false', {
          min: -1000,
          max: -100
        });
        ...
        checkTranslator('canvas_position_start', 1500, 'Start position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_top', 500, 'Top position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_left', 500, 'Left position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_end', 500, 'End position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_bottom', 1500, 'Bottom position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_right', 1500, 'Right position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_middle', 1000, 'Middle position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
        checkTranslator('canvas_position_center', 1000, 'Center position. Range is negative. Invert = true', {
          min: -1000,
          max: -100,
          invert: true
        });
      ...
      QUnit.test('canvas_position_start, canvas_position_end with paddings', function(assert) {
        ...
        translator.updateCanvas($.extend({
          startPadding: 10,
          endPadding: 40
        }, canvasTemplate));
      ...
      QUnit.test('All translators process special cases', function(assert) {
        ...
        function checkTranslator(value, expected, message, range) {
          ...
          translator2DModule.Translator2D.prototype.translateSpecialCase = function(value) {
        ...
        checkTranslator('100', 100500, 'Numeric translator can process special case', {
          min: 100,
          max: 1000,
          axisType: 'continuous',
          dataType: 'numeric'
        });
        checkTranslator('100', 100500, 'Datetime translator can process special case', {
          min: new Date(1000),
          max: new Date(10000),
          axisType: 'continuous',
          dataType: 'datetime'
        });
        checkTranslator('100', 100500, 'Logarithmic translator can process special case', {
          min: 1,
          max: 1000,
          axisType: 'logarithmic',
          dataType: 'numeric'
        });
        checkTranslator('100', 100500, 'Discrete translator can process special case', {
          categories: ['a1', 'a2'],
          axisType: 'discrete',
          dataType: 'string'
        });
      ...
      QUnit.test('scroll. Canvas start point is zero', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate, {left: 0});
        var translator = new translator2DModule.Translator2D(range, canvas, {
          isHorizontal: true,
          breaksSize: 0
        });
        ...
        assert.equal(translator._canvasOptions.endPoint, 580);
        var zoom = translator.zoom(-100, 1);
      ...
      QUnit.test('zoom single point range', function(assert) {
        ...
        var canvas = $.extend({}, canvasTemplate);
        var translator = new translator2DModule.Translator2D(range, canvas, {isHorizontal: true});
        var zoom = translator.zoom(48, 1.1);
      ...
      QUnit.test('Scroll with whole range', function(assert) {
        ...
        assert.deepEqual(translator.zoom(-10, 1, {
          startValue: 5,
          endValue: 300
        }), {
          min: 5,
      ...
      QUnit.test('Min/max correction by wholeRange', function(assert) {
        ...
        assert.deepEqual(translator.zoom(-9.6, 0.9, {
          startValue: 3.93,
          endValue: 5.5
        }), {
          min: 3.93,
      ...
      QUnit.test('Do not go out from wholeRange. number with big precision', function(assert) {
        ...
        assert.deepEqual(translator.zoom(-2.745928338762212, 0.9739413680781759, {
          startValue: 10,
          endValue: 0
        }), {
          max: 10,
      ...
      QUnit.test('Scroll. Max in the user break, that was out of the range', function(assert) {
        ...
        var translator = createTranslatorWithScaleBreaks.call(this, {
          breaks: breaks,
          userBreaks: breaks.concat([{
            from: 700,
            to: 800
          }])
        });
        var scrollInfo = translator.zoom(25, 1);
      ...
      QUnit.test('Zoom out. Max and min in the user break, that was out of the range', function(assert) {
        ...
        var scrollInfo = translator.zoom(-25, 0.9);
        assert.equal(scrollInfo.min, 0);
        assert.equal(scrollInfo.max, 800);
      });
      ...
      QUnit.test('positive scroll with stick=true', function(assert) {
        ...
        var zoom = translator.zoom(250, 1);
        assert.equal(zoom.min, 'a2');
        assert.equal(zoom.max, 'a5');
      ...
      QUnit.test('getMinScale', function(assert) {
        ...
        assert.equal(this.createTranslator({
          minVisible: 'a1',
          maxVisible: 'a2'
        }).getMinScale(true).toFixed(2), 2.00, ' visibleCategories zoom in');
        assert.equal(this.createTranslator({
          minVisible: 'a1',
          maxVisible: 'a2'
        }).getMinScale(false).toFixed(2), 0.5, 'visibleCategories zoom out');
        assert.equal(this.createTranslator({
          minVisible: 'a2',
          maxVisible: 'a2'
        }).getMinScale(true).toFixed(2), 1, 'visibleCategories zoom in');
        assert.equal(this.createTranslator({
          minVisible: 'a2',
          maxVisible: 'a2'
        }).getMinScale(false).toFixed(2), 0.33, 'visibleCategories zoom out');
        assert.equal(this.createTranslator({
          minVisible: 'a2',
          maxVisible: 'a4'
        }).getMinScale(true).toFixed(2), 3, 'visibleCategories zoom in');
        assert.equal(this.createTranslator({
          minVisible: 'a2',
          maxVisible: 'a4'
        }).getMinScale(false).toFixed(2), 0.6, 'visibleCategories zoom out');
      });
      ...
      QUnit.test('From discrete to continuous', function(assert) {
        var translator = this.createTranslator({
          categories: [new Date(100000), new Date(200000), new Date(300000), new Date(400000), new Date(500000)],
          axisType: 'discrete',
          dataType: 'datetime'
        });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","viz/translators/translator2d","core/utils/math"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("viz/translators/translator2d"), require("core/utils/math"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=translator2D.tests.js.map