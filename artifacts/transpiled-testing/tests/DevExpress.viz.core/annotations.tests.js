...
(["testing/tests/DevExpress.viz.core/annotations.tests.js"], ["jquery","viz/core/annotations","../../helpers/vizMocks.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core/annotations.tests.js", ["jquery", "viz/core/annotations", "../../helpers/vizMocks.js"], function($__export) {
  "use strict";
  var $,
      createAnnotations,
      plugins,
  ...
  function checkCloudPath(assert, cloud, points, rotation) {
    var path = cloud.attr.lastCall.args[0].d;
  ...
  function roundPathCoords(cloud) {
    return cloud.attr.lastCall.args[0].d.replace(/\.\d+/g, function(d) {
      return d.slice(0, 3);
    });
  }
  return {
    setters: [function($__m) {
      ...
      createAnnotations = $__m.createAnnotations;
      plugins = $__m.plugins;
    }, function($__m) {
    ...
    execute: function() {
      environment = {
        beforeEach: function() {
          ...
          this.widget = {
            ...
            _isAnnotationBounded: function() {
              return true;
            },
            _renderer: this.renderer,
            _getAnnotationCoords: sinon.stub().returns({
              x: 100,
              y: 200,
              canvas: {
                left: 0,
        ...
        createAnnotations: function(items) {
          var options = arguments[1] !== (void 0) ? arguments[1] : {};
          var customizeAnnotation = arguments[2];
          var widget = arguments[3] !== (void 0) ? arguments[3] : 'chart';
          var pullOptions = plugins[widget].members._pullOptions;
      ...
      QUnit.test('Do not draw annotation if cannot get coords, or coords out of canvas', function(assert) {
        var $__4 = this;
        var testCase = function(anchor, coords, message) {
          $__4.widget._getAnnotationCoords.returns($.extend({canvas: {
              left: 50,
              top: 50,
              right: 50,
              bottom: 50,
          ...
          var annotation = $__4.createAnnotations([$.extend({
            type: 'image',
            image: {url: 'some_url'}
          }, coords)], {})[0];
          annotation.draw($__4.widget, $__4.group);
          assert.equal($__4.renderer.stub('image').callCount, 0, message);
        };
        testCase({
          x: undefined,
          y: undefined
      ...
      QUnit.test('Image params', function(assert) {
        var annotation = this.createAnnotations([{
          x: 10,
          y: 20,
          type: 'image',
          image: {
      ...
      QUnit.test('Merge customizeAnnotation result and common+item options', function(assert) {
        var customizeAnnotation = sinon.stub().returns({image: {url: 'customized_url'}});
        var itemOptions = {
          x: 10,
          y: 20,
          type: 'image',
        ...
        var annotation = this.createAnnotations([itemOptions], {image: {height: 10}}, customizeAnnotation)[0];
        annotation.draw(this.widget, this.group);
      ...
      QUnit.test('Merge customizeAnnotation result and common. Image as string', function(assert) {
        var customizeAnnotation = sinon.stub().returns({image: 'customized_url'});
        var itemOptions = {
          x: 10,
          y: 20,
          type: 'image',
      ...
      QUnit.test('Merge customizeAnnotation result and common. Customize callback returned undefined', function(assert) {
        var customizeAnnotation = sinon.stub().returns(undefined);
        var itemOptions = {
          x: 10,
          y: 20,
          type: 'image',
      ...
      QUnit.test('Image option is string', function(assert) {
        ...
        var annotation = this.createAnnotations([itemOptions])[0];
        annotation.draw(this.widget, this.group);
      ...
      QUnit.test('Image option is string in common annotation options', function(assert) {
        ...
        var annotation = this.createAnnotations([itemOptions], {image: 'some_url'})[0];
        annotation.draw(this.widget, this.group);
        assert.deepEqual(this.renderer.image.firstCall.args[4], 'some_url');
      });
      QUnit.test('Draw image inside a plaque with borders and arrow', function(assert) {
        ...
        var wrapperGroup = this.renderer.g.getCall(0).returnValue;
        assert.equal(wrapperGroup.append.firstCall.args[0], this.group);
        assert.deepEqual(wrapperGroup.css.lastCall.args[0], {
          'font-size': 20,
          fill: 'red'
        });
        var annotationGroup = this.renderer.g.getCall(1).returnValue;
        ...
        var plaque = this.renderer.path.getCall(0).returnValue;
        ...
        var cloudGroup = this.renderer.g.getCall(2).returnValue;
        ...
        var contentGroup = this.renderer.g.getCall(3).returnValue;
      ...
      QUnit.test('Get size from annotation setting if it less than image size', function(assert) {
        ...
        this.renderer.g.reset();
        annotation.draw(this.widget, this.group);
        var imageArgs = this.renderer.image.lastCall.args;
      ...
      QUnit.module('Check plaque path on pane bounds', {
        ...
        createAnnotations: function(items, options) {
          ...
          }, options), undefined, plugins.chart.members._pullOptions);
        }
      });
      ...
      QUnit.test('Draw text inside plaque', function(assert) {
        ...
        var text = this.renderer.text.firstCall.returnValue;
      ...
      QUnit.test('Draw text with width/height', function(assert) {
        ...
        assert.deepEqual(text.setMaxSize.lastCall.args, [100, 150, {
          textOverflow: 'hide',
          wordWrap: 'normal'
        }]);
        var plaque = this.renderer.path.getCall(0).returnValue;
      ...
      QUnit.test('Use functional template to draw custom annotation', function(assert) {
        ...
        assert.deepEqual(template.getCall(0).args, [{
          argument: 0,
          x: 0,
          y: 0,
          type: 'custom',
          template: template,
          data: {isCustomData: true}
        }, this.renderer.g.getCall(3).returnValue.element]);
      });
      ...
      QUnit.test('customizeTooltip in item', function(assert) {
        ...
        var annotation = this.createAnnotations(items, {customizeTooltip: function() {
            return 1;
          }})[0];
        annotation.draw(this.widget, this.group);
      ...
      QUnit.test('customizeAnnotation is not a function', function(assert) {
        var customizeAnnotation = {call: true};
        var itemOptions = {
          x: 10,
          y: 20,
          type: 'image',
      ...
      QUnit.test('Do not create annotation with wrong type', function(assert) {
        var annotations = this.createAnnotations([{type: 'image'}, {type: 'wrongtype'}, {type: 'text'}], {});
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","viz/core/annotations","../../helpers/vizMocks.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("viz/core/annotations"), require("../../helpers/vizMocks.js"));
  else
