...
(["testing/tests/DevExpress.viz.core.series/label.tests.js"], ["jquery","../../helpers/vizMocks.js","viz/series/points/base_point","viz/series/points/label","../../helpers/chartMocks.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core.series/label.tests.js", ["jquery", "../../helpers/vizMocks.js", "viz/series/points/base_point", "viz/series/points/label", "../../helpers/chartMocks.js"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      pointModule,
  ...
  return {
    ...
    execute: function() {
      ...
      QUnit.module('Label displayFormat', {format: labelModule.Label._DEBUG_formatText});
      QUnit.test('Numeric label displayFormat', function(assert) {
        ...
        assert.equal(this.format(pointValues, {
          format: '0#.##',
          argumentFormat: '0#.#',
          displayFormat: '{argumentText} : {valueText}'
        }), '30.1 : 1.62');
        assert.equal(this.format(pointValues, {
          format: '0#.##',
          argumentFormat: '0#.#',
      ...
      QUnit.test('Date label displayFormat', function(assert) {
        ...
        assert.equal(this.format(pointValues, {
          format: '0#.##',
          argumentFormat: 'year',
          displayFormat: '{argumentText:yyyy} : {valueText:#.###}'
        }), '2022 : 1.62');
      });
      QUnit.test('Label formatting order', function(assert) {
        var pointValues = {value: 2};
        assert.equal(this.format(pointValues, {
          format: 'format',
          displayFormat: ''
        }), 'format');
      ...
      QUnit.module('Draw Label', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          this.translator = new MockTranslator({getCanvasVisibleArea: {
              minX: 0,
              maxX: 100,
              minY: 0,
              maxY: 210
      ...
      QUnit.test('Draw(true) - draw label', function(assert) {
        ...
        assert.deepEqual(this.renderer.text.lastCall.returnValue.attr.firstCall.args, [{
          text: '15',
          align: 'left',
          'class': 'label_class'
        }], 'text attr');
      ...
      QUnit.test('Draw label with custom format', function(assert) {
        this.options.customizeText = function() {
          return this.argumentText + ':' + this.valueText + ':' + this.seriesName;
        };
      ...
      QUnit.test('Draw labelBackground (fill is specified)', function(assert) {
        ...
        assert.deepEqual(label._background.stub('attr').getCall(1).args, [{
          x: 2,
          y: 36,
          width: 36,
          height: 18
      ...
      QUnit.test('Draw labelBackground (fill is specified) with reduction color', function(assert) {
        ...
        assert.deepEqual(label._background.stub('attr').getCall(2).args, [{
          x: 2,
          y: 36,
          width: 36,
          height: 18
      ...
      QUnit.module('Connector', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          this.options.background = {fill: 'red'};
        }}));
      ...
      QUnit.test('To right', function(assert) {
        ...
        var label = this.createAndDrawLabel();
        var connector = this.getConnectorElement();
        label.setFigureToDrawConnector({
          x: 100,
          y: 100,
          width: 0,
          height: 0
      ...
      QUnit.test('bar with odd height. Horizontal connector', function(assert) {
        var label = this.createLabelWithBBox({
          x: 0,
          y: 0,
          height: 20,
          width: 10
      ...
      QUnit.module('Connector. Pie strategy', $.extend({}, environment, {beforeEach: function() {
          environment.beforeEach.apply(this, arguments);
          this.options.background = {fill: 'red'};
        }}));
      ...
      QUnit.module('Set options', $.extend({}, environment, {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.translator = new MockTranslator({getCanvasVisibleArea: {
              minX: 0,
      ...
      QUnit.module('Dispose', $.extend({}, environment, {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.translator = new MockTranslator({getCanvasVisibleArea: {
              minX: 0,
      ...
      QUnit.module('getBoundingRect', $.extend({}, environment, {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.translator = new MockTranslator({getCanvasVisibleArea: {
              minX: 0,
      ...
      QUnit.module('Layouted label', $.extend({}, environment, {beforeEach: function() {
          this.options = {
            horizontalOffset: 0,
            verticalOffset: 0,
            radialOffset: 0,
      ...
      QUnit.test('Groups are added to the corresponding elements', function(assert) {
        ...
        label.show();
        label.shift(10, 10);
        var innerGroup = label._insideGroup;
      ...
      QUnit.test('getBoundingRect after shift', function(assert) {
        var label = this.createLabel();
        label.show();
        var innerGroup = this.group.children[0];
        label.shift(100, 100);
        var BBox = label.getBoundingRect();
        assert.ok(BBox, 'bbox returned');
      ...
      QUnit.test('getLayoutOptions without background', function(assert) {
        this.options.background = {visible: false};
        var label = this.createLabel(this.options);
        var options = label.getLayoutOptions();
      ...
      QUnit.test('fit with background', function(assert) {
        ...
        label.fit(31);
        var text = this.renderer.text.getCall(0).returnValue;
        var background = this.renderer.rect.getCall(0).returnValue;
        assert.equal(text.setMaxSize.args[0][0], 15, 'Max width param');
      ...
      QUnit.module('Format label', $.extend({}, environment, {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.data = {
            formatObject: {
...
//# sourceMappingURL=label.tests.js.map