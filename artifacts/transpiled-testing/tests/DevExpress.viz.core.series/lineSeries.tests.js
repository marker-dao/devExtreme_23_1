System.register(["jquery", "../../helpers/vizMocks.js", "core/utils/common", "viz/core/utils", "viz/series/points/base_point", "viz/series/base_series", "../../helpers/chartMocks.js", "core/utils/object"], function (_export, _context) {
  "use strict";

  var $, vizMocks, noop, vizUtils, pointModule, SeriesModule, insertMockFactory, MockAxis, restoreMockFactory, objectUtils, Series, originalPoint, createSeries, environment, environmentWithSinonStubPoint, createPoint, mockPoints, checkThreeGroups;
  ...
  return {
    setters: [function (_jquery) {
      ...
      vizMocks = _helpersVizMocksJs;
    }, function (_coreUtilsCommon) {
      noop = _coreUtilsCommon.noop;
    }, function (_vizCoreUtils) {
      vizUtils = _vizCoreUtils.default;
    }, function (_vizSeriesPointsBase_point) {
      pointModule = _vizSeriesPointsBase_point.default;
    }, function (_vizSeriesBase_series) {
      SeriesModule = _vizSeriesBase_series.default;
    }, function (_helpersChartMocksJs) {
      insertMockFactory = _helpersChartMocksJs.insertMockFactory;
      MockAxis = _helpersChartMocksJs.MockAxis;
      restoreMockFactory = _helpersChartMocksJs.restoreMockFactory;
    }, function (_coreUtilsObject) {
      objectUtils = _coreUtilsObject.default;
    }],
    execute: function () {
      ...
      (function LineElements() {
        ...
        QUnit.test('getPointCenterByArg', function (assert) {
          ...
          assert.deepEqual(centerCoord, {
            x: 'translation_result',
            y: 'translation_result'
          });
        });
        ...
        QUnit.test('draw tracker. strokeWidth < default value', function (assert) {
          ...
          assert.deepEqual(series._trackers[0].data.lastCall.args, [{
        ...
        QUnit.test('Draw data with null values. Remove segment', function (assert) {
          ...
          element1 = series._trackers[0], settings = element1._stored_settings;
          assert.ok(element2.stub('remove').called, 'second element should be removed');
        ...
        QUnit.test('Update width option - Normal State', function (assert) {
          ...
          series.createPoints();
          series.draw();
          const newOptions = $.extend(true, {}, series._options, {
            width: 'm width'
          });
      ...
      (function SplineElements() {
        ...
        QUnit.test('draw polar line. Two point with equal angle', function (assert) {
          ...
          assert.equal(this.renderer.stub('path').getCall(0).args[1], 'line');
          assert.deepEqual(this.renderer.stub('path').getCall(0).args[0], [series.getPointByPos(0), series.getPointByPos(1)]);
        });
      ...
      (function StackedLine() {
        ...
        QUnit.test('getStackName returns axis\' name', function (assert) {
          assert.equal(createSeries({
            axis: 'axisName',
            stack: 'stackName',
            type: 'stackedLine'
          }, this.renderOptions).getStackName(), 'axis_axisName', 'series with defined axis name');
          assert.equal(createSeries({
            type: 'stackedLine',
            stack: 'stackName'
          }, this.renderOptions).getStackName(), 'axis_default', 'series with undefined axis name');
        });
      })();
    }
  };
