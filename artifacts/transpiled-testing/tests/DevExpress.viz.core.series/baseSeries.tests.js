...
(["testing/tests/DevExpress.viz.core.series/baseSeries.tests.js"], ["jquery","../../helpers/vizMocks.js","core/utils/common","core/utils/type","viz/series/points/base_point","viz/series/base_series","../../helpers/chartMocks.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core.series/baseSeries.tests.js", ["jquery", "../../helpers/vizMocks.js", "core/utils/common", "core/utils/type", "viz/series/points/base_point", "viz/series/base_series", "../../helpers/chartMocks.js"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      commonUtils,
  ...
  return {
    ...
    execute: function() {
      ...
      createSeries = function(options, settings) {
        ...
        var labelsGroup = settings.renderer.g();
        options = $.extend(true, {
          type: 'mockType',
          visible: true,
          point: {
      ...
      QUnit.test('isStacked. stackedline', function(assert) {
        var series = createSeries({type: 'stackedline'});
        assert.ok(series);
      ...
      QUnit.test('isFullStacked. fullstackedline', function(assert) {
        var series = createSeries({type: 'fullstackedline'});
        assert.ok(series);
      ...
      QUnit.test('IncidentOccurred. Data without argument field', function(assert) {
        var data = [{val: 1}, {val: 2}, {val: 3}, {val: 4}, {val: 5}];
        var incidentOccurred = sinon.spy();
        var options = {
          type: 'mockType',
          argumentField: 'arg',
      ...
      QUnit.test('Pass errorBars options to point (on creation). ErrorBars are not visible', function(assert) {
        var series = createSeries({
          type: 'line',
          valueErrorBar: {someErrorBarsProperty: true}
        });
        var data = [{
          arg: 1,
          val: 2
        }];
        series.areErrorBarsVisible = function() {
          return false;
        };
        series.updateData(data);
        series.createPoints();
      ...
      QUnit.test('Pass errorBars options to point (on update). ErrorBars are visible', function(assert) {
        ...
        var points = series.getPoints();
        assert.equal(points[0].update.callCount, 1);
        assert.deepEqual(points[0].update.firstCall.args[1].errorBars, {
          someErrorBarsProperty: true,
          error: true
        }, 'errorBars options passed to point');
      });
      ...
      QUnit.module('Drawing', {
        beforeEach: function() {
          ...
          this.argumentAxis = new MockAxis({renderer: this.renderer});
      ...
      QUnit.test('Draw simple data. Without animation', function(assert) {
        ...
        assert.deepEqual(series.drawnSegments, [{
          segment: series.getAllPoints(),
          num: 0,
      ...
      QUnit.test('marker group style after updating', function(assert) {
        ...
        var appliedOptions = $.extend(true, {}, series._options, options);
      ...
      QUnit.module('Disposing', {
        beforeEach: function() {
          ...
          _this.series = createSeries({border: {visible: true}}, {
      ...
      QUnit.test('Groups disposing when tracker not drawn', function(assert) {
        ...
        var errorBarGroup = series._errorBarGroup = this.renderer.g();
      ...
      QUnit.test('Series groups disposing private API (T1028256)', function(assert) {
        ...
        var elementsDetachSpy = series._elementsGroup.stub('dispose');
        var bordersDetachSpy = series._bordersGroup.stub('dispose');
        var markersDetachSpy = series._markersGroup.stub('dispose');
      ...
      QUnit.test('Arrays disposing', function(assert) {
        var series = this.series;
        var trackerElement = this.renderer.g();
        series.updateData([{
          arg: 1,
          val: 1
        }]);
      ...
      QUnit.module('Labels visibility', {beforeEach: function() {
          this.options = {
            type: 'line',
            label: {visible: true},
            maxLabelCount: 10
      ...
      QUnit.test('Release selected point - selected series in includePoints mode - release selection point', function(assert) {
        var series = createSeries({selectionMode: 'includepoints'});
        series.updateData([{arg: 1}]);
        series.createPoints();
        series.select();
      ...
      QUnit.test('Release hover point with series.hover in includePoints mode ', function(assert) {
        var series = createSeries({selectionMode: 'excludepoints'});
        series.updateData([{arg: 1}]);
        series.createPoints();
        series.hover('includePoints');
        series.select();
      ...
      QUnit.test('apply point view after drawing selected series', function(assert) {
        var series = this.series;
        var options = {selectionMode: 'includePoints'};
        series.updateOptions($.extend(true, {}, series._options, options));
        series.updateData([{
          arg: 1,
          val: 1
      ...
      QUnit.test('apply point view after drawing hovered series', function(assert) {
        var series = this.series;
        var options = {hoverMode: 'includePoints'};
        series.updateOptions($.extend(true, {}, series._options, options));
        series.updateData([{
          arg: 1,
          val: 1
      ...
      QUnit.test('set visibility from options. updating true -> false', function(assert) {
        ...
        series.updateOptions($.extend(true, {}, series._options, {visible: false}));
        series.draw();
        assert.equal(spy.callCount, 0);
        assert.ok(!series.isVisible());
        assert.ok(series._group.stub('remove').called);
      ...
      QUnit.test('set visibility from options. updating true -> true', function(assert) {
        ...
        series.updateOptions($.extend(true, {}, series._options, {visible: true}));
        series.draw();
        assert.equal(spy.callCount, 0);
        assert.ok(series.isVisible());
      ...
      QUnit.test('get range from visible series', function(assert) {
        var series = createSeries({visible: true});
        assert.deepEqual(series.getRangeData(), {isStub: true});
      });
      QUnit.test('get range from invisible series', function(assert) {
        var series = createSeries({visible: false});
        assert.deepEqual(series.getRangeData(), {
          arg: {},
          val: {}
        });
      });
      QUnit.module('API', {
        beforeEach: function() {
          ...
          this.options = {selectionMode: 'seriesSelectionMode'};
          this.renderOptions = {seriesGroup: this.seriesGroup};
        },
        afterEach: environmentWithSinonStubPoint.afterEach
      });
      ...
      QUnit.test('get visible point - drawn series', function(assert) {
        ...
        series.updateData(this.data);
        series.createPoints();
        series._drawnPoints = [series.getAllPoints()[0], series.getAllPoints()[2], series.getAllPoints()[3]];
      ...
      QUnit.test('get axes', function(assert) {
        var valueAxisStub = {};
        var argumentAxisStub = {};
        var series = createSeries({}, {
          valueAxis: valueAxisStub,
          argumentAxis: argumentAxisStub
        });
      ...
      QUnit.test('notification of series. allSeriesPoints mode', function(assert) {
        ...
        series.updateData([{arg: 1}, {arg: 2}]);
        series.createPoints();
        var target = series.getAllPoints()[0];
      ...
      QUnit.test('notification of series. allSeriesPoints. release selection multiply mode', function(assert) {
        var series = createSeries();
        series.updateData([{arg: 1}, {arg: 2}]);
        series.createPoints();
        var allPoints = series.getAllPoints();
        allPoints.forEach(function(point) {
          point.getOptions.returns({selectionMode: 'allSeriesPoints'});
        });
      ...
      QUnit.test('notify other series. allSeriesPoints mode', function(assert) {
        var series1 = createSeries({}, {commonSeriesModes: {pointSelectionMode: 'single'}});
        var series2 = createSeries({}, {commonSeriesModes: {pointSelectionMode: 'single'}});
      ...
      QUnit.test('notification of series. allArgumentPoints. Deselect', function(assert) {
        var series = createSeries({}, {commonSeriesModes: {pointSelectionMode: 'single'}});
        series.updateData([{
          arg: 1,
          val: 1
        }]);
      ...
      QUnit.test('notification of series. allArgumentPoints. multiple mode', function(assert) {
        ...
        series1.createPoints();
        series2.createPoints();
        var series1_point1 = series1.getAllPoints()[0];
        var series2_point1 = series2.getAllPoints()[0];
      ...
      QUnit.test('allArgumentPoints & multiple modes', function(assert) {
        var series = [createSeries({}, {commonSeriesModes: {pointSelectionMode: 'multiple'}}), createSeries({}, {commonSeriesModes: {pointSelectionMode: 'multiple'}})];
        var selectPoint = function(target) {
        ...
        var point1 = series[0].getAllPoints()[0];
        var point2 = series[1].getAllPoints()[0];
      ...
      QUnit.test('call event pipe on hoverPoint', function(assert) {
        ...
        var series = createSeries({}, {eventPipe: eventPipe});
        series.updateData([{
          arg: 1,
          val: 1
        }]);
        series.createPoints();
        series.hoverPoint(series.getAllPoints()[0]);
        assert.deepEqual(eventPipe.lastCall.args[0], {
          target: series.getAllPoints()[0],
          action: 'pointHover'
        });
      });
      ...
      QUnit.test('Hover series in hovered state', function(assert) {
        ...
        var series = createSeries({}, {eventTrigger: eventTrigger});
        series.hover();
        eventTrigger.reset();
        series.hover();
      ...
      QUnit.module('States with aggregation', {
        beforeEach: function() {
          ...
          chartSeriesNS['serieswithresample'] = $.extend({}, chartSeriesNS['mocktype'], {_resample: function() {
              return [{
                argument: 1,
                value: 1
              }, {
          ...
          this.setupAggregation = function(min, max, start, end, canvasLength) {
            ...
            that.argumentAxis.getTranslator = function() {
              return translator;
            };
          ...
          that.series = createSeries({
            ...
            commonSeriesModes: {pointSelectionMode: 'single'},
            argumentAxis: this.argumentAxis
          });
        },
        afterEach: function() {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","core/utils/common","core/utils/type","viz/series/points/base_point","viz/series/base_series","../../helpers/chartMocks.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("core/utils/common"), require("core/utils/type"), require("viz/series/points/base_point"), require("viz/series/base_series"), require("../../helpers/chartMocks.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=baseSeries.tests.js.map