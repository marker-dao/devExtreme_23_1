...
(["testing/tests/DevExpress.viz.core.series/symbolPoint.tests.js"], ["jquery","../../helpers/vizMocks.js","viz/series/points/base_point","viz/series/points/label","../../helpers/chartMocks.js","viz/core/tooltip","viz/components/consts"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.core.series/symbolPoint.tests.js", ["jquery", "../../helpers/vizMocks.js", "viz/series/points/base_point", "viz/series/points/label", "../../helpers/chartMocks.js", "viz/core/tooltip", "viz/components/consts"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      pointModule,
      ...
      translateXData,
      translateYData,
      checkTooltipFormatObject;
  function getTranslators(translateX, translateY) {
    return {
      arg: new MockTranslator({translate: translateX}),
      val: new MockTranslator({translate: translateY})
    };
  }
  function createCorrectionLabel(x, y, visibility, bBox) {
    ...
    bBox && (point._getSymbolBBox = function() {
      return bBox;
    });
    ...
    execute: function() {
      ...
      QUnit.module('Point coordinates translation', {
        ...
        setVerticalCategoryTranslators: function() {
          this.translators = getTranslators(translateXData, {
            cat1: 'ycat1',
            cat2: 'ycat2',
            cat3: 'ycat3',
            cat4: 'ycat4',
            cat5: 'ycat5',
            'canvas_position_default': 'default'
          });
        }
      });
      ...
      QUnit.test('getCrosshairData', function(assert) {
        ...
        assert.equal(point.vx, point.x, 'crosshair x Coord');
        assert.equal(point.vy, point.y, 'crosshair y Coord');
        assert.deepEqual(point.getCrosshairData(), {
          x: point.vx,
          y: point.vy,
          xValue: point.argument,
          yValue: point.value,
          axis: 'valueAxisName'
        });
      });
      ...
      QUnit.module('Correct value', {beforeEach: function() {
          var that = this;
          this.options = {
            widgetType: 'chart',
            label: {visible: false},
          ...
          this.series = {
            ...
            getValueAxis: function() {
              return {validateUnit: that.validateUnit};
            },
            _argumentChecker: function() {
              return true;
            },
      ...
      QUnit.module('HasValue method', {beforeEach: function() {
          this.opt = {
            widgetType: 'chart',
            label: {visible: false},
            styles: {}
      ...
      QUnit.module('Check point in visible area', {beforeEach: function() {
          ...
          var visibleArea = [100, 200, 100, 210];
          this.series = {
            name: 'series',
            isFullStackedSeries: function() {
              return false;
      ...
      QUnit.test('hasCoords returns false if point doesn\'t have y', function(assert) {
        this.translators.val = new MockTranslator({translate: {1: null}});
        var point = createPoint(this.series, {
          argument: 1,
          value: 1
        }, this.options);
      ...
      QUnit.test('With animation enabled', function(assert) {
        ...
        point.x = 10, point.y = 20, point.defaultY = 0;
        point.visible = true;
      ...
      QUnit.test('With animation enabled. Rotated', function(assert) {
        ...
        point.x = 10, point.y = 20, point.defaultX = 0;
        point.visible = true;
        point.draw(this.renderer, this.groups, true);
        assert.ok(point);
        assert.ok(point.graphic);
      ...
      QUnit.test('double drawing without animation', function(assert) {
        ...
        point.x = 20, point.y = 50, point.defaultY = 0;
        point.visible = true;
        point.draw(this.renderer, this.groups, false);
        assert.ok(point);
        assert.ok(point.graphic);
      ...
      QUnit.test('Animate point with complete', function(assert) {
        ...
        var stubComplete = sinon.stub();
        point.x = 10, point.y = 20, point.defaultY = 0;
        point.visible = true;
        point.draw(this.renderer, this.groups, true);
        point.animate(stubComplete, {translate: {
            x: point.x,
            y: point.y
          }});
        var complete = point.graphic.stub('animate').lastCall.args[2];
        complete();
      ...
      QUnit.test('Draw point with errorBar', function(assert) {
        ...
        assert.deepEqual(this.renderer.path.lastCall.returnValue.attr.lastCall.args[0], {visibility: 'visible'});
      ...
      QUnit.test('Hide error bar', function(assert) {
        ...
        assert.ok(point.graphic);
        assert.strictEqual(this.renderer.path.callCount, 1);
        assert.deepEqual(this.renderer.path.lastCall.returnValue.attr.lastCall.args[0], {visibility: 'hidden'});
      });
      ...
      QUnit.module('Update point', {beforeEach: function() {
          var that = this;
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.errorBarGroup = this.renderer.g();
      ...
      QUnit.test('Update errorBars', function(assert) {
        ...
        assert.deepEqual(this.renderer.path.lastCall.returnValue.attr.firstCall.args[0], {visibility: 'visible'});
      ...
      QUnit.test('Update errorBars - hide errorBar', function(assert) {
        ...
        assert.strictEqual(this.renderer.path.lastCall.returnValue.attr.callCount, 2);
        assert.deepEqual(this.renderer.path.lastCall.returnValue.attr.firstCall.args[0], {visibility: 'visible'});
        assert.deepEqual(this.renderer.path.lastCall.returnValue.attr.secondCall.args[0], {visibility: 'hidden'});
      });
      QUnit.module('Point visibility', {beforeEach: function() {
          var that = this;
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.data = {
      ...
      QUnit.test('Apply style for visible point (in visible area)', function(assert) {
        ...
        point.applyView();
        assert.ok(point.graphic);
        assert.deepEqual(point.graphic.stub('attr').secondCall.args[0], {
          r: 6,
          style: {
            fill: 'red',
            stroke: 'yellow',
      ...
      QUnit.test('keep style after redraw', function(assert) {
        ...
        point.applyView();
        point.draw(this.renderer, this.groups);
        assert.deepEqual(this.renderer.circle.getCall(0).returnValue.attr.lastCall.args[0].r, this.options.styles.selection.r);
      });
      QUnit.module('Tooltip', {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          var axis = this.axis = new MockAxis({renderer: this.renderer});
          this.data = {
            value: 10,
            argument: 1
          };
          ...
          var StubTooltip = vizMocks.stubClass(tooltipModule.Tooltip, {
            ...
            getOptions: sinon.spy(function() {
              return ({argumentFormat: 'argument-format'});
            })
          });
      ...
      QUnit.test('Get tooltip format object with stackPoints without stack', function(assert) {
        var point = createPoint(this.series, this.data, this.options);
        var options1 = $.extend(true, {}, this.options);
        var data1 = {
          value: 15,
          argument: 1,
          originalValue: 15,
        ...
        var options2 = $.extend(true, {}, this.options);
        var data2 = {
          value: 20,
          argument: 1,
          originalValue: 20,
        ...
        var stackPoints = [createPoint($.extend({}, this.series, {name: 'series1'}), data1, options1), createPoint($.extend({}, this.series, {name: 'series2'}), data2, options2)].map(function(point) {
          point.setPercentValue(40, 30);
      ...
      QUnit.test('Get tooltip format object with stackPoints with stack', function(assert) {
        this.series.getStackName = function() {
          return 'someStackName';
        };
        var point = createPoint(this.series, this.data, this.options);
        var options1 = $.extend(true, {}, this.options);
        var data1 = {
      ...
      QUnit.test('Get tooltip format object, series of points no visible', function(assert) {
        this.series.isVisible = function() {
          return false;
        };
        var point = createPoint(this.series, this.data, this.options);
        var options1 = $.extend(true, {}, this.options);
      ...
      QUnit.test('Get tooltip format object with aggreagation info', function(assert) {
        ...
        var formatObject = createPoint(this.series, this.data, this.options).getTooltipFormatObject(this.tooltip);
        assert.equal(formatObject.valueText, '10:undefined\nrange');
      ...
      checkTooltipFormatObject = function(assert, point, argument, value, originalValue, originalArgument, seriesName, percent, total) {
      ...
      QUnit.module('Get coordinates', {beforeEach: function() {
          this.data = {
            value: 10,
            argument: 1
          };
      ...
      QUnit.module('Get default coordinates', {beforeEach: function() {
          this.data = {
            value: 10,
            argument: 1
          };
      ...
      QUnit.test('Label format object percent argument', function(assert) {
        ...
        var label = pt._label;
        var format = label.setData.firstCall.args[0];
      ...
      QUnit.test('Circle symbol point', function(assert) {
        ...
        assert.deepEqual(label.setFigureToDrawConnector.firstCall.args[0], {
          x: 33,
          y: 32,
          r: 8
        });
      ...
      QUnit.test('Default, not rotated', function(assert) {
        var label = createCorrectionLabel.call(this, 30, 32, true, {
          x: 40,
          y: 40,
          height: 10,
          width: 20
      ...
      QUnit.test('Default, rotated fullstacked with negative value', function(assert) {
        ...
        this.options.rotated = true;
        var label = createCorrectionLabel.call(this, 43, 12, true, {
          x: 40,
          y: 40,
          height: 10,
          width: 20
      ...
      QUnit.module('Calculate tracker size', {beforeEach: function() {
      ...
      QUnit.module('Tracker size calculation on MS Touch Devices', {beforeEach: function() {
          this.renderer = new vizMocks.Renderer();
          this.group = this.renderer.g();
          this.options = {
            widgetType: 'chart',
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","viz/series/points/base_point","viz/series/points/label","../../helpers/chartMocks.js","viz/core/tooltip","viz/components/consts"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("viz/series/points/base_point"), require("viz/series/points/label"), require("../../helpers/chartMocks.js"), require("viz/core/tooltip"), require("viz/components/consts"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=symbolPoint.tests.js.map