...
(["testing/tests/DevExpress.ui.widgets.treeList/dataController.tests.js"], ["ui/tree_list/ui.tree_list","jquery","data/data_source/data_source","data/array_store","core/guid","data/query","../../helpers/treeListMocks.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.treeList/dataController.tests.js", ["ui/tree_list/ui.tree_list", "jquery", "data/data_source/data_source", "data/array_store", "core/guid", "data/query", "../../helpers/treeListMocks.js"], function($__export) {
  "use strict";
  var $,
      DataSource,
      ArrayStore,
    ...
    execute: function() {
      ...
      QUnit.module('Initialization', {
        ...
        QUnit.test('nodes should not be recreated after expand', function(assert) {
          ...
          this.applyOptions({onNodesInitialized: nodesInitialized});
          this.dataController.setDataSource(dataSource);
          dataSource.load();
          var rootNode = this.getRootNode();
          this.expandRow(2);
        ...
        QUnit.test('Initialize from dataSource with plain structure when virtual scrolling enabled', function(assert) {
          ...
          var dataSource = createDataSource(array, {}, {pageSize: 3});
          this.applyOptions({
            scrolling: {
              mode: 'virtual',
              preventPreload: true
        ...
        QUnit.test('Initialize when remoteOperations and virtual scrolling are enabled and two pages are loaded', function(assert) {
          ...
          var dataSource = createDataSource(array, {}, {pageSize: 2});
          this.applyOptions({
            scrolling: {mode: 'virtual'},
        ...
        QUnit.test('Getting key when there are keyExpr and store hasn\'t key', function(assert) {
          ...
          this.applyOptions({dataSource: {load: function() {
        ...
        QUnit.test('Call forEachNode method when the first parameter as the array of nodes', function(assert) {
          ...
          this.dataController.setDataSource(dataSource);
          dataSource.load();
          var rootNode = this.dataController.getRootNode();
        ...
        QUnit.test('Initialize when data as classes with a hierarchical structure', function(assert) {
          ...
          var dataSource = [new Person(1, [new Person(2, [new Person(3), new Person(4), new Person(5)])])];
          this.applyOptions({
            autoExpandAll: true,
            dataSource: dataSource,
            dataStructure: 'tree',
      ...
      QUnit.module('Expand/Collapse nodes', {
        beforeEach: setupModule,
        afterEach: teardownModule
      }, function() {
        ...
        QUnit.test('Update expandedRowKeys', function(assert) {
          ...
          this.dataController.setDataSource(dataSource);
          dataSource.load();
          var expandedRowKeys = this.dataController.option('expandedRowKeys');
          this.expandRow(1);
        ...
        QUnit.test('Cancel expand row on an expanding event', function(assert) {
          ...
          var dataSource = createDataSource(array);
          that.applyOptions({onRowExpanding: function(e) {
              e.cancel = true;
            }});
          that.dataController.setDataSource(dataSource);
          dataSource.load();
        ...
        QUnit.test('Cancel collapse row on a collapsing event', function(assert) {
          ...
          var dataSource = createDataSource(array);
          that.applyOptions({onRowCollapsing: function(e) {
              e.cancel = true;
            }});
          that.dataController.setDataSource(dataSource);
          dataSource.load();
      ...
      QUnit.module('Sorting', {
        ...
        QUnit.test('sortOrder changing by columnOption should be applied', function(assert) {
          this.setupTreeList({dataSource: this.items});
      ...
      QUnit.module('Remote Operations', {
        beforeEach: function() {
          this.items = [{
            id: 1,
            parentId: 0,
        ...
        QUnit.test('Initial load when dataSource has filter and filterMode is withAncestors (default)', function(assert) {
          var loadingArgs = [];
          var arrayStore = new ArrayStore({data: this.items});
          this.setupTreeList({
            expandNodesOnFiltering: true,
            dataSource: {
              load: function(loadOptions) {
        ...
        QUnit.test('Initial load when dataSource has filter and filterMode is withAncestors (default) when remoteOperations false', function(assert) {
          ...
          assert.deepEqual(loadingArgs, [{userData: {}}], 'loading arguments');
          var items = this.dataController.items();
          assert.equal(items.length, 4, 'count items');
          assert.equal(items[0].data.name, 'Name 3', 'item 1 name value');
          assert.equal(items[0].level, 0, 'item 1 level');
        ...
        QUnit.test('expand -> collapse -> expand row', function(assert) {
          var items;
          var loadingArgs = [];
          this.setupTreeList({dataSource: {store: {
                type: 'array',
                data: this.items,
                onLoading: function(e) {
                  loadingArgs.push(e);
        ...
        QUnit.test('Checking the \'hasChildren\' property of the node after expand when key as Guid', function(assert) {
          ...
          this.setupTreeList({dataSource: [{
              id: keys[0],
              parentId: 0
            }, {
              id: keys[1],
              parentId: new Guid(keys[0])
            }]});
      ...
      QUnit.module('Load data on demand', {
        beforeEach: function() {
          this.setupTreeList = function(options) {
            if (!('loadingTimeout' in options)) {
              options.loadingTimeout = null;
            ...
            if (!('remoteOperations' in options)) {
              options.remoteOperations = {filtering: true};
            }
        ...
        QUnit.test('Initialize load', function(assert) {
          var loadOptions = [];
          this.setupTreeList({dataSource: {load: function(e) {
                var d = $.Deferred();
                var nodes = [];
                var parentIds = e.parentIds;
                if (parentIds) {
      ...
      QUnit.module('Filtering', {
        ...
        QUnit.test('There should not be looping after search when filterMode is \'fullBranch\' and all nodes are expanded', function(assert) {
          ...
          this.setupTreeList({
            ...
            expandNodesOnFiltering: true,
            searchPanel: {text: 'b'},
            expandedRowKeys: data.map(function(item) {
              return item.id;
            })
          });
      ...
      QUnit.module('Push API', {
        ...
        QUnit.test('Update the node using the push method of the store when changes have the \'data\' property', function(assert) {
          ...
          store.push([{
            ...
            data: {
              name: 'Root name 1',
              data: {name: 'Inner name 1'}
            }
          }]);
          this.clock.tick(100);
          var firstNode = this.getVisibleRows()[0];
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["ui/tree_list/ui.tree_list","jquery","data/data_source/data_source","data/array_store","core/guid","data/query","../../helpers/treeListMocks.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("ui/tree_list/ui.tree_list"), require("jquery"), require("data/data_source/data_source"), require("data/array_store"), require("core/guid"), require("data/query"), require("../../helpers/treeListMocks.js"));
  else
