System.register(["generic_light.css!", "ui/tree_list/ui.tree_list", "jquery", "animation/fx", "../../helpers/treeListMocks.js", "data/array_store"], function (_export, _context) {
  "use strict";

  var $, fx, setupTreeListModules, ArrayStore, setupModule, teardownModule;
  return {
    setters: [function (_generic_lightCss) {}, function (_uiTree_listUiTree_list) {}, function (_jquery) {
      $ = _jquery.default;
    }, function (_animationFx) {
      fx = _animationFx.default;
    }, function (_helpersTreeListMocksJs) {
      setupTreeListModules = _helpersTreeListMocksJs.setupTreeListModules;
    }, function (_dataArray_store) {
      ArrayStore = _dataArray_store.default;
    }],
    execute: function () {
      QUnit.testStart(function () {
        const markup = '<!--qunit-fixture-->\
    <div id="container">\
        <div id="treeList">\
        </div>\
    </div>\
';
      ...
      QUnit.module('Editing', {
        ...
        QUnit.module('Add row', () => {
          ...
          QUnit.test('AddRow method should expand row and add item after parent', function (assert) {
            ...
            assert.deepEqual(rows[1].data, {
              parentId: 1
            }, 'row 1 data should contains parentId');
          ...
          QUnit.test('Add item in node without children (Angular)', function (assert) {
            ...
            this.dataController.optionChanged({
              ...
              value: [1, 2],
              previousValue: [1, 2]
            }); // simulate the call from ngDoCheck hook
            this.clock.tick(100);
            ...
            assert.deepEqual(rows[2].data, {
              parentId: 2
            }, 'third row data should contain parentId');
          });
          QUnit.test('AddRow method witout parameters should add item at begin', function (assert) {
            ...
            assert.deepEqual(rows[0].data, {
              parentId: 0
            }, 'row 0 data should contains parentId');
          ...
          QUnit.test('Add row to the end via changes with index = -1', function (assert) {
            ...
            // act
            this.addRowViaChanges(undefined, {
              insertAfterKey: 2
            });
            this.clock.tick(10);
        ...
        QUnit.test('Editing with validation - save edit data', function (assert) {
          ...
          this.options.columns[0].validationRules = [{
            type: 'required'
          }];
          this.setupTreeList();
          this.rowsView.render($testElement);
        ...
        QUnit.test('Add row when \'keyExpr\' and \'parentIdExpr\' options are specified as functions', function (assert) {
          ...
          this.options.keyExpr = function (data) {
            return data['id'];
          };
      ...
      ['reshape', 'repaint'].forEach(function (refreshMode) {
        QUnit.module('Refresh mode ' + refreshMode, {
          ...
          QUnit.test('Remove node with children', function (assert) {
            ...
            this.setupTreeList();
            this.expandRow(1);
            this.getDataSource().store().insert({
              id: 3,
              field1: 'test3',
              hasItems: false
            });
