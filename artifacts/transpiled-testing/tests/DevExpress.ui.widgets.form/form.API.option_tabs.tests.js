...
(["testing/tests/DevExpress.ui.widgets.form/form.API.option_tabs.tests.js"], ["jquery","ui/form/ui.form","generic_light.css!"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.form/form.API.option_tabs.tests.js", ["jquery", "ui/form/ui.form", "generic_light.css!"], function($__export) {
  "use strict";
  var $,
      testStart,
      test,
  ...
  return {
    ...
    execute: function() {
      var $__5;
      (($__5 = QUnit, testStart = $__5.testStart, test = $__5.test, module = $__5.module, assert = $__5.assert, $__5));
      testStart(function() {
      ...
      FormTestWrapper = function() {
        function FormTestWrapper(options) {
          this._form = $('#form').dxForm(options || {items: [{
              itemType: 'tabbed',
              name: 'tabbedItem',
              tabs: [{
                title: 'title0',
        ...
        return ($traceurRuntime.createClass)(FormTestWrapper, {
          _getTabsTextsElements: function() {
            return this._form.$element().find('.dx-tab-content .dx-tab-text');
          },
          _getMultiViewItemsElements: function() {
          ...
          checkTabBadge: function(tabIndex, expectedText) {
            assert.equal($('.dx-tabs-item-badge').eq(tabIndex).text(), expectedText, (tabIndex + " tab badge"));
          },
          checkTabDisabled: function(tabIndex, expectedValue) {
            var $tabItems = $('.dx-tab');
            assert.equal($tabItems.eq(tabIndex).hasClass('dx-state-disabled'), expectedValue, (tabIndex + " tab disabled"));
          },
          checkTabIcon: function(tabIndex, expectedIcon) {
            var $icon = $('.dx-tab .dx-icon').eq(tabIndex);
            assert.ok($icon.hasClass(("dx-icon-" + expectedIcon)), (tabIndex + " tab icon"));
          },
          checkTabContentTemplate: function(tabIndex, $expectedTemplate) {
            var $multiViewItemContent = $('.dx-multiview-item-content').eq(tabIndex);
            assert.equal($multiViewItemContent.html(), $('<div>').append($expectedTemplate).html(), (tabIndex + " tab template"));
          },
          checkTabTemplate: function(tabIndex, $expectedTemplate) {
            var $multiViewItemContent = $('.dx-tab-content').eq(tabIndex);
            assert.equal($multiViewItemContent.html(), $('<div>').append($expectedTemplate).html(), (tabIndex + " tab template"));
          },
          checkTabTitle: function(tabIndex, expectedText) {
            var $title = this._getTabsTextsElements().eq(tabIndex);
            assert.strictEqual($title.text(), expectedText, (tabIndex + " tab title"));
          },
          checkTabsElements: function(expectedTabsCount) {
            var $tabs = this._getTabsTextsElements();
            assert.equal($tabs.length, expectedTabsCount, 'count of tabs elements');
            var $multiViewItems = this._getMultiViewItemsElements();
            assert.equal($multiViewItems.length, expectedTabsCount, 'count of multiview items elements');
          },
          checkTabOption: function(tabIndex, optionName, expected) {
            var value = this._form.option(("items[0].tabs[" + tabIndex + "]." + optionName));
            assert.strictEqual(value, expected, (optionName + " of " + tabIndex + " tab"));
          },
          checkEditorRendered: function(id, isEditorRendered) {
          ...
          checkTabSelection: function(expectedSelectedIndex) {
            var $form = this._form.$element();
            var $tabs = $form.find('.dx-item.dx-tab');
            var $multiViewItem = $form.find('.dx-item.dx-multiview-item');
      ...
      module('Public API: option method', function() {
        ...
        test('Change the tab template option', function() {
          var testWrapper = new FormTestWrapper();
          var template1 = '<div class=\'custom-tab-template-1\'></div>';
          testWrapper.setOption('items[0].tabs[0].tabTemplate', template1);
          testWrapper.checkFormsReRender();
          testWrapper.checkTabTemplate(0, $(template1));
          var template2 = '<div class=\'custom-tab-template-2\'></div>';
        ...
        ['badge', 'icon', 'template', 'tabTemplate', 'title'].forEach(function(optionName) {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","ui/form/ui.form","generic_light.css!"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("ui/form/ui.form"), require("generic_light.css!"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=form.API.option_tabs.tests.js.map