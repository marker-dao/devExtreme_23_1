...
(["testing/tests/DevExpress.viz.sankey/sankey.base.tests.js"], ["jquery","./commonParts/common.js","viz/core/renderers/renderer","viz/palette","viz/themes"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.viz.sankey/sankey.base.tests.js', ['jquery', './commonParts/common.js', 'viz/core/renderers/renderer', 'viz/palette', 'viz/themes'], true, function ($__require, exports, module) {
    ...
    QUnit.test('Trigger error on invalid source data', function (assert) {
        const invalidDataSets = [[{ source: 'A', target: 'B', weight: 1 }, { source: 'A', target: 'C' }], [{ source: 'A', target: 'B', weight: 0 }, { source: 'A', target: 'C', weight: 1 }], [{ source: 'A', target: 1, weight: 1 }, { source: 'A', target: 'C', weight: 1 }], [{ source: 'A', target: 'B', weight: 1 }, { source: 1, target: 'C', weight: 1 }], [{ source: 'A', target: 'B', weight: 1 }, { source: 'A', target: 'C', weight: -5 }], ['ABCD', { source: 'A', target: 'C', weight: 1 }]];
        const expectedErrorCodes = ['E2007', 'E2009', 'E2008', 'E2008', 'E2009', 'E2007'];
        const spy = sinon.spy();
    ...
    QUnit.test('Trigger "cycle detected" error on invalid source data', function (assert) {
        const invalidDataSets = [[{ source: 'A', target: 'B', weight: 1 }, { source: 'B', target: 'A', weight: 1 }], [{ source: 'A', target: 'B', weight: 1 }, { source: 'B', target: 'C', weight: 1 }, { source: 'C', target: 'D', weight: 1 }, { source: 'D', target: 'A', weight: 1 }]];
        const spy = sinon.spy();
    ...
    QUnit.test('Sort nodes in all cascades by default', function (assert) {
        createSankey({
            layoutBuilder: layoutBuilder,
            dataSource: [{ source: 'A', target: 'M', weight: 1 }, { source: 'A', target: 'M', weight: 1 }, { source: 'B', target: 'M', weight: 1 }, { source: 'B', target: 'M', weight: 1 }, { source: 'C', target: 'M', weight: 1 }, { source: 'C', target: 'N', weight: 1 }, { source: 'M', target: 'X', weight: 1 }, { source: 'N', target: 'X', weight: 1 }, { source: 'N', target: 'Y', weight: 1 }, { source: 'N', target: 'Z', weight: 1 }]
        });
        assert.deepEqual(spiesLayoutBuilder.computeLayout.returnValues[0].nodes[0].map(function (node) {
            return node._name;
        }), ['A', 'B', 'C']);
        assert.deepEqual(spiesLayoutBuilder.computeLayout.returnValues[0].nodes[1].map(function (node) {
            return node._name;
        }), ['M', 'N']);
        assert.deepEqual(spiesLayoutBuilder.computeLayout.returnValues[0].nodes[2].map(function (node) {
            return node._name;
        }), ['X', 'Y', 'Z']);
    });
    ...
    QUnit.test('layout.overlap utility method', function (assert) {
        ...
        assert.equal(layoutBuilder.overlap({ x: 10, y: 10, width: 10, height: 15 }, { x: 50, y: 50, width: 20, height: 25 }), false);

        assert.equal(layoutBuilder.overlap({ x: 10, y: 10, width: 20, height: 10 }, { x: 15, y: 15, width: 10, height: 10 }), true);

        assert.equal(layoutBuilder.overlap({ x: 10, y: 10, width: 20, height: 10 }, { x: 30, y: 10, width: 10, height: 10 }), true);

        assert.equal(layoutBuilder.overlap( // touching on y-axis is not overlapping
        { x: 10, y: 10, width: 20, height: 10 }, { x: 10, y: 30, width: 10, height: 10 }), false);
    });
});
})
(function(factory) {
...
//# sourceMappingURL=sankey.base.tests.js.map