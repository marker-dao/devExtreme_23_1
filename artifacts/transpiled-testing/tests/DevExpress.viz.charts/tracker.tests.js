...
(["testing/tests/DevExpress.viz.charts/tracker.tests.js"], ["jquery","../../helpers/vizMocks.js","../../helpers/pointerMock.js","viz/components/consts","viz/axes/base_axis","viz/chart_components/crosshair","viz/chart_components/tracker","../../helpers/chartMocks.js","events/hold","core/errors"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.charts/tracker.tests.js", ["jquery", "../../helpers/vizMocks.js", "../../helpers/pointerMock.js", "viz/components/consts", "viz/axes/base_axis", "viz/chart_components/crosshair", "viz/chart_components/tracker", "../../helpers/chartMocks.js", "events/hold", "core/errors"], function($__export) {
  "use strict";
  var $,
      vizMocks,
      pointerMock,
  ...
  function strictEqualForAllFields(assert, instance, etalon, message) {
    var key;
    var keysInInstance = 0;
    var keysInEtalon = 0;
    var allKeysIsValid = true;
    for (key in etalon) {
      keysInEtalon++;
  ...
  function createCompleteTracker(options) {
    ...
    var point1 = createPoint(series1, 1);
    var point2 = createPoint(series1, 2);
    var point3 = createPoint(series2, 1);
    var point4 = createPoint(series2, 2);
    ...
    that.axis.updateOptions({
      hoverMode: 'allargumentpoints',
      mockTickValues: [1, 2, 4],
      argument: 1
    });
    that.point1 = point1;
    that.point2 = point2;
  ...
  return {
    setters: [function($__m) {
      ...
      eventsConsts = $__m.events;
    }, function($__m) {
      axisModule = $__m.default;
    }, function($__m) {
      Crosshair = $__m.Crosshair;
    }, function($__m) {
      trackers = $__m.default;
    }, function($__m) {
    ...
    execute: function() {
      createLegend = function() {
        ...
        legend.stub('getOptions').returns({hoverMode: 'includepoints'});
        return legend;
      };
      createSeries = function() {
        ...
        series.stub('getOptions').returns({hoverMode: 'excludePoints'});
      ...
      createPoint = function(series, argument) {
        ...
        point.stub('getCrosshairData').withArgs(97, 45).returns({
          x: 90,
          y: 40,
          xValue: 10,
          yValue: 20
        ...
        point.stub('getTooltipParams').withArgs('tooltipLocation').returns({
          x: 200,
          y: 100
        });
        point.stub('getOptions').returns({hoverMode: 'onlyPoint'});
        point.series = series;
      ...
      createAxis = function(translator) {
      ...
      createCrosshair = function(startPoint, endPoint) {
        var subCrosshair = vizMocks.stubClass(Crosshair);
        var crosshair = new subCrosshair();
        crosshair.stub('hide');
        crosshair.stub('show');
        return crosshair;
      };
      chartEnvironment = {
        beforeEach: function() {
          ...
          that.axis.getOptions.returns({hoverMode: 'allargumentpoints'});
          ...
          that.options = {
            ...
            eventTrigger: sinon.stub(),
            chart: {getStackedPoints: sinon.spy(function() {
                return 'points_by_stack_name';
              })}
          };
        },
        afterEach: function() {
      ...
      QUnit.test('Subscriptions on init', function(assert) {
        ...
        var events = $._data(rootElement.element, 'events') || {};
        var holdEvents = events[holdEvent.name] || [];
        ...
        assert.strictEqual(holdEvents.length, 0, 'dxhold event handler is not exists');
      });
      ...
      QUnit.test('dxpointermove without series over when shared tooltip', function(assert) {
        var series2 = createSeries();
        var point2 = createPoint(series2);
        ...
        this.point.stub('getCrosshairData').withArgs(97, 45).returns({
          x: 90,
          y: 30,
          xValue: 10,
          yValue: 20
        });
        point2.stub('getCrosshairData').withArgs(97, 45).returns({
          x: 90,
          y: 10,
          xValue: 30,
          yValue: 40
        ...
        var showTooltip = this.options.tooltip.stub('show');
        ...
        assert.deepEqual(showTooltip.lastCall.args[1], {
          x: 200 + 3,
          y: 100 + 5
        });
        assert.deepEqual(showTooltip.lastCall.args[2], {target: this.point});
        assert.equal(showTooltip.lastCall.args[3], undefined);
        assert.equal($traceurRuntime.typeof(showTooltip.lastCall.args[4]), 'function');
        assert.ok(this.options.crosshair.show.calledOnce, 'crosshair[0] moved');
        strictEqualForAllFields(assert, this.options.crosshair.show.lastCall.args[0], {
          point: this.point,
          x: 97,
          y: 45
        });
      ...
      QUnit.test('dxpointermove on series', function(assert) {
        ...
        $(this.renderer.root.element).trigger(getEvent('dxpointermove', {
          pageX: 100,
          pageY: 50,
          target: this.seriesGroup.element
        }));
        this.clock.tick(this.tracker.__trackerDelay);
      ...
      QUnit.test('move on series between two point', function(assert) {
        ...
        this.point.stub('getCrosshairData').withArgs(92, 45).returns({
          x: 92,
          y: 45,
          xValue: 10,
          yValue: 20
      ...
      QUnit.test('dxpointermove over point', function(assert) {
        $(this.renderer.root.element).trigger(getEvent('dxpointermove', {
          pageX: 100,
          pageY: 50,
          target: this.pointElement.element
        }));
      ...
      QUnit.test('dxpointermove over point. move crosshair on point', function(assert) {
        ...
        strictEqualForAllFields(assert, this.options.crosshair.show.getCall(0).args[0], {
          point: this.point,
          x: 97,
          y: 45
        });
        strictEqualForAllFields(assert, this.options.crosshair.show.getCall(1).args[0], {
          point: this.point,
          x: 77,
          y: 45
        });
      ...
      QUnit.test('no pointClick on invisible point', function(assert) {
        var clickEvent = getEvent('dxclick', {
          pageX: 100,
          pageY: 50,
          target: this.seriesGroup.element
        });
      ...
      QUnit.test('mouseout from series to point another series', function(assert) {
        var series2 = createSeries();
        var point = createPoint(series2);
        var pointElement = this.renderer.g();
      ...
      QUnit.test('dxpointermove when there are two series', function(assert) {
        var series2 = createSeries();
        var pointSeries2 = createPoint(series2);
        var series2Element = this.renderer.g();
      ...
      QUnit.test('dxpointermove from point one series to point other series', function(assert) {
        var series2 = createSeries();
        var pointSeries2 = createPoint(series2);
        var point2Element = this.renderer.g();
      ...
      QUnit.test('touch without series over', function(assert) {
        this.series.stub('getNeighborPoint').returns(this.point);
        $(this.renderer.root.element).trigger(getEvent('dxpointerdown', {
          pageX: 100,
          pageY: 50
        }));
        this.clock.tick(this.tracker.__trackerDelay);
      ...
      QUnit.test('click on canvas', function(assert) {
        $(this.renderer.root.element).trigger(getEvent('dxclick', {
          pageX: 100,
          pageY: 50
        }));
        $(this.renderer.root.element).trigger(getEvent('dxclick', {
      ...
      QUnit.test('dxpointermove on series, click', function(assert) {
        ...
        assert.ok(this.options.eventTrigger.withArgs('seriesClick').calledOnce);
        assert.deepEqual(this.options.eventTrigger.withArgs('seriesClick').lastCall.args[1], {
          target: this.point.series,
          event: clickEvent
        }, 'series event arg');
      });
      QUnit.test('dxpointermove on series, click, pointClick with cancel (DEPRECATED) seriesClick', function(assert) {
        ...
        this.options.eventTrigger.withArgs('pointClick').lastCall.args[2]();
        assert.ok(!this.options.eventTrigger.withArgs('seriesClick').called);
        assert.deepEqual(errors.log.lastCall.args, ['W0003', 'pointClick handler argument', 'event.cancel', '22.1', 'Use the \'cancel\' field instead'], 'args of the log method');
      });
      QUnit.test('dxpointermove on series, click, pointClick with cancel seriesClick', function(assert) {
        this.options.eventTrigger = sinon.spy(function(eventName, eventArgs) {
      ...
      QUnit.test('stop propagation event after dispose series on pointClick', function(assert) {
        ...
        this.options.eventTrigger = function(eventName, event, complete) {
          if (eventName === 'pointClick') {
            series.getOptions.returns(null);
            complete();
      ...
      QUnit.test('click on legend with chancel (DEPRECATED) in legendClick handler', function(assert) {
        ...
        this.options.eventTrigger.withArgs('legendClick').lastCall.args[2]();
        assert.ok(!this.options.eventTrigger.withArgs('seriesClick').called);
        assert.deepEqual(errors.log.lastCall.args, ['W0003', 'legendClick handler argument', 'event.cancel', '22.1', 'Use the \'cancel\' field instead'], 'args of the log method');
      });
      ...
      QUnit.test('click on argument axis element. Axis hoverMode is \'none\'', function(assert) {
        ...
        assert.deepEqual(this.options.eventTrigger.withArgs('argumentAxisClick').lastCall.args[1], {
          argument: 'argument1',
          event: event
        });
      });
      QUnit.test('click on argument axis. Parent of target has data', function(assert) {
        var parent = this.renderer.g();
        var target = this.renderer.g();
      ...
      QUnit.test('pointermove on axis element (axis element is multiline text', function(assert) {
        ...
        $(this.renderer.root.element).trigger(getEvent('dxpointermove', {
          ...
          target: {
            tagName: 'tspan',
            parentNode: axisElement.element
          }
        }));
        assert.strictEqual(this.series.notify.callCount, 1);
        assert.strictEqual(this.series.notify.lastCall.args[0].action, 'pointHover');
      ...
      QUnit.test('pointermove from axis element1 to axis element2', function(assert) {
        ...
        var axisElement2 = this.renderer.g();
        this.axis.coordsIn.withArgs(97, 45).returns(true);
      ...
      QUnit.module('Update tracker', $.extend({}, chartEnvironment, {beforeEach: function() {
          chartEnvironment.beforeEach.call(this);
          this.tracker = this.createTracker(this.options);
        }}));
      QUnit.test('update with old series', function(assert) {
        ...
        var series2 = createSeries();
        var series = [this.series, series2];
      ...
      QUnit.test('Can be disposed', function(assert) {
        ...
        $(document).trigger(getEvent('dxpointermove'));
        $(document).trigger(getEvent('dxpointerdown', {
          pageX: 100,
          pageY: 50
        }));
      ...
      QUnit.module('Chart tracker with disabled sticked mode', $.extend({}, chartEnvironment, {beforeEach: function() {
          chartEnvironment.beforeEach.call(this);
      ...
      QUnit.test('hover point after hover legend', function(assert) {
        ...
        this.legend.getItemByCoord.withArgs(87, 35).returns({
          id: 0,
          argument: 'argument1',
          argumentIndex: 11
        });
      ...
      QUnit.test('mousemove from legend item to another one', function(assert) {
        ...
        this.legend.getItemByCoord.withArgs(97 + 10, 45 + 10).returns({
          id: 0,
          argument: 'argument2',
          argumentIndex: 12
        });
      ...
      QUnit.test('point click', function(assert) {
        ...
        $(this.renderer.root.element).trigger(event);
        assert.ok(this.options.eventTrigger.withArgs('pointClick').calledOnce);
        strictEqualForAllFields(assert, this.options.eventTrigger.withArgs('pointClick').lastCall.args[1], {
          target: this.point,
          event: event
        });
      });
      ...
      QUnit.module('clean tracker on user event', {beforeEach: function() {
          var that = this;
          var clean = function() {
      ...
      QUnit.test('tooltip hidden on point unhover, pointHover', function(assert) {
        ...
        $(that.that.options.seriesTrackerGroup.element).trigger(getEvent('mousemove', {
          pageX: 15,
          pageY: 20
        }));
      ...
      QUnit.test('show Tooltip event. TooltipShown fired', function(assert) {
        ...
        var showTooltip = this.tooltip.stub('show');
        assert.equal(showTooltip.callCount, 1, 'tooltip showing was calling once');
      ...
      QUnit.test('show Tooltip event when point is invisible, TooltipShown not fired', function(assert) {
        this.environment.point1.isVisible = function() {
          return false;
        };
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","../../helpers/pointerMock.js","viz/components/consts","viz/axes/base_axis","viz/chart_components/crosshair","viz/chart_components/tracker","../../helpers/chartMocks.js","events/hold","core/errors"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("../../helpers/pointerMock.js"), require("viz/components/consts"), require("viz/axes/base_axis"), require("viz/chart_components/crosshair"), require("viz/chart_components/tracker"), require("../../helpers/chartMocks.js"), require("events/hold"), require("core/errors"));
  else
