System.register(["jquery", "core/utils/common", "core/utils/type", "../../helpers/executeAsyncMock.js", "../../helpers/ajaxMock.js", "data/data_source/data_source", "data/abstract_store", "data/array_store", "data/odata/store", "ui/data_grid/aggregate_calculator", "data/local_store"], function (_export, _context) {
  "use strict";

  var $, noop, typeUtils, executeAsyncMock, ajaxMock, DataSource, Store, ArrayStore, ODataStore, AggregateCalculator, LocalStore, TEN_NUMBERS, moduleConfig;
  return {
    ...
    }, function (_dataAbstract_store) {
      Store = _dataAbstract_store.default;
    }, function (_dataArray_store) {
      ArrayStore = _dataArray_store.default;
    }, function (_dataOdataStore) {
      ODataStore = _dataOdataStore.default;
    }, function (_uiData_gridAggregate_calculator) {
      AggregateCalculator = _uiData_gridAggregate_calculator.default;
    }, function (_dataLocal_store) {
      LocalStore = _dataLocal_store.default;
    }],
    execute: function () {
      ...
      QUnit.test('with no key specified', function (assert) {
        ...
        source.pageIndex(1);
        $.when(source.loadSingle('a', '2').done(function (r) {
          assert.ok(r, 'should ignore skip option');
          assert.deepEqual(r, {
            a: '2',
            i: 3
          }, 'shouldn\'t ignore sort option');
      ...
      QUnit.test('use key if no propName specified', function (assert) {
        new DataSource({
          ...
          assert.deepEqual(r, {
            a: 1
          }, 'should use store.key() as propName');
        });
      });
      ...
      QUnit.test('error handling', function (assert) {
        ...
        source.loadSingle(function (obj) {
          throw Error('forced');
        }, 1).fail(function () {
          failFired.resolve();
        });
      ...
      QUnit.test('error handling (with key)', function (assert) {
        ...
        const source = new DataSource({
          store: new (ArrayStore.inherit({
            _byKeyImpl: function () {
              return $.Deferred().reject(Error('forced')).promise();
            }
          }))({
            key: 'a',
            data: []
      ...
      QUnit.test('error handling (data item cannot be found, T283407)', function (assert) {
        ...
        }).on('loadError', handleError).loadSingle('b', 2) // forces data mining through load implementation
        .fail(handleError).done(function () {
          assert.ok(false);
        });
      });
      ...
      QUnit.test('reload must clean cache of LocalStore when reload', function (assert) {
        ...
        localStorage.setItem(`dx-data-localStore-${storeName}`, JSON.stringify(nextData));
      ...
      QUnit.test('parameter should be passed from customizeStoreLoadOptions to customizeLoadResult if call load inside customizeStoreLoadOptions', function (assert) {
        ...
        source.load();
        assert.deepEqual(calls, [['customizeStoreLoadOptions', 0], ['customizeStoreLoadOptions', 1], ['customizeLoadResult', 1], ['customizeLoadResult', 0]], 'calls order is correct');
      });
      QUnit.test('cancel works', function (assert) {
        ...
        const promise = source.load().fail(handleFail).done(handleDone);
        function handleFail() {
      ...
      QUnit.test('canceling on customizeStoreLoadOptions', function (assert) {
        ...
        source.load().fail(handleFail).done(handleDone);
      });
      QUnit.test('cancel event flow', function (assert) {
        ...
        const promise = ds.load().always(function (state) {
      ...
      QUnit.test('search API, default impl, complex selector', function (assert) {
        ...
        const source = new DataSource(store);
        source.searchExpr(function (item) {
      ...
      QUnit.test('search API, useDefaultSearch', function (assert) {
        ...
        $.when(sourceWithCustomSearch.load(), sourceWithDefaultSearch.load()).fail(function () {
          assert.ok(false, 'Shouldn\'t reach this point');
        });
      });
