System.register(["jquery", "data/query", "../../helpers/data.errorHandlingHelper.js"], function (_export, _context) {
  "use strict";

  var $, QUERY, ErrorHandlingHelper;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_dataQuery) {
      QUERY = _dataQuery.default;
    }, function (_helpersDataErrorHandlingHelperJs) {
      ErrorHandlingHelper = _helpersDataErrorHandlingHelperJs.default;
    }],
    execute: function () {
      QUnit.module('Misc');
      QUnit.test('no operations', function (assert) {
        ...
        const q = QUERY([1, 2, 3]);
        $.when(q.enumerate().done(function (r) {
      ...
      QUnit.test('toArray convenience method', function (assert) {
        assert.deepEqual(QUERY([1, 2, 3]).select(function (i) {
          return i * 2;
        }).toArray(), [2, 4, 6]);
      });
      ...
      QUnit.test('sort using functional getter', function (assert) {
        ...
        const input = [2, 1, 3];
        const output = [3, 2, 1];
        QUERY(input).sortBy(function (item) {
          return 3 - item;
      ...
      QUnit.test('sort with falsy null and undefined values', function (assert) {
        ...
        const sort = function (data, desc) {
          return QUERY(data).sortBy('this', desc || false).toArray();
        };
      ...
      QUnit.test('filter by function', function (assert) {
        ...
        const done = assert.async();
        const input = [2, 0, 5];
        const q1 = QUERY(input).filter(function (i) {
          return i > 1;
        });
        const q2 = q1.filter(function (i) {
          return i < 5;
        });
        $.when(q1.enumerate().done(function (r) {
          assert.deepEqual(r, [2, 5]);
        })).done(function () {
          $.when(q2.enumerate().done(function (r) {
      ...
      QUnit.test('group criterion with function', function (assert) {
        ...
        assert.equal(QUERY(data).filter([functionCondition, arrayCondition]).toArray()[0].value, 2);
        assert.equal(QUERY(data).filter([arrayCondition, functionCondition]).toArray()[0].value, 2);
      });
      QUnit.test('comparison operators', function (assert) {
        ...
        $.when(check(['f', '=', 2], [{
          ...
          f: 2
        }]), check(['f', '<', -2], []), check(['f', '<=', -1], []), check(['f', '=', null], [{
          f: null
        }]), check(['f', '<>', null], [{
          f: 1
        }, {
          f: 2
        }]), check(['f', '>', null], []), check(['f', '>=', null], [{
          f: null
        }]), check(['f', '<', null], []), check(['f', '<=', null], [{
          f: null
        }])).done(function () {
      ...
      QUnit.test('AND', function (assert) {
        ...
        const check = function (op) {
          return QUERY(input).filter([cond1, op, cond2]).enumerate().fail(function () {
        ...
        $.when(check('and'), check('AND'), check('&'), check('&&')).done(function () {
          done();
        }).fail(function () {
          assert.ok(false, 'Shouldn\'t reach this point');
        });
      ...
      QUnit.test('OR', function (assert) {
        ...
        $.when(check('or'), check('OR'), check('|'), check('||')).done(function () {
          done();
        });
      });
      ...
      QUnit.test('conditions are not checked if it\'s not necessary', function (assert) {
        ...
        $.when(check('and'), check('or')).done(done);
      });
      ...
      QUnit.test('filter array and func regression', function (assert) {
        const done = assert.async();
        QUERY([1, 2, 3]).filter([['this', '>', 1], function (i) {
          return i < 3;
        }]).enumerate().done(function (r) {
          assert.deepEqual(r, [2]);
          done();
      ...
      QUnit.test('T141181: Filtering with \'endswith\' operation is not working properly in some cases', function (assert) {
        const done = assert.async();
        QUERY(['bar']).filter(['this', 'endswith', 'fooo']).enumerate().done(function (r) {
          assert.deepEqual(r, []);
          done();
        });
      });
      ...
      QUnit.test('NOT with binary operators', function (assert) {
        ...
        const check = function (op) {
          return QUERY(input).filter([op, cond]).enumerate().fail(function () {
            assert.ok(false, 'Shouldn\'t reach this point');
          }).done(function (r) {
            assert.deepEqual(r, [{
              f: 'a'
      ...
      QUnit.test('NOT with group operation', function (assert) {
        ...
        const cond2 = ['f', '<>', 3];
        const check = function (op) {
          return QUERY(input).filter([op, [cond1, 'and', cond2]]).enumerate().fail(function () {
            assert.ok(false, 'Shouldn\'t reach this point');
          }).done(function (r) {
            assert.deepEqual(r, [{
              f: 1
        ...
        function createFn(crit) {
          return function () {
            QUERY([{
              ...
              bar: true,
              foobar: true
            }]).filter(crit).enumerate();
          };
        }
        assert.throws(createFn([['foo'], ['bar'], 'or', ['foobar']]));
        assert.throws(createFn([['foo'], '&&', ['bar'], '||', ['foobar']]));
        assert.throws(createFn([['foo'], 'or', ['bar'], ['foobar']]));
        assert.throws(createFn([['foo'], 'or', ['bar'], 'and', ['foobar']]));
      });
      ...
      QUnit.test('T348632: Rows in a group with an undefined group value are not sorted', function (assert) {
        ...
        assert.deepEqual(QUERY(data).sortBy('foo').thenBy('bar', true).toArray(), [{
          foo: null,
          bar: 2
        }, {
          foo: null,
      ...
      QUnit.test('custom aggregate with 3 args', function (assert) {
        ...
        QUERY([{
          ...
          f: 2
        }]).aggregate(0, function (accumulator, obj) {
          return accumulator += obj.f;
      ...
      QUnit.test('custom aggregate with 1 args', function (assert) {
        assert.expect(1);
        const done = assert.async();
        QUERY([1, 2]).aggregate(function (accumulator, obj) {
      ...
      QUnit.test('standard aggregate functions', function (assert) {
        ...
        $.when(q.count().done(function (r) {
          assert.equal(r, 2, 'countable count');
        }), q.filter(function (i) {
          return i.f < 2;
        }).count().done(function (r) {
      ...
      QUnit.test('select with prop name list', function (assert) {
        ...
        $.when(QUERY(data).select('a', 'c', 'missing').enumerate().done(function (r) {
          assert.deepEqual(r, [{
            a: 'A',
            c: 'C'
          }]);
      ...
      QUnit.test('select single property returns object', function (assert) {
        ...
        QUERY(data).select('a').enumerate().done(function (r) {
          assert.deepEqual(r, data);
          done();
        });
      });
      ...
      QUnit.test('slice', function (assert) {
        ...
        const done = assert.async();
        const q = QUERY([1, 2, 3]);
        $.when(q.slice(-1, 1).enumerate().done(function (r) {
          assert.deepEqual(r, [1]);
      ...
      QUnit.test('re-enumeration of changed data (sorting)', function (assert) {
        ...
        $.when(q.enumerate()).done(function () {
      ...
      QUnit.test('special for javascript cases (filtering)', function (assert) {
        ...
        $.when(query.filter(['this', 0]).enumerate().done(function (r) {
