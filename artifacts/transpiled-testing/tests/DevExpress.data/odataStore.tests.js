...
(["testing/tests/DevExpress.data/odataStore.tests.js"], ["jquery","data/odata/utils","data/odata/store","data/odata/context","core/guid","core/config","../../helpers/data.errorHandlingHelper.js","../../helpers/ajaxMock.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.data/odataStore.tests.js", ["jquery", "data/odata/utils", "data/odata/store", "data/odata/context", "core/guid", "core/config", "../../helpers/data.errorHandlingHelper.js", "../../helpers/ajaxMock.js"], function($__export) {
  "use strict";
  var $,
      EdmLiteral,
      ODataStore,
    ...
    execute: function() {
      ...
      QUnit.test('fieldTypes are updated correctly', function(assert) {
        var params = [{
          ...
          expectedFieldTypes: {
            name: 'String',
            '5d46402c-7899-4ea9-bd81-8b73c47c7683': 'Int32'
          }
        }, {
          storeOptions: {keyType: {
              id1: 'Int32',
              id2: 'String'
            }},
          expectedFieldTypes: {
      ...
      QUnit.test('options are not changed after the merger', function(assert) {
        ...
        assert.deepEqual(storeOptions.keyType, {
          id1: 'Int32',
          id2: 'String'
        });
      ...
      QUnit.test('works', function(assert) {
        ...
        ajaxMock.setup({
          ...
          callback: function(bag) {
            this.responseText = {d: {results: [bag]}};
          }
        });
        ajaxMock.setup({
          url: 'odata4.org',
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).load().done(function(r, extra) {
          assert.ok(!extra);
          assert.deepEqual(r, [{
            ...
            'method': 'get',
            'url': 'odata2.org',
            'xhrFields': {'withCredentials': undefined}
          }]);
        }), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).load().done(function(r, extra) {
          assert.ok(!extra);
          assert.deepEqual(r, [{
            'accepts': {'json': 'application/json;odata=verbose,text/plain'},
            'async': true,
      ...
      QUnit.test('with params', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).load(options).done(function(r, extra) {
          assert.ok(!extra);
          assert.deepEqual(r, [{
            '$orderby': 'a desc',
            '$filter': 'b eq 1',
          ...
          version: 3,
          url: 'odata2.org'
        }).load(options).done(function(r, extra) {
          assert.ok(!extra);
          assert.deepEqual(r, [{
            '$orderby': 'a desc',
            '$filter': 'b eq 1',
      ...
      QUnit.test('with explicit expand', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata.org'}).load(options).done(function(r, extra) {
          assert.ok(!extra);
      ...
      QUnit.test('with requireTotalCount', function(assert) {
        ...
        ajaxMock.setup({
          ...
          callback: function(bag) {
            this.responseText = {
              value: [bag.data['$count']],
              '@odata.count': 123
            };
          }
        });
        var promises = [new ODataStore({url: 'odata.org'}).load({requireTotalCount: true}).done(function(r, extra) {
          assert.deepEqual(r, ['allpages']);
          assert.deepEqual(extra, {totalCount: 123});
        }), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).load({requireTotalCount: true}).done(function(r, extra) {
      ...
      QUnit.test('works', function(assert) {
        ...
        var assertFunc = function(count) {
          assert.equal(count, 123);
        };
        var promises = [new ODataStore({url: 'odata2.org'}).totalCount().done(assertFunc), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).totalCount().done(assertFunc), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).totalCount().done(assertFunc)];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      QUnit.test('respects customQueryParams (T413790)', function(assert) {
        var done = assert.async();
        var capturedAjaxSettings;
        ajaxMock.setup({
          url: 'example.com',
          callback: function(settings) {
            capturedAjaxSettings = settings;
            this.responseText = {d: {__count: 123}};
          }
        });
        new ODataStore({url: 'example.com'}).totalCount({customQueryParams: {p1: 42}}).done(function(count) {
          assert.equal(count, 123);
      ...
      QUnit.test('works', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey(42).done(assertFunc), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).byKey(42).done(assertFunc), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).byKey(42).done(assertFunc)];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      QUnit.test('with expand', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey(42, {expand: ['prop1.subprop', 'prop2']}).done(assertFunc), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).byKey(42, {expand: ['prop1.subprop', 'prop2']}).done(assertFunc), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).byKey(42, {expand: ['prop1.subprop', 'prop2']}).done(function(value) {
      ...
      QUnit.test('expand should be undefined if it is not specified in the extraOptions', function(assert) {
        ...
        new ODataStore({url: 'odata2.org'}).byKey(42, {select: ['prop1']}).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      QUnit.test('with select', function(assert) {
        ...
        ajaxMock.setup({
          ...
          callback: function(bag) {
            this.responseText = {d: {
                expandClause: bag.data.$expand,
                selectClause: bag.data.$select
              }};
          }
        });
        ajaxMock.setup({
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey(42, {select: ['prop1', 'prop2']}).done(function(value) {
          ...
          expand: ['prop1'],
          select: ['prop1.subprop', 'prop2']
        }).done(function(value) {
          assert.deepEqual(value, {
            expandClause: 'prop1',
            selectClause: 'prop1/subprop,prop2'
          });
        }), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).byKey(42, {select: ['prop1.subprop', 'prop2']}).done(function(value) {
      ...
      QUnit.test('compound key', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey({
          key1: 42,
          key2: 'abc'
        }).done(assertFunc), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).byKey({
          key1: 42,
      ...
      QUnit.test('Guid as key', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey(new Guid(guid)).done(function(r) {
          assert.ok(r.url.indexOf('(guid\'' + guid + '\')') > -1);
        }), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).byKey(new Guid(guid)).done(function(r) {
          assert.ok(r.url.indexOf('(guid\'' + guid + '\')') > -1);
        }), new ODataStore({
          version: 4,
          url: 'odata2.org'
      ...
      QUnit.test('string as key', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata2.org'}).byKey('abc').done(assertFunc), new ODataStore({
          version: 3,
          url: 'odata2.org'
        }).byKey('abc').done(assertFunc), new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).byKey('abc').done(assertFunc)];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      QUnit.test('key type conversions by keyType option', function(assert) {
        ...
        var promises = [new ODataStore({
          url: 'odata.org',
          keyType: 'Int32'
        }).byKey(42).done(function(r) {
      ...
      QUnit.test('Should show E4024 error when searching or filtering an Int32 field (T848126)', function(assert) {
        ...
        function assertFunc(message, operation) {
          assert.equal(message.replace(/\d+_\d+/, '[VERSION]'), ("E4024 - String function " + operation + " cannot be used with the data field id of type Int32. See:\nhttp://js.devexpress.com/error/[VERSION]/E4024"));
        }
        var promises = [store.load({filter: ['id', 'contains', '123']}).fail(function(error) {
          assertFunc(error.message, 'contains');
        }), store.load({filter: ['id', 'startswith', '123']}).fail(function(error) {
          assertFunc(error.message, 'startswith');
        }), store.load({filter: ['id', 'endswith', '123']}).fail(function(error) {
          assertFunc(error.message, 'endswith');
        }), store.load({filter: ['id', 'notcontains', '123']}).fail(function(error) {
      ...
      QUnit.test('no double conversion for Int64', function(assert) {
        ...
        var assertFunc = function(r) {
          assert.ok(r.url.indexOf('(123L)') > -1);
        };
        var promises = [new ODataStore({
          url: 'odata2.org',
          keyType: 'Int64'
        }).byKey(new EdmLiteral('123L')).done(assertFunc), new ODataStore({
          version: 4,
          url: 'odata2.org',
          keyType: 'Int64'
        }).byKey(new EdmLiteral('123L')).done(assertFunc)];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      ...
      QUnit.test('the request with filter is correct if key type is Int64, Decimal or Single (T346008)', function(assert) {
        ...
        var promises = [store.load({filter: ['id1', '=', '123']}).done(function(r) {
          assert.equal(r.filter, 'id1 eq 123L');
        }), store.load({filter: ['id2', '=', '123']}).done(function(r) {
          assert.equal(r.filter, 'id2 eq 123m');
        }), store.load({filter: ['id3', '=', '123']}).done(function(r) {
          assert.equal(r.filter, 'id3 eq 123f');
        }), new ODataStore({
          url: 'odata.org',
          key: 'id',
          keyType: 'Int64'
        }).load({filter: ['id', '=', '123']}).done(function(r) {
      ...
      QUnit.test('Guid key processed as string if key type is String (T316902)', function(assert) {
        ...
        var promises = [new ODataStore({
          ...
          assert.equal($traceurRuntime.typeof(r.result[0].id1), 'string');
        }), new ODataStore({
          url: 'odata.org',
          key: ['id1', 'id2'],
          keyType: {
          ...
          assert.equal($traceurRuntime.typeof(r.result[0].id1), 'string');
          assert.equal($traceurRuntime.typeof(r.result[0].id2), 'object');
        })];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      ...
      QUnit.test('works', function(assert) {
        ...
        var store = new ODataStore({
          ...
          onInserting: function(data) {
            logger.push(['onInserting', data]);
          },
          onInserted: function(data, key) {
            logger.push(['onInserted', data, key]);
          }
        });
        store.on('inserting', function(data) {
          logger.push(['inserting', data]);
        });
        store.on('inserted', function(data, key) {
          logger.push(['inserted', data, key]);
        });
        store.insert({
          id: 1,
          foo: 'bar'
        }).fail(function() {
          assert.fail(false, MUST_NOT_REACH_MESSAGE);
        }).done(function(data, key) {
          logger.push(['done', data, key]);
        }).done(function() {
          assert.deepEqual(logger, [['onInserting', {
            id: 1,
            foo: 'bar'
          }], ['inserting', {
            id: 1,
      ...
      QUnit.test('with 201 status', function(assert) {
        ...
        store.insert({id: 1}).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).done(function(data, key) {
          assert.equal(key, 1);
        }).always(done);
      ...
      QUnit.test('works', function(assert) {
        ...
        var promises = [new ODataStore({
        ...
        }).on('updating', compileLoggerFor('updating')).on('updated', compileLoggerFor('updated')).update(1, {foo: 'bar'}).done(compileLoggerFor('done')), new ODataStore({
          version: 3,
          url: 'odata.org/DataSet',
          beforeSend: function(request) {
            assert.equal(request.url, 'odata.org/DataSet(1)');
          ...
          onUpdating: compileLoggerFor('onUpdating'),
          onUpdated: compileLoggerFor('onUpdated')
        }).on('updating', compileLoggerFor('updating')).on('updated', compileLoggerFor('updated')).update(1, {foo: 'bar'}).done(compileLoggerFor('done'))];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).done(function() {
          assert.deepEqual(log, [['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', {foo: 'bar'}, 1], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', {foo: 'bar'}, 1], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', {foo: 'bar'}, 1], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', 'OK', 1]]);
        }).always(done);
      });
      QUnit.test('works with useLegacyStoreResult', function(assert) {
        assert.expect(13);
        ...
        $.when.apply($, promises).fail(function() {
          ...
          assert.deepEqual(log, [['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdating', 1, {foo: 'bar'}], ['updating', 1, {foo: 'bar'}], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', 1, {foo: 'bar'}], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', 1, {foo: 'bar'}], ['onUpdated', 1, {foo: 'bar'}], ['updated', 1, {foo: 'bar'}], ['done', 1, {foo: 'bar'}]]);
        }).always(function() {
          config({useLegacyStoreResult: oldUseLegacyStoreResult});
        }).always(done);
      });
      ...
      QUnit.test('works', function(assert) {
        ...
        var promises = [new ODataStore({
          ...
          onRemoving: compileHandlerFor('onRemoving'),
          onRemoved: compileHandlerFor('onRemoved')
        }).on('removing', compileHandlerFor('removing')).on('removed', compileHandlerFor('removed')).remove(1).done(compileHandlerFor('done')), new ODataStore({
          version: 3,
          url: 'odata2.org/DataSet',
          beforeSend: handleBeforeSend,
          onRemoving: compileHandlerFor('onRemoving'),
          ...
          version: 4,
          url: 'odata4.org/DataSet',
          beforeSend: handleBeforeSend,
          onRemoving: compileHandlerFor('onRemoving'),
          onRemoved: compileHandlerFor('onRemoved')
        }).on('removing', compileHandlerFor('removing')).on('removed', compileHandlerFor('removed')).remove(1).done(compileHandlerFor('done'))];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(function() {
          assert.deepEqual(log, [['onRemoving', 1], ['removing', 1], ['onRemoving', 1], ['removing', 1], ['onRemoving', 1], ['removing', 1], ['onRemoved', 1], ['removed', 1], ['done', 1], ['onRemoved', 1], ['removed', 1], ['done', 1], ['onRemoved', 1], ['removed', 1], ['done', 1]]);
        }).always(done);
      });
      ...
      QUnit.test('Dates, on loading', function(assert) {
        ...
        var promises = [new ODataStore({
          url: 'odata2.org',
          key: 'id'
        }).load({filter: ['date', new Date(1945, 4, 9, 14, 25, 1, 1)]}), new ODataContext({url: 'odata2.org'}).get('methodToGet', {date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataContext({url: 'odata2.org'}).invoke('methodToInvoke', {date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataStore({
          version: 3,
          url: 'odata3.org',
          key: 'id'
        }).load({filter: ['date', new Date(1945, 4, 9, 14, 25, 1, 1)]}), new ODataStore({
          version: 4,
          url: 'odata4.org',
          key: 'id'
        }).load({filter: ['date', new Date(1945, 4, 9, 14, 25, 1, 1)]}), new ODataContext({
          version: 4,
          url: 'odata4.org'
        }).get('function', {date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataContext({
          version: 4,
          url: 'odata4.org'
      ...
      QUnit.test('Dates, on inserting', function(assert) {
        ...
        var promises = [new ODataStore({
          url: 'odata2.org',
          key: 'id'
        }).insert({date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataStore({
          version: 3,
          url: 'odata3.org',
          key: 'id'
        }).insert({date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataStore({
      ...
      QUnit.test('Dates, on updating', function(assert) {
        ...
        var promises = [new ODataStore({
          url: 'odata2.org',
          key: 'id'
        }).update(1, {date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataStore({
          version: 3,
          url: 'odata3.org',
          key: 'id'
        }).update(1, {date: new Date(1945, 4, 9, 14, 25, 1, 1)}), new ODataStore({
      ...
      QUnit.test('Dates, disableable, ODataStore', function(assert) {
        ...
        var promises = [store.load().done(function(r) {
          assert.strictEqual($traceurRuntime.typeof(r[0].dateProperty), 'string');
        }), store.byKey(1).done(function(r) {
          assert.strictEqual($traceurRuntime.typeof(r.dateProperty), 'string');
        })];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      ...
      QUnit.test('Dates, disableable, ODataContext', function(assert) {
        ...
        var promises = [ctx.get('function').done(function(r) {
          assert.strictEqual($traceurRuntime.typeof(r.dateProperty), 'string');
        }), ctx.invoke('action').done(function(r) {
          assert.strictEqual($traceurRuntime.typeof(r.dateProperty), 'string');
        }), ctx.X.load().done(function(r) {
          assert.strictEqual($traceurRuntime.typeof(r[0].dateProperty), 'string');
        }), ctx.Y.load().done(function(r) {
      ...
      QUnit.test('generic HTTP error', function(assert) {
        ...
        new ODataStore({url: 'odata.org'}).byKey('http-error').fail(function(error) {
      ...
      QUnit.test('OData service error', function(assert) {
        ...
        ajaxMock.setup({
          url: 'odata.org(\'error\')',
          status: 500,
          responseText: {error: {message: 'test entity error'}}
        });
        new ODataStore({url: 'odata.org'}).byKey('error').fail(function(error) {
      ...
      QUnit.test('unexpected server response with 200 status', function(assert) {
        ...
        new ODataStore({url: 'odata.org'}).byKey('bad-response').fail(function(error) {
      ...
      QUnit.test('non HTTP failure', function(assert) {
        ...
        var store = new ODataStore({url: 'odata.org/get'});
        store.load().fail(function(e) {
      ...
      QUnit.test('error handlers (query evaluation)', function(assert) {
        ...
        helper.run(function() {
          return store.load({select: function(i) {
              throw Error('test');
            }});
        }, done, assert);
      });
      ...
      QUnit.test('Recursive inner exception (B232110)', function(assert) {
        ...
        ajaxMock.setup({
          url: 'odata.org',
          responseText: {error: {
              'message': {'value': 'An error occurred while processing this request.'},
              'innererror': {
                'message': 'An error occurred while updating the entries.See the inner exception for details.',
                'internalexception': {
                  'message': 'An error occurred while updating the entries.See the inner exception for details.',
                  'internalexception': {'message': 'The DELETE statement conflicted with the REFERENCE constraint'}
                }
              }
            }}
        });
      ...
      QUnit.test('Recursive empty inner exception (T1081655) - primitive case', function(assert) {
        ...
        ajaxMock.setup({
          url: 'odata.org',
          responseText: {error: {
              'message': {'value': 'The DELETE statement conflicted with the REFERENCE constraint'},
              'innererror': {}
            }}
        });
        var helper = new ErrorHandlingHelper();
      ...
      QUnit.test('No recursive inner exception (B232110) on validation error', function(assert) {
        ...
        ajaxMock.setup({
          url: 'odata.org',
          responseText: {error: {message: 'The Product Name field is required.'}}
        });
        var helper = new ErrorHandlingHelper();
        helper.extraChecker = function(error) {
      ...
      QUnit.test('get', function(assert) {
        ...
        var promises = [new ODataContext({url: 'odata.org'}).get('operation', {
          Int: -42,
          Null: null,
          True: true,
          False: false,
      ...
      QUnit.test('T213119: The ODataContext.get method crashes when return value is a boolean true', function(assert) {
        ...
        var promises = [new ODataContext({url: 'odata.org'}).get('true').done(function(r) {
          assert.strictEqual(r, true);
        }), new ODataContext({url: 'odata.org'}).get('false').done(function(r) {
      ...
      QUnit.test('invoke for service operation', function(assert) {
        ...
        var promises = [new ODataContext({url: 'odata.org'}).invoke('operation', {n: 13}).done(function(r) {
          assert.deepEqual(r, {
            method: 'post',
          ...
          version: 4,
          url: 'odata4.org'
        }).invoke('action', {n: 13}).done(function(r) {
          assert.deepEqual(r, {
            method: 'post',
            url: 'odata4.org/action',
            data: '{"n":13}'
      ...
      QUnit.test('invoke and get methods should understand scalar types in result', function(assert) {
        ...
        ajaxMock.setup({
          ...
          responseText: {d: {results: {scalar: 'scalar value'}}}
        });
        var promises = [new ODataContext({url: 'odata.org'}).get('scalar').done(function(r) {
          assert.strictEqual(r, 'scalar value');
        }), new ODataContext({url: 'odata.org'}).invoke('scalar').done(function(r) {
      ...
      QUnit.test('works', function(assert) {
        ...
        var promises = [new ODataStore({url: 'odata.org'}).load({customQueryParams: {customName: 'customValue'}}), new ODataStore({
          version: 3,
          url: 'odata.org'
        }).load({customQueryParams: {customName: 'customValue'}}), new ODataStore({
          version: 4,
          url: 'odata4.org'
      ...
      QUnit.test('url is correct when customQueryParams is undefined (T382714)', function(assert) {
        ...
        ajaxMock.setup({
          ...
          responseText: {message: 'expected message'}
        });
        ajaxMock.setup({
          url: 'odata4.org/messages()',
          responseText: {message: 'unexpected message'}
        });
        new ODataStore({
          version: 4,
          url: 'odata4.org/messages'
        }).load({customQueryParams: undefined}).done(function(r) {
      ...
      QUnit.test('array value for odata 4', function(assert) {
        ...
        var expectedUrl = 'odata4.org(customName=[1,\'\'\'1\',1945-05-09T14:25:01.001Z,' + guid + ',123L])';
        ajaxMock.setup({
          url: expectedUrl,
          responseText: {},
          callback: function(bag) {
        ...
        new ODataStore({
          version: 4,
          url: 'odata4.org'
        }).load({customQueryParams: {customName: value}}).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).always(done);
      });
      ...
      QUnit.test('T226529', function(assert) {
        ...
        ajaxMock.setup({
          ...
          responseText: {d: {results: [{
                keyProperty: 0,
                collectionProperty1: {results: [{keyProperty: 0}, {keyProperty: 1}]},
                collectionProperty2: [{keyProperty: 0}, {keyProperty: 1}]
              }]}}
        });
        new ODataStore({url: 'odata.org'}).load().fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
        }).done(function(r) {
          assert.deepEqual(r, [{
            keyProperty: 0,
            collectionProperty1: [{keyProperty: 0}, {keyProperty: 1}],
            collectionProperty2: [{keyProperty: 0}, {keyProperty: 1}]
          }]);
        }).done(done);
      });
      QUnit.test('Sorting and grouping shouldn\'t add duplicate rules', function(assert) {
        ...
        new ODataStore({url: 'odata.org'}).load({
          sort: 'a',
          group: 'a'
        }).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
      ...
      QUnit.test('Custom headers, query string params and timeout (beforeSend event)', function(assert) {
        ...
        var HEADER_NAME = 'x-custom-header';
      ...
      QUnit.test('link creation helper method', function(assert) {
        ...
        assert.deepEqual(context.objectLink('My Obj', 'key 1'), {__metadata: {uri: 'http://devexpress.com/My%20Obj(\'key%201\')'}});
        assert.equal(context.objectLink('alias', 1).__metadata.uri, 'http://devexpress.com/RealName(1)');
      ...
      QUnit.test('custom entity name in ODataContext', function(assert) {
        ...
        var context = new ODataContext({
          url: 'odata.org',
          entities: {'abc': {name: 'TestEntity1'}}
        });
        assert.expect(0);
        context['abc'].load().always(done);
      });
      ...
      QUnit.test('filterToLower equal false for ODataContext', function(assert) {
        ...
        var promises = [ctx.X.load({filter: ['B', 'contains', 'O']}).done(function(request) {
          assert.equal(request[0].data.$filter, 'contains(tolower(B),\'o\')');
        }), ctx.Y.load({filter: ['B', 'contains', 'O']}).done(function(request) {
      ...
      QUnit.test('oDataFilterToLower equal false for ODataContext', function(assert) {
        ...
        var promises = [ctx.X.load({filter: ['B', 'contains', 'O']}).done(function(request) {
          ...
          assert.equal(request[0].data.$filter, 'contains(B,\'O\')');
        }), ctx.Z.load({filter: ['B', 'contains', 'O']}).done(function(request) {
          assert.equal(request[0].data.$filter, 'contains(tolower(B),\'o\')');
        })];
        $.when.apply($, promises).fail(function() {
          assert.ok(false, MUST_NOT_REACH_MESSAGE);
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","data/odata/utils","data/odata/store","data/odata/context","core/guid","core/config","../../helpers/data.errorHandlingHelper.js","../../helpers/ajaxMock.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("data/odata/utils"), require("data/odata/store"), require("data/odata/context"), require("core/guid"), require("core/config"), require("../../helpers/data.errorHandlingHelper.js"), require("../../helpers/ajaxMock.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=odataStore.tests.js.map