...
(["testing/tests/DevExpress.data/storeArray.tests.js"], ["jquery","data/array_store","core/config","../../helpers/data.errorHandlingHelper.js"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.data/storeArray.tests.js", ["jquery", "data/array_store", "core/config", "../../helpers/data.errorHandlingHelper.js"], function($__export) {
  "use strict";
  var $,
      ArrayStore,
      config,
  ...
  return {
    ...
    execute: function() {
      ...
      QUnit.test('totalCount', function(assert) {
        var done = assert.async();
        var data = [{a: 2}, {a: 3}, {a: 4}, {a: 1}, {a: 4}, {a: 2}];
        new ArrayStore(data).totalCount({
          filter: ['a', '<>', 1],
          group: 'a',
          pageSize: 2
        }).done(function(r) {
      ...
      QUnit.test('sorting, short syntax', function(assert) {
        var done = assert.async();
        new ArrayStore([{name: 'Z'}, {name: 'A'}]).load({sort: 'name'}).done(function(data) {
      ...
      QUnit.test('sorting, using sortingMethod', function(assert) {
        ...
        new ArrayStore([{name: 'A'}, {name: 'B'}, {name: 'C'}]).load({sort: {
            getter: 'name',
            compare: function(value1, value2) {
              return namesByOrder.indexOf(value1) - namesByOrder.indexOf(value2);
            }
          }}).done(function(data) {
          assert.deepEqual(data.map(function(item) {
            return item.name;
          }), namesByOrder);
          done();
        });
      ...
      QUnit.test('filtering', function(assert) {
        var done = assert.async();
        new ArrayStore({data: [{a: 1}, {a: 2}]}).load({filter: ['a', '>', 1]}).done(function(data) {
      ...
      QUnit.test('grouping and no sorting', function(assert) {
        ...
        new ArrayStore([{
          ...
          name: 'Ann',
          year: 2011
        }]).load({group: [{selector: function(i) {
              return i.name.charAt(0);
            }}, {
            field: 'year',
            desc: true
      ...
      QUnit.test('selector', function(assert) {
        ...
        new ArrayStore([{
          ...
          b: 2,
          c: 3
        }]).load({select: ['a', 'b']}).done(function(data) {
          assert.deepEqual(data, [{
            a: 1,
            b: 2
          }]);
      ...
      QUnit.test('sort/group by function (regression)', function(assert) {
        var done = assert.async();
        new ArrayStore([{value: 'z'}, {value: 'a'}]).load({
          sort: function(i) {
            return i.value;
          },
          group: function(i) {
      ...
      QUnit.test('byKey called with unknown id', function(assert) {
        assert.expect(2);
        var done = assert.async();
        var store = new ArrayStore({key: 'k'});
        store.byKey(1).done(function() {
          assert.ok(false);
        }).fail(function(error) {
      ...
      QUnit.test('Big data grouping', function(assert) {
        ...
        arrayStore.load({group: [{selector: 'Description'}]}).done(function() {
      ...
      QUnit.test('insert with autogenerated key and with useLegacyStoreResult', function(assert) {
        ...
        store.insert({a: 1}).done(function(values, key) {
          assert.equal((typeof key === 'undefined' ? 'undefined' : $traceurRuntime.typeof(key)), 'string');
      ...
      QUnit.test('insert with autogenerated compound key throws', function(assert) {
        var store = new ArrayStore({key: ['key1', 'key2']});
      ...
      QUnit.test('modification operations without key use instance as key', function(assert) {
        var done = assert.async();
        var store = new ArrayStore();
        assert.equal($traceurRuntime.typeof(store.key()), 'undefined');
        var insertValues = {a: 1};
        var insertedObj;
        store.insert(insertValues).done(function(data, key) {
          insertedObj = store._array[0];
          assert.strictEqual((typeof insertedObj === 'undefined' ? 'undefined' : $traceurRuntime.typeof(insertedObj)), 'object');
      ...
      QUnit.test('updating key value throws an error (simple key)', function(assert) {
        ...
        store.update(1, {id: 1}).fail(function() {
          assert.ok(false);
        }).done(function() {
          assert.ok(true);
        });
        store.update(1, {id: 2}).fail(function() {
          assert.ok(true);
        }).done(function() {
          assert.ok(false);
        });
        store.update(1, {id: null}).fail(function() {
          assert.ok(true);
        }).done(function() {
          assert.ok(false);
        });
      ...
      QUnit.test('error in during query evaluation', function(assert) {
        ...
        helper.run(function() {
          return store.load({select: function() {
              throw Error('test');
            }});
        }, done, assert);
      });
      ...
      QUnit.test('forced error for insert', function(assert) {
        ...
        var store = $.extend(new ArrayStore({errorHandler: helper.optionalHandler}), {_insertImpl: function() {
      ...
      QUnit.test('forced error for update', function(assert) {
        ...
        var store = $.extend(new ArrayStore({errorHandler: helper.optionalHandler}), {_updateImpl: function() {
            return $.Deferred().reject(Error('FORCED'));
          }});
        helper.run(function() {
      ...
      QUnit.test('forced error for remove', function(assert) {
        ...
        var store = $.extend(new ArrayStore({errorHandler: helper.optionalHandler}), {_removeImpl: function() {
            return $.Deferred().reject(Error('FORCED'));
          }});
        helper.run(function() {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","data/array_store","core/config","../../helpers/data.errorHandlingHelper.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("data/array_store"), require("core/config"), require("../../helpers/data.errorHandlingHelper.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=storeArray.tests.js.map