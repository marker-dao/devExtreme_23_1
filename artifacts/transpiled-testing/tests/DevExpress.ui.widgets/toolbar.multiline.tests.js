...
(["testing/tests/DevExpress.ui.widgets/toolbar.multiline.tests.js"], ["jquery","ui/toolbar","ui/button","ui/select_box","core/utils/extend","core/utils/view_port","ui/toolbar/strategy/toolbar.multiline","ui/toolbar/strategy/toolbar.singleline","generic_light.css!"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets/toolbar.multiline.tests.js", ["jquery", "ui/toolbar", "ui/button", "ui/select_box", "core/utils/extend", "core/utils/view_port", "ui/toolbar/strategy/toolbar.multiline", "ui/toolbar/strategy/toolbar.singleline", "generic_light.css!"], function($__export) {
  "use strict";
  var $,
      extend,
      viewPort,
  ...
  return {
    setters: [function($__m) {
      ...
      MultiLineStrategy = $__m.MultiLineStrategy;
    }, function($__m) {
      SingleLineStrategy = $__m.SingleLineStrategy;
    }, function($__m) {}],
    execute: function() {
      TOOLBAR_ITEMS_CLASS = 'dx-toolbar-item';
      TOOLBAR_ITEMS_CONTAINER_CLASS = 'dx-toolbar-items-container';
      TOOLBAR_ITEM_HEIGHT = 36;
      BUTTON_TEXT_CLASS = 'dx-button-text';
      ToolbarTestWrapper = function() {
        function ToolbarTestWrapper(options) {
          viewPort($('#container'));
          this._$toolbar = $('#toolbar').dxToolbar(extend({items: [{
              location: 'before',
              widget: 'dxButton',
              options: {type: 'back'}
            }, {
        ...
        return ($traceurRuntime.createClass)(ToolbarTestWrapper, {
          set width(value) {
          ...
          checkToolBarHeight: function(expected) {
            QUnit.assert.roughEqual(this._$toolbar.height(), expected, 2.1, 'toolbar height');
          },
          checkItemsInLine: function(itemsIndexes, lineIndex) {
            var $__5 = this;
            var itemsElements = this._$toolbar.find('.dx-toolbar-item').toArray();
            var widths = itemsIndexes.map(function(itemIndex) {
              return $(itemsElements[itemIndex]).outerWidth();
            });
            var getAssertMessage = function(index, offsetName) {
              return (lineIndex + " line, " + index + " item, " + offsetName + " offset");
            };
            itemsIndexes.forEach(function(itemIndex, i) {
              var item = itemsElements[itemIndex];
              var expectedLeftOffset = widths.slice(0, i).reduce(function(result, value) {
                return result + value;
              }, 0);
              QUnit.assert.roughEqual($(item).offset().top - $__5._$toolbar.offset().top, TOOLBAR_ITEM_HEIGHT * lineIndex, 2, getAssertMessage(itemIndex, 'top'));
              QUnit.assert.roughEqual($(item).offset().left - $__5._$toolbar.offset().left, expectedLeftOffset, 2.1, getAssertMessage(itemIndex, 'left'));
            });
          },
          checkMultilineOption: function(expected) {
            QUnit.assert.equal(this._toolbar.option('multiline'), expected, 'multiline option');
          },
          checkMultilineCssClass: function(expected) {
            QUnit.assert.equal(this._$toolbar.hasClass('dx-toolbar-multiline'), expected, 'multiline CSS class');
          },
          checkRenderLayoutStrategy: function(isMultiline) {
            if (isMultiline) {
      ...
      QUnit.testStart(function() {
        var markup = '<div id="container" class="dx-viewport"><div id=\'toolbar\'></div></div>';
        $('#qunit-fixture').html(markup);
      });
      QUnit.module('render', function() {
        QUnit.test('multiline is disabled by default', function() {
          var testWrapper = new ToolbarTestWrapper({compactMode: true});
        ...
        QUnit.test('1 line: [0, 1, 2, 3, 4, 5, 6]', function() {
          var testWrapper = new ToolbarTestWrapper({multiline: true});
        ...
        ['always', 'inMenu'].forEach(function(showText) {
          QUnit.test(("button text visibility when showText is " + showText), function() {
            var testWrapper = new ToolbarTestWrapper({
              items: [{
                location: 'before',
                widget: 'dxButton',
                showText: showText,
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","ui/toolbar","ui/button","ui/select_box","core/utils/extend","core/utils/view_port","ui/toolbar/strategy/toolbar.multiline","ui/toolbar/strategy/toolbar.singleline","generic_light.css!"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("ui/toolbar"), require("ui/button"), require("ui/select_box"), require("core/utils/extend"), require("core/utils/view_port"), require("ui/toolbar/strategy/toolbar.multiline"), require("ui/toolbar/strategy/toolbar.singleline"), require("generic_light.css!"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=toolbar.multiline.tests.js.map