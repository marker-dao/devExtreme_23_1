System.register(["jquery", "animation/frame", "core/devices", "ui/scroll_view/ui.scrollbar", "../../../helpers/pointerMock.js", "ui/scroll_view/ui.scrollable", "renovation/ui/scroll_view/utils/get_translate_values", "renovation/ui/scroll_view/utils/get_element_style", "generic_light.css!", "renovation/ui/scroll_view/common/consts.js", "./scrollable.constants.js"], function (_export, _context) {
  "use strict";

  var $, animationFrame, devices, Scrollbar, pointerMock, Scrollable, getTranslateValues, getElementOverflowY, getElementOverflowX, DIRECTION_HORIZONTAL, DIRECTION_VERTICAL, DIRECTION_BOTH, SCROLLABLE_CONTAINER_CLASS, SCROLLABLE_CONTENT_CLASS, SCROLLABLE_SCROLLBAR_CLASS, SCROLLABLE_SCROLL_CLASS, SCROLLABLE_SCROLL_CONTENT_CLASS, SCROLLBAR_VERTICAL_CLASS, SCROLLBAR_HORIZONTAL_CLASS, SCROLLABLE_SCROLLBAR_ACTIVE_CLASS, SCROLLABLE_SCROLLBARS_HIDDEN, SCROLLABLE_CLASS, RESIZE_WAIT_TIMEOUT, SCROLLBAR_MIN_HEIGHT, moduleConfig, isRenovatedScrollable;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_animationFrame) {
      animationFrame = _animationFrame.default;
    }, function (_coreDevices) {
      devices = _coreDevices.default;
    }, function (_uiScroll_viewUiScrollbar) {
      Scrollbar = _uiScroll_viewUiScrollbar.default;
    }, function (_helpersPointerMockJs) {
      pointerMock = _helpersPointerMockJs.default;
    }, function (_uiScroll_viewUiScrollable) {
      Scrollable = _uiScroll_viewUiScrollable.default;
    }, function (_renovationUiScroll_viewUtilsGet_translate_values) {
      getTranslateValues = _renovationUiScroll_viewUtilsGet_translate_values.getTranslateValues;
    }, function (_renovationUiScroll_viewUtilsGet_element_style) {
      getElementOverflowY = _renovationUiScroll_viewUtilsGet_element_style.getElementOverflowY;
      getElementOverflowX = _renovationUiScroll_viewUtilsGet_element_style.getElementOverflowX;
    }, function (_generic_lightCss) {}, function (_renovationUiScroll_viewCommonConstsJs) {
      DIRECTION_HORIZONTAL = _renovationUiScroll_viewCommonConstsJs.DIRECTION_HORIZONTAL;
      DIRECTION_VERTICAL = _renovationUiScroll_viewCommonConstsJs.DIRECTION_VERTICAL;
      DIRECTION_BOTH = _renovationUiScroll_viewCommonConstsJs.DIRECTION_BOTH;
    }, function (_scrollableConstantsJs) {
      SCROLLABLE_CONTAINER_CLASS = _scrollableConstantsJs.SCROLLABLE_CONTAINER_CLASS;
      SCROLLABLE_CONTENT_CLASS = _scrollableConstantsJs.SCROLLABLE_CONTENT_CLASS;
      SCROLLABLE_SCROLLBAR_CLASS = _scrollableConstantsJs.SCROLLABLE_SCROLLBAR_CLASS;
      SCROLLABLE_SCROLL_CLASS = _scrollableConstantsJs.SCROLLABLE_SCROLL_CLASS;
      SCROLLABLE_SCROLL_CONTENT_CLASS = _scrollableConstantsJs.SCROLLABLE_SCROLL_CONTENT_CLASS;
      SCROLLBAR_VERTICAL_CLASS = _scrollableConstantsJs.SCROLLBAR_VERTICAL_CLASS;
      SCROLLBAR_HORIZONTAL_CLASS = _scrollableConstantsJs.SCROLLBAR_HORIZONTAL_CLASS;
      SCROLLABLE_SCROLLBAR_ACTIVE_CLASS = _scrollableConstantsJs.SCROLLABLE_SCROLLBAR_ACTIVE_CLASS;
      SCROLLABLE_SCROLLBARS_HIDDEN = _scrollableConstantsJs.SCROLLABLE_SCROLLBARS_HIDDEN;
      SCROLLABLE_CLASS = _scrollableConstantsJs.SCROLLABLE_CLASS;
      RESIZE_WAIT_TIMEOUT = _scrollableConstantsJs.RESIZE_WAIT_TIMEOUT;
    }],
    execute: function () {
      SCROLLBAR_MIN_HEIGHT = 15;
      moduleConfig = {
        beforeEach: function () {
          const markup = `
      ...
      QUnit.test('scrollbar in scaled container has correct position after update', function (assert) {
        ...
        $content.height(contentHeight);
        $scrollable.dxScrollable('instance').update();
        pointerMock($scrollable.find('.' + SCROLLABLE_CONTENT_CLASS)).start().down().move(0, distance).up();
      ...
      QUnit.test('moving scrollable moves scrollbar', function (assert) {
        ...
        $content.height(contentHeight);
        $scrollable.dxScrollable('instance').update();
        pointerMock($scrollable.find('.' + SCROLLABLE_CONTENT_CLASS)).start().down().move(0, distance).move(0, distance).up();
      });
      ...
      QUnit.test('scroll updated before start', function (assert) {
        ...
        setTimeout(() => {
          pointerMock($scrollable.find('.' + SCROLLABLE_CONTENT_CLASS)).start().down().move(0, -10);
      ...
      QUnit.test('scrollbar jumps to the mouse click point on scrollbar area', function (assert) {
        ...
        const $scrollable = $('#scrollable').height(containerSize).wrapInner('<div>');
        ...
        const pointer = pointerMock($scrollbarContainer).start().move(0, moveDistance).down();
      ...
      QUnit.test('scrollbar add active class when click on scrollbar area', function (assert) {
        ...
        const pointer = pointerMock($scrollbar).start().down();
      ...
      QUnit.test('scrollbar should be hidden when container size is almost similar to content size when zooming', function (assert) {
        ...
        sinon.stub(scrollableContainerElement, 'getBoundingClientRect').returns({
          [dimension]: fakeContainerSizeWhenZoomIs125
        });
        sinon.stub(scrollableContentElement, 'getBoundingClientRect').returns({
          [dimension]: fakeContentSizeWhenZoomIs125
        });
      ...
      QUnit.module('scrollbar visibility', {
        ...
        const checkScrollbarStyles = (assert, scrollbarClass, $scrollable, useNative, showScrollbar, shouldRenderScrollbar) => {
          const $scrollbar = useNative ? $scrollable.closest(`.${SCROLLABLE_CLASS}`).find(`> .${scrollbarClass}`) : $scrollable.find(`.${SCROLLABLE_CONTAINER_CLASS}`).first().find(`> .${scrollbarClass}`);
        ...
        const checkStyles = (assert, $scrollable, {
          useNative,
          direction,
          showScrollbar,
          ...
          configs.forEach(innerScrollableOptions => {
            QUnit.test(`check scrollbar visibility: outerScrollable: ${JSON.stringify(outerScrollableOptions)}, innerScrollable: ${JSON.stringify(innerScrollableOptions)}`, function (assert) {
              ...
              showScrollbarValues.forEach(outerShowScrollbarValue => {
                this.$outerScrollable.dxScrollable({
                  showScrollbar: outerShowScrollbarValue,
                  ...outerScrollableOptions
                });
                showScrollbarValues.forEach(innerShowScrollbarValue => {
                  this.$innerScrollable.dxScrollable({
                    showScrollbar: innerShowScrollbarValue,
                    ...innerScrollableOptions
                  });
                  checkStyles(assert, this.$outerScrollable, {
                    showScrollbar: outerShowScrollbarValue,
                    ...outerScrollableOptions
                  });
                  checkStyles(assert, this.$innerScrollable, {
                    showScrollbar: innerShowScrollbarValue,
                    ...innerScrollableOptions
                  });
                });
