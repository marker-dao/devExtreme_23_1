System.register(["jquery", "ui/file_manager", "animation/fx", "events/pointer", "localization", "localization/message", "../../../helpers/fileManagerHelpers.js", "../../../helpers/fileManager/events.js", "core/utils/size"], function (_export, _context) {
  "use strict";

  var $, fx, pointerEvents, localization, messageLocalization, FileManagerWrapper, createTestFileSystem, isDesktopDevice, createHugeFileSystem, triggerCellClick, implementationsMap, test, moduleConfig, getSelectedItemNames, prepareParentDirectoryTesting;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_uiFile_manager) {}, function (_animationFx) {
      fx = _animationFx.default;
    }, function (_eventsPointer) {
      pointerEvents = _eventsPointer.default;
    }, function (_localization) {
      localization = _localization.default;
    }, function (_localizationMessage) {
      messageLocalization = _localizationMessage.default;
      ...
      isDesktopDevice = _helpersFileManagerHelpersJs.isDesktopDevice;
      createHugeFileSystem = _helpersFileManagerHelpersJs.createHugeFileSystem;
    }, function (_helpersFileManagerEventsJs) {
      triggerCellClick = _helpersFileManagerEventsJs.triggerCellClick;
    ...
    execute: function () {
      ...
      getSelectedItemNames = fileManager => fileManager.getSelectedItems().map(item => item.name);
      prepareParentDirectoryTesting = (context, singleSelection) => {
      ...
      QUnit.module('Details View', moduleConfig, () => {
        ...
        test('Using custom formats of JSON files', function (assert) {
          $('#fileManager').dxFileManager('instance').option('fileSystemProvider', {
            data: [{
              title: 'Folder',
              noFolder: true,
              myDate: '2/2/2000'
        ...
        test('Parent directory item is not selectable in multi-selection mode via clicks', function (assert) {
          ...
          this.wrapper.getRowNameCellInDetailsView(2).trigger('dxpointerdown').trigger('dxclick');
          ...
          this.wrapper.getRowNameCellInDetailsView(1).trigger('dxpointerdown').trigger('dxclick');
        ...
        test('Customize columns with customizeDetailColumns callback', function (assert) {
          ...
          fileManager.option({
            ...
            customizeDetailColumns: function (columns) {
              const fileSizeColumn = columns.filter(function (c) {
                return c.dataField === 'size';
              })[0];
              columns.splice(columns.indexOf(fileSizeColumn), 1);
              const modifiedColumn = columns.filter(function (c) {
                return c.dataField === 'dateModified';
              })[0];
          ...
          this.wrapper.getInstance().option({
            ...
            itemView: {
              ...
              details: {
                columns: [{
                  dataField: 'thumbnail',
                  caption: 'thumbnailsColumnCaption'
                }, 'name']
              }
            },
            width: '500px'
        ...
        test('sorting by file size is correct (T962735)', function (assert) {
          ...
          assert.equal(this.wrapper.getDetailsItemName(4), 'Folder 1', 'sorted ascending');
          columnHeader.trigger($.Event('dxclick', {
            ctrlKey: true
          }));
          this.clock.tick(400);
          assert.equal(columnHeader.attr('aria-sort'), 'none', 'sorting default');
