System.register(["ui/file_manager", "file_management/object_provider", "file_management/file_system_item", "file_management/error_codes", "exporter/file_saver", "../../../helpers/fileManagerHelpers.js"], function (_export, _context) {
  "use strict";

  var ObjectFileSystemProvider, FileSystemItem, ErrorCode, fileSaver, createUploaderFiles, createUploadInfo, test, moduleConfig;
  return {
    setters: [function (_uiFile_manager) {}, function (_file_managementObject_provider) {
      ObjectFileSystemProvider = _file_managementObject_provider.default;
    }, function (_file_managementFile_system_item) {
    ...
    }, function (_exporterFile_saver) {
      fileSaver = _exporterFile_saver.fileSaver;
    ...
    execute: function () {
      ...
      QUnit.module('Array File Provider', moduleConfig, () => {
        test('get directory file items', function (assert) {
          ...
          this.provider.getItems(this.rootItem).done(items => {
            done1();
          ...
          }).then(() => this.provider.getItems(dir1)).done(items => {
            done2();
          ...
          }).then(() => this.provider.getItems(dir2)).done(items => {
            done3();
        ...
        test('move directory', function (assert) {
          ...
          this.provider.getItems(this.rootItem).then(result => {
            done();
            items = result;
        ...
        test('create new folder with existing name', function (assert) {
          ...
          this.provider.createDirectory(root, 'F1').then(result => {
            done();
            assert.strictEqual(result, undefined, 'resolved with no result');
            return this.provider.getItems(this.rootItem);
          }).then(dirs => {
        ...
        test('create directory via arguments without data items', function (assert) {
          ...
          const done = assert.async(1);
          this.provider.createDirectory(dir, 'new F.2.2 test dir').done(() => {
            done();
        ...
        test('throw error on creating new directory in unexisting directory', function (assert) {
          const done = assert.async(2);
          this.provider.getItems(this.rootItem).then(([f1Dir]) => {
            done();
        ...
        test('rename directory via arguments without data items', function (assert) {
          ...
          const done = assert.async(1);
          this.provider.renameItem(dir, 'new F2 test name').done(() => {
            done();
        ...
        test('upload file', function (assert) {
          ...
          this.provider.getItems(this.rootItem).then(items => {
            done();
        ...
        test('upload file via arguments without data items', function (assert) {
          ...
          const file = createUploaderFiles(1)[0];
          let uploadInfo = createUploadInfo(file);
          this.provider.uploadFileChunk(file, uploadInfo, dir).then(() => {
            done();
            uploadInfo = createUploadInfo(file, 1, uploadInfo.customData);
            return this.provider.uploadFileChunk(file, uploadInfo, dir);
          }).then(() => {
        ...
        test('download single file', function (assert) {
          ...
          this.provider.getItems(dir).then(([file1]) => {
            done();
            file = file1;
        ...
        test('download multiple files', function (assert) {
          ...
          const dir = new FileSystemItem('F1/F1.2', true);
          this.provider.getItems(dir).then(files => {
            done();
