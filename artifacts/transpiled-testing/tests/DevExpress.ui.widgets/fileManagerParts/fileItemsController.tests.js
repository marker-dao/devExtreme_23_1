System.register(["ui/file_manager/file_items_controller", "file_management/error_codes", "../../../helpers/fileManagerHelpers.js", "core/utils/type", "core/utils/extend"], function (_export, _context) {
  "use strict";

  var FileItemsController, ErrorCode, createUploaderFiles, createUploadInfo, stubFileReader, createEditingEvents, isString, extend, test, moduleConfig, stubFileReaderInProvider;
  return {
    setters: [function (_uiFile_managerFile_items_controller) {
      ...
      createUploadInfo = _helpersFileManagerHelpersJs.createUploadInfo;
      ...
      isString = _coreUtilsType.isString;
    ...
    execute: function () {
      ...
      stubFileReaderInProvider = controller => stubFileReader(controller._fileProvider);
      QUnit.module('FileItemsController tests', moduleConfig, () => {
        test('raise current directory change event', function (assert) {
          ...
          controller.getDirectories(rootDir).done(directories => {
        ...
        test('get directory contents', function (assert) {
          ...
          this.controller.getDirectoryContents(selectedDir).done(items => {
          ...
          this.controller.getDirectories(selectedDir).done(directories => {
        ...
        test('create new directory', function (assert) {
          ...
          this.controller.getDirectories(selectedDir).then(() => {
        ...
        test('rename file item', function (assert) {
          ...
          this.controller.getDirectories(currentDir).then(directories => {
        ...
        test('move file items from parent folder to child folder', function (assert) {
          ...
          this.controller.getDirectories(rootDir).then(directories => {
        ...
        test('move file items between sibling folders (T1132584)', function (assert) {
          const done = assert.async();
          const rootDir = this.controller.getCurrentDirectory();
          this.controller.getDirectories(rootDir).then(dirs => {
            const f1Dir = dirs[0];
            const f2Dir = dirs[1];
            let targetItems = null;
            let destinationDir = null;
            const onMoving = this.editingEvents.onItemMoving;
            const onMoved = this.editingEvents.onItemMoved;
            this.controller.getDirectories(f1Dir).then(directories => {
              targetItems = [directories[0]];
        ...
        test('delete file items', function (assert) {
          ...
          this.controller.getDirectoryContents(currentDir).then(itemInfos => {
        ...
        test('get current path', function (assert) {
          ...
          const rootDir = this.controller.getCurrentDirectory();
          controller.getDirectoryContents(rootDir).then(directories => controller.getDirectories(directories[0])).then(directories => {
        ...
        test('refresh data and restore state', function (assert) {
          ...
          controller.getDirectoryContents(rootDir).then(directories => {
        ...
        test('set current path', function (assert) {
          ...
          this.controller.setCurrentPath('F2/F2.1').then(() => {
        ...
        test('upload fails when max file size exceeded', function (assert) {
          ...
          const done2 = assert.async();
          const currentDir = this.controller.getCurrentDirectory();
          this.controller.getDirectories(currentDir).then(() => this.controller.uploadFileChunk(files[0], createUploadInfo(files[0]), currentDir.fileItem)).then(() => {
            done1();
            assert.throws(() => this.controller.uploadFileChunk(files[1], createUploadInfo(files[1]), currentDir.fileItem), error => {
              done2();
        ...
        test('files can have extensions of any letter case', function (assert) {
          const extendedData = this.data.concat({
            name: 'File2.JPEG'
          }, {
            name: 'File3.Doc'
          });
