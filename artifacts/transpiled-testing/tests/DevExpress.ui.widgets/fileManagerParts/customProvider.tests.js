System.register(["core/utils/deferred", "file_management/file_system_item", "file_management/custom_provider", "../../../helpers/fileManagerHelpers.js"], function (_export, _context) {
  "use strict";

  var when, FileSystemItem, CustomFileSystemProvider, createSampleFileItems, test, filesPathInfo, itemData, fileSystemItems, moduleConfig;
  return {
    ...
    execute: function () {
      ...
      QUnit.module('Custom file provider', moduleConfig, () => {
        test('get directory file items', function (assert) {
          ...
          this.provider.getItems(filesDir).done(items => {
        ...
        test('create directory', function (assert) {
          ...
          this.provider.createDirectory(parentDir, 'Test 1').done(result => {
            done();
            assert.strictEqual(result, undefined, 'resolved with no result');
        ...
        test('delete items', function (assert) {
          const done = assert.async();
          const items = [new FileSystemItem(filesPathInfo, 'Documents'), new FileSystemItem(filesPathInfo, 'Article.txt')];
          ...
          when.apply(null, deferreds).done((res1, res2) => {
            done();
        ...
        test('upload file chunk', function (assert) {
          ...
          this.provider.uploadFileChunk(fileData, chunksInfo, destinationDir).done(result => {
            done();
            assert.strictEqual(result, undefined, 'resolved with no result');
        ...
        test('abort file upload', function (assert) {
          ...
          const destinationDir = new FileSystemItem(filesPathInfo, 'Documents');
          this.provider.abortFileUpload(fileData, chunksInfo, destinationDir).done(result => {
            done();
            assert.strictEqual(result, undefined, 'resolved with no result');
