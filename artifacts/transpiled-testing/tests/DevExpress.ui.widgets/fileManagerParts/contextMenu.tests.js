System.register(["jquery", "ui/file_manager", "animation/fx", "events/pointer", "../../../helpers/fileManagerHelpers.js"], function (_export, _context) {
  "use strict";

  var $, fx, pointerEvents, Consts, FileManagerWrapper, createTestFileSystem, isDesktopDevice, test, moduleConfig;
  return {
    ...
    execute: function () {
      ...
      QUnit.module('Cutomize context menu', moduleConfig, () => {
        ...
        test('nested items set and use', function (assert) {
          ...
          this.clock.tick(400);
          this.wrapper.getDialogTextInput().val('New name.txt').trigger('change');
        ...
        test('context menu items can be updated for visible menu after action button click', function (assert) {
          ...
          this.fileManager.option('contextMenu.items', [{
            text: 'Item 1'
          }, {
            text: 'Item 2'
          }, {
        ...
        test('Hidden context menu can be updated with change of the permissions option (T1051605)', function (assert) {
          ...
          this.wrapper.getContextMenuItem('Refresh').trigger('dxclick');
          this.wrapper.getInstance().option('permissions', {
            rename: false
          });
          this.clock.tick(400);
          this.wrapper.getRowActionButtonInDetailsView(2).trigger('dxclick');
