System.register(["jquery", "ui/file_manager", "animation/fx", "events/pointer", "../../../helpers/fileManagerHelpers.js", "../../../helpers/grid/keyboardNavigationHelper.js", "core/utils/size"], function (_export, _context) {
  "use strict";

  var $, fx, pointerEvents, FileManagerWrapper, FileManagerProgressPanelWrapper, createTestFileSystem, createHugeFileSystem, Consts, CLICK_EVENT, implementationsMap, test, moduleConfig;
  return {
    ...
    execute: function () {
      ...
      QUnit.module('Scroll', moduleConfig, () => {
        ...
        test('All views - must keep scroll position for sync focused item - scroll position at the end', function (assert) {
          ...
          this.clock.tick(400);
          this.wrapper.getThumbnailsViewPort().trigger($.Event('keydown', {
            key: 'End'
          }));
          this.clock.tick(400);
        ...
        test('Details view must keep scroll position when selection is changed', function (assert) {
          ...
          this.wrapper.getDetailsViewScrollableContainer().trigger('scroll');
          this.clock.tick(400);
          this.wrapper.getDetailsViewScrollable().dxScrollable('instance').scrollTo({
            top: 500
          });
        ...
        test('NavPane - must keep scroll position after refresh', function (assert) {
          ...
          this.wrapper.getDirsPanel().find('.dx-scrollable').dxScrollable('instance').scrollTo({
            top: scrollPosition
          });
          this.clock.tick(400);
          this.fileManager.refresh();
