System.register(["jquery", "ui/file_manager", "file_management/error.js", "file_management/custom_provider", "file_management/object_provider.js", "ui/file_manager/file_items_controller", "ui/file_manager/ui.file_manager.breadcrumbs", "animation/fx", "../../../helpers/fileManagerHelpers.js", "../../../helpers/fileManager/file_provider.slow.js", "core/utils/size", "core/utils/deferred", "core/utils/extend"], function (_export, _context) {
  "use strict";

  var $, FileSystemError, CustomFileSystemProvider, ObjectFileSystemProvider, FileItemsController, FileManagerBreadcrumbs, fx, FileManagerWrapper, FileManagerBreadcrumbsWrapper, FileManagerProgressPanelWrapper, createTestFileSystem, SlowFileProvider, implementationsMap, Deferred, extend, test, moduleConfig, moduleConfig_T1085224, createFileManager_T1085224, createBreadcrumbs, createFileProviderWithIncorrectName;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_uiFile_manager) {}, function (_file_managementErrorJs) {
      FileSystemError = _file_managementErrorJs.default;
    }, function (_file_managementCustom_provider) {
      CustomFileSystemProvider = _file_managementCustom_provider.default;
    }, function (_file_managementObject_providerJs) {
      ObjectFileSystemProvider = _file_managementObject_providerJs.default;
    ...
    }, function (_uiFile_managerUiFile_managerBreadcrumbs) {
      FileManagerBreadcrumbs = _uiFile_managerUiFile_managerBreadcrumbs.default;
    }, function (_animationFx) {
      fx = _animationFx.default;
    }, function (_helpersFileManagerHelpersJs) {
      FileManagerWrapper = _helpersFileManagerHelpersJs.FileManagerWrapper;
      FileManagerBreadcrumbsWrapper = _helpersFileManagerHelpersJs.FileManagerBreadcrumbsWrapper;
      FileManagerProgressPanelWrapper = _helpersFileManagerHelpersJs.FileManagerProgressPanelWrapper;
      createTestFileSystem = _helpersFileManagerHelpersJs.createTestFileSystem;
    }, function (_helpersFileManagerFile_providerSlowJs) {
      SlowFileProvider = _helpersFileManagerFile_providerSlowJs.default;
    }, function (_coreUtilsSize) {
      implementationsMap = _coreUtilsSize.implementationsMap;
    }, function (_coreUtilsDeferred) {
      Deferred = _coreUtilsDeferred.Deferred;
    }, function (_coreUtilsExtend) {
      extend = _coreUtilsExtend.extend;
    }],
    execute: function () {
      ({
        test
      ...
      createFileManager_T1085224 = (context, useThumbnailViewMode, extOptions) => {
        const provider = new CustomFileSystemProvider({
          getItems: () => {
            throw new FileSystemError(42, null, 'Custom text');
          }
        });
      ...
      createBreadcrumbs = (context, controller) => {
      ...
      createFileProviderWithIncorrectName = (incorrectName, isOnlyNewItem) => {
        let fileProvider = [];
        const newItem = {
          name: incorrectName,
          isDirectory: true,
      ...
      QUnit.module('Navigation operations', moduleConfig, () => {
        ...
        test('getSelectedItems method', function (assert) {
          const testCases = [{
            mode: 'thumbnails',
            wrapperMethod: 'findThumbnailsItem',
            eventName: 'dxclick'
          }, {
            mode: 'details',
            wrapperMethod: 'findDetailsItem',
            eventName: 'dxclick'
          }];
          testCases.forEach(({
            mode,
        ...
        test('slashes in directory name must be processed correctly', function (assert) {
          ...
          const controller = new FileItemsController({
            ...
            onSelectedDirectoryChanged: () => {
              this.breadcrumbs.setCurrentDirectory(controller.getCurrentDirectory());
            }
          });
          createBreadcrumbs(this, controller);
          const rootDirectory = controller.getCurrentDirectory();
          controller.getDirectoryContents(rootDirectory).then(items => {
          ...
          this.clock.tick(400);
          controller.getDirectoryContents(controller.getCurrentDirectory()).then(items => {
        ...
        test('whitespace as directory name must be processed correctly', function (assert) {
          ...
          createBreadcrumbs(this, controller);
          const rootDirectory = controller.getCurrentDirectory();
          controller.getDirectoryContents(rootDirectory).then(items => controller.getDirectoryContents(items[6])).then(items => controller.getDirectoryContents(items[1])).then(items => {
            controller.setCurrentDirectory(items[0]);
        ...
        test('special symbols in \'currentPath\' option must be processed correctly', function (assert) {
          ...
          let counter = 0;
          inst.option('onOptionChanged', e => {
            e.name === 'currentPath' && counter++;
          });
        ...
        test('\'Back\' directory must have attributes of the represented directory', function (assert) {
          ...
          this.clock.tick(400);
          fileManager._getItemViewItems().then(fileInfos => {
        ...
        test('Navigation to forbidden folder rises an error', function (assert) {
          this.fileManager.option('fileSystemProvider', new CustomFileSystemProvider({
            getItems: pathInfo => {
              if (pathInfo.path === '') {
                return [{
                  name: 'Folder',
        ...
        test('errorText can be customized on the getItems', function (assert) {
          ...
          this.fileManager.option({
            fileSystemProvider: new CustomFileSystemProvider({
              getItems: () => {
                throw new FileSystemError(0, null, customMessage);
              }
            })
          });
          this.clock.tick(400);
        ...
        test('currentPath option value stays the same when changed along with the fileSystemProvider option with delay (T1045617)', function (assert) {
          ...
          this.fileManager.option({
            fileSystemProvider: new SlowFileProvider({
              operationDelay,
              operationsToDelay: 'r',
              realProviderInstance: new ObjectFileSystemProvider({
                data: [{
                  name: 'Folder 1',
                  isDirectory: true,
                  hasSubDirectories: true,
        ...
        test('notification does not disappear when permissions set dynamically (T1051605)', function (assert) {
          ...
          this.fileManager.option({
            ...
            onCurrentDirectoryChanged: ({
              component
            }) => component.option('permissions', component.option('permissions')),
            onOptionChanged: optionChangedSpy
          });
          this.clock.tick(400);
          let currentPath = this.fileManager.option('currentPath');
      ...
      QUnit.module('initial navigation with error (T1085224)', moduleConfig_T1085224, () => {
        ...
        test('getItems must be invoked only once in case of exception detailsView (provider+path) (T1085224)', function (assert) {
          const {
            getItemsSpy
          } = createFileManager_T1085224(this, false, {
            currentPath: 'somePath'
          });
          assert.strictEqual(getItemsSpy.callCount, 1, 'getItems function must be called once');
        });
