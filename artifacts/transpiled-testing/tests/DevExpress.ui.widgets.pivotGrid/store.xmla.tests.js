...
(["testing/tests/DevExpress.ui.widgets.pivotGrid/store.xmla.tests.js"], ["jquery","core/utils/browser","core/devices","__internal/grids/pivot_grid/module_widget_utils","__internal/grids/pivot_grid/data_source/module_utils","__internal/grids/pivot_grid/xmla_store/module","../../helpers/pivotGridTestSettings.js"], true, function($__System) {
...
define("testing/tests/DevExpress.ui.widgets.pivotGrid/store.xmla.tests.js", ["require", "jquery", "core/utils/browser", "core/devices", "__internal/grids/pivot_grid/module_widget_utils", "__internal/grids/pivot_grid/data_source/module_utils", "__internal/grids/pivot_grid/xmla_store/module", "../../helpers/pivotGridTestSettings.js"], function(require) {
  ...
  const CATEGORIES_HIERARCHY_DATA = [{
    key: '[Product].[Product Categories].[Category].&[4]',
    value: 'Accessories',
    text: 'Accessories',
    index: 1
  }, {
    key: '[Product].[Product Categories].[Category].&[1]',
    value: 'Bikes',
    text: 'Bikes',
    index: 2
  }, {
    key: '[Product].[Product Categories].[Category].&[3]',
    value: 'Clothing',
    text: 'Clothing',
    index: 3
  }];
  ...
  QUnit.module('Adventure Works', testEnvironment, () => {
    ...
    QUnit.test('Adventure Works', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[18484], [15114], [9132], [6852]], [[962], [null], [962], [null]], [[2665], [null], [2665], [null]], [[9002], [6470], [4756], [2717]], [[11753], [9745], [5646], [4340]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    ...
    QUnit.test('Loaded with two level expanded items', function(assert) {
      ...
      this.store.load({
        ...
        rows: [{
          ...
          expanded: true
        }, {dataField: '[Ship Date].[Month of Year]'}, {dataField: '[Ship Date].[Day Name]'}],
        values: [{
          dataField: '[Measures].[Customer Count]',
          caption: 'Count'
        }]
    ...
    QUnit.test('defined only cells', function(assert) {
      ...
      this.store.load({values: [{
        ...
        assert.deepEqual(data.values, [[[18484]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('defined columns and cells', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[29358677.2207], [700759.96], [28318144.6507], [339772.61]]], 'cells');
    ...
    QUnit.test('defined columns and rows', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[80450596.9823], [8065435.3053], [24144429.654], [32202669.4252], [16038062.5978]], [[571297.9278], [20235.3646], [92735.3534], [296532.8766], [161794.3332]], [[66302381.557], [7395348.6266], [19956014.6741], [25551775.0727], [13399243.1836]], [[1777840.8391], [34376.3353], [485587.1546], [871864.1866], [386013.1626]], [[11799076.6584], [615474.9788], [3610092.4719], [5482497.2893], [2091011.9184]]], 'cells');
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('defined columns only', function(assert) {
      const done = assert.async();
      this.store.load({columns: [{dataField: '[Ship Date].[Calendar Year]'}]}).done(function(data) {
        assert.ok(data);
        assert.deepEqual(data.rows, [], 'rows');
        assert.deepEqual(data.columns, CALENDAR_YEAR_DATA, 'columns');
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[80450596.9823], [8065435.3053], [24144429.654], [32202669.4252], [16038062.5978]]], 'cells');
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('defined rows only', function(assert) {
      const done = assert.async();
      this.store.load({rows: [{dataField: '[Ship Date].[Calendar Year]'}]}).done(function(data) {
        assert.ok(data);
        assert.deepEqual(data.rows, CALENDAR_YEAR_DATA);
    ...
    QUnit.test('Load with two measures', function(assert) {
      ...
      this.store.load({
        ...
        values: [{
          ...
          dataField: '[Measures].[Order Count]',
          caption: 'Count'
        }]
      }).done(function(data) {
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[18484, 31455], [15114, 19523], [9132, 18358], [6852, 9871], [null, 2646]], [[962, 1328], [null, 135], [962, 1307], [null, 242], [null, 205]], [[2665, 3680], [null, 356], [2665, 3515], [null, 644], [null, 702]], [[9002, 12027], [6470, 7701], [4756, 6772], [2717, 3816], [null, 1138]], [[11753, 14420], [9745, 11331], [5646, 6764], [4340, 5169], [null, 601]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    ...
    QUnit.test('T677334. Correct parse result with empty member value', function(assert) {
      ...
      sinon.stub(pivotGridUtils, 'sendRequest').callsFake(function() {
        ...
        send.apply(this, arguments).then(function() {
      ...
      }).fail(getFailCallBack(assert)).always(function() {
  ...
  QUnit.module('Hierarchies', testEnvironment, () => {
    ...
    QUnit.test('Hierarchy. Expand second level child', function(assert) {
      ...
      this.store.load({
        ...
        rows: [{
          ...
          dataField: '[Ship Date].[Calendar].[Date]',
          hierarchyName: '[Ship Date].[Calendar]'
        }],
        values: [{
          dataField: '[Measures].[Customer Count]',
    ...
    QUnit.test('Hierarchy. Expand child when opposite axis expanded on several levels', function(assert) {
      ...
      this.store.load({
        ...
        headerName: 'columns',
        rowExpandedPaths: [[CALENDAR_HIERARCHY_YEAR_DATA[2].key], [CALENDAR_HIERARCHY_YEAR_DATA[2].key, '[Ship Date].[Calendar].[Month].&[2003]&[8]']],
        path: [CATEGORIES_HIERARCHY_DATA[1].key]
      }).done(function(data) {
        assert.deepEqual(data.columns, BIKES_SUBCATEGORY_DATA);
        assert.strictEqual(data.rows.length, 4);
    ...
    QUnit.test('Not hierarchy & hierarchy. expanded not hierarchy level', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns[0].children), [{text: 'July 2001'}, {text: 'August 2001'}, {text: 'September 2001'}, {text: 'October 2001'}, {text: 'November 2001'}, {text: 'December 2001'}]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Hierarchy & not hierarchy. Load expanded axis', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{
          ...
          children: [{text: 'Accessories'}, {text: 'Bikes'}, {text: 'Clothing'}],
          text: 'CY 2003'
        }, {
          children: [{text: 'Accessories'}, {text: 'Bikes'}, {text: 'Clothing'}],
          text: 'CY 2004'
    ...
    QUnit.test('Hierarchy & not hierarchy. Expand column & row.', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'CY 2001'}, {
          text: 'CY 2002',
          children: [{text: 'Black'}, {text: 'Red'}, {text: 'Silver'}, {text: 'Yellow'}]
        }, {
          children: [{text: 'Black'}, {text: 'Blue'}, {text: 'Red'}, {text: 'Silver'}, {text: 'Yellow'}],
          text: 'CY 2003'
        }, {text: 'CY 2004'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'Mountain Bikes'}, {text: 'Road Bikes'}, {text: 'Touring Bikes'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0, 'GT row index');
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Hierarchy & not hierarchy. Expand row & column.', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'Black'}, {text: 'Blue'}, {text: 'Multi'}, {text: 'NA'}, {text: 'Red'}, {text: 'Silver'}, {text: 'White'}, {text: 'Yellow'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'Accessories'}, {
          children: [{text: 'Mountain Bikes'}, {text: 'Road Bikes'}, {text: 'Touring Bikes'}],
          text: 'Bikes'
        }, {text: 'Clothing'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0, 'GT row index');
    ...
    QUnit.test('Hierarchy & not hierarchy. expanded hierarchy level', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns[4].children), [{text: 'Bikes'}]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Hierarchy & hierarchy. expanded hierarchy level', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns)[3], {
          children: [{
            children: [{text: 'Bike Racks'}, {text: 'Bike Stands'}, {text: 'Bottles and Cages'}, {text: 'Cleaners'}, {text: 'Fenders'}, {text: 'Helmets'}, {text: 'Hydration Packs'}, {text: 'Tires and Tubes'}],
            text: 'Accessories'
          }, {
            children: [{text: 'Mountain Bikes'}, {text: 'Road Bikes'}, {text: 'Touring Bikes'}],
            text: 'Bikes'
          }, {
            children: [{text: 'Caps'}, {text: 'Gloves'}, {text: 'Jerseys'}, {text: 'Shorts'}, {text: 'Socks'}, {text: 'Vests'}],
            text: 'Clothing'
          }],
          text: 'CY 2004'
        });
  ...
  if (browser.chrome && devices.real().deviceType === 'desktop') {
    QUnit.module('Discover', testEnvironment, () => {
      ...
      QUnit.test('Discover. Incorrect dataSource cube', function(assert) {
        const done = assert.async();
        new XmlaStore($.extend({}, this.dataSource, {cube: 'cube'})).getFields().done(function(data) {
          assert.ok(!data.length);
        }).fail(getFailCallBack(assert)).always(done);
      });
      QUnit.test('Discover. Incorrect dataSource catalog', function(assert) {
        const done = assert.async();
        new XmlaStore($.extend({}, this.dataSource, {catalog: 'catalog'})).getFields().done(function(data) {
  ...
  QUnit.module('Filtering', testEnvironment, () => {
    QUnit.test('FilterValues in row and column fields. Include filter', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'Accessories'}, {text: 'Bikes'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'CY 2002'}, {text: 'CY 2003'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[10054], [6470], [6246]], [[2665], [null], [2665]], [[8564], [6470], [4756]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('FilterValues in row and column fields. Exclude Filter', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'Clothing'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'CY 2004'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[4340], [4340]], [[4340], [4340]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('FilterValues in columns field', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'Accessories'}, {text: 'Bikes'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'CY 2001'}, {text: 'CY 2002'}, {text: 'CY 2003'}, {text: 'CY 2004'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[17719], [15114], [9132]], [[962], [null], [962]], [[2665], [null], [2665]], [[8564], [6470], [4756]], [[11182], [9745], [5646]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('FilterValues in rows field', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'Accessories'}, {text: 'Bikes'}, {text: 'Clothing'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'CY 2003'}, {text: 'CY 2004'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[3908], [2918], [2005], [1272]], [[521], [null], [521], [null]], [[3538], [2918], [1635], [1272]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Filter field. Include type', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(data.columns, CATEGORIES_DATA);
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'CY 2002'}, {text: 'CY 2003'}, {text: 'CY 2004'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[4201], [2918], [2298], [1272]], [[361], [null], [361], [null]], [[521], [null], [521], [null]], [[3538], [2918], [1635], [1272]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Filter field. Exclude type', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[16439], [12860], [8375], [5741]], [[962], [null], [962], [null]], [[2304], [null], [2304], [null]], [[8695], [6470], [4449], [2717]], [[8790], [7220], [4192], [3164]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Filter field. Include and exclude filters', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[1231], [901], [608], [398]], [[92], [null], [92], [null]], [[121], [null], [121], [null]], [[1057], [901], [434], [398]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    ...
    QUnit.test('Filter hierarchyLevel field. Include type', function(assert) {
      ...
      this.store.load({
        ...
        'filters': [{
          ...
          'filterValues': ['H2 CY 2002', ['CY 2003'], ['CY 2004', 'H1 CY 2004'], ['CY 2004', 'H2 CY 2004', 'Q3 CY 2004', 'August 2004'], []]
        }]
      }).done(function(data) {
        assert.deepEqual(removeIndexesAndValue(data.columns), [{text: 'CY 2002'}, {text: 'CY 2003'}, {text: 'CY 2004'}]);
        assert.deepEqual(data.rows, []);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
    ...
    QUnit.test('Filter hierarchyLevel field. Exclude type', function(assert) {
      ...
      this.store.load({
        ...
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.deepEqual(data.values, [[[17366], [15114], [7990]]]);
      }).fail(getFailCallBack(assert)).always(done);
    });
  ...
  QUnit.module('Sorting', testEnvironment, () => {
    QUnit.test('Sorting by value', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'December'}, {text: 'November'}, {text: 'October'}, {text: 'September'}, {text: 'August'}, {text: 'July'}, {text: 'June'}, {text: 'May'}, {text: 'April'}, {text: 'March'}, {text: 'February'}, {text: 'January'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Sorting by display text', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(data.columns, CATEGORIES_DATA.slice().reverse());
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'September'}, {text: 'October'}, {text: 'November'}, {text: 'May'}, {text: 'March'}, {text: 'June'}, {text: 'July'}, {text: 'January'}, {text: 'February'}, {text: 'December'}, {text: 'August'}, {text: 'April'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    ...
    QUnit.test('Sorting by none two dimension on axis and expanded item', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns[1].children), [{text: 'Touring Bikes'}, {text: 'Road Bikes'}, {text: 'Mountain Bikes'}]);
    ...
    QUnit.test('Sorting by Summary field', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.rows[2].children), [{text: 'December'}, {text: 'October'}, {text: 'November'}, {text: 'September'}, {text: 'August'}, {text: 'July'}, {text: 'May'}, {text: 'June'}, {text: 'April'}, {text: 'March'}, {text: 'January'}, {text: 'February'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Sorting by Summary field when expanded items', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(data.rows[2].text, 'CY 2003');
        assert.deepEqual(data.rows[3].text, 'CY 2004');
        assert.deepEqual(removeIndexesAndValue(data.rows[1].children), [{text: 'December'}, {text: 'August'}, {text: 'July'}, {text: 'May'}, {text: 'October'}, {text: 'September'}, {text: 'March'}, {text: 'June'}, {text: 'January'}, {text: 'April'}, {text: 'November'}, {text: 'February'}]);
        assert.deepEqual(removeIndexesAndValue(data.rows[2].children), [{text: 'December'}, {text: 'October'}, {text: 'November'}, {text: 'September'}, {text: 'August'}, {text: 'July'}, {text: 'May'}, {text: 'June'}, {text: 'April'}, {text: 'March'}, {text: 'January'}, {text: 'February'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    ...
    QUnit.test('Sorting group by Summary field', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.rows[1].children), [{text: 'December 2003'}, {text: 'October 2003'}, {text: 'November 2003'}, {text: 'September 2003'}, {text: 'August 2003'}, {text: 'July 2003'}, {text: 'May 2003'}, {text: 'June 2003'}, {text: 'April 2003'}, {text: 'March 2003'}, {text: 'January 2003'}, {text: 'February 2003'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Sorting by Summary field in first field on axis', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(data.rows[2].text, 'CY 2003');
        assert.deepEqual(data.rows[3].text, 'CY 2004');
        assert.deepEqual(removeIndexesAndValue(data.rows[2].children), [{text: 'January'}, {text: 'February'}, {text: 'March'}, {text: 'April'}, {text: 'May'}, {text: 'June'}, {text: 'July'}, {text: 'August'}, {text: 'September'}, {text: 'October'}, {text: 'November'}, {text: 'December'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Sorting by Summary field in first field on axis with path', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.rows[2].children), [{text: 'January'}, {text: 'February'}, {text: 'March'}, {text: 'April'}, {text: 'May'}, {text: 'June'}, {text: 'July'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, 0);
      }).fail(getFailCallBack(assert)).always(done);
    });
    QUnit.test('Sorting columns by row', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.columns[2].children), [{text: 'January'}, {text: 'February'}, {text: 'March'}, {text: 'April'}, {text: 'May'}, {text: 'June'}, {text: 'July'}]);
        assert.strictEqual(data.grandTotalRowIndex, 0);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
      }).fail(function(e) {
        e = e || {};
    ...
    QUnit.test('Sorting by Summary two dimension on axis', function(assert) {
      ...
      this.load({
        ...
        assert.deepEqual(removeIndexesAndValue(data.rows[2].children), [{text: 'Jerseys'}, {text: 'Caps'}, {text: 'Gloves'}, {text: 'Shorts'}, {text: 'Vests'}, {text: 'Socks'}]);
    ...
    QUnit.test('Sort group when expanded item', function(assert) {
      ...
      this.load({
        ...
        assert.strictEqual(data.columns[1].text, 'Bikes');
        assert.strictEqual(data.columns[2].text, 'Accessories');
        assert.deepEqual(removeIndexesAndValue(data.rows), [{text: 'August 2004'}, {text: 'July 2004'}, {text: 'June 2004'}, {text: 'May 2004'}, {text: 'April 2004'}, {text: 'March 2004'}, {text: 'February 2004'}, {text: 'January 2004'}]);
        assert.strictEqual(data.grandTotalColumnIndex, 0);
        assert.strictEqual(data.grandTotalRowIndex, expectedTotalIndex);
      }).fail(getFailCallBack(assert)).always(done);
    });
  ...
  QUnit.module('XMLA Store with another cubes', () => {
    QUnit.test('T248791. Dimension with zero level members', function(assert) {
      ...
      store.load({
        columns: [{
          dataField: '[Агрегация по дате].[Агрегация по дате]',
          area: 'column'
        }],
  ...
  QUnit.module('Subset', testEnvironment, () => {
    ...
    QUnit.test('Skip and take rows', function(assert) {
      ...
      this.store.load({
        rows: [{
          dataField: '[Product].[Subcategory]',
          filterValues: ['Bike Racks', 'Bottles and Cages', 'Caps', 'Cleaners']
        }],
        values: [{dataField: '[Measures].[Customer Count]'}],
        rowSkip: 2,
        rowTake: 2
    ...
    QUnit.test('Skip and take rows if expand', function(assert) {
      ...
      this.store.load({
        ...
        area: 'column',
        headerName: 'columns',
        rows: [{dataField: '[Customer].[Customer]'}],
        columns: [{dataField: '[Ship Date].[Calendar Year]'}, {dataField: '[Ship Date].[Month of Year]'}],
        values: [{dataField: '[Measures].[Customer Count]'}],
        rowSkip: 0,
        rowTake: 2
      }).done(function(data) {
    ...
    QUnit.test('Skip and take rows if expand and if oppositePath', function(assert) {
      ...
      this.store.load({
        ...
        path: ['&[2003]'],
        oppositePath: ['&[4]'],
        rows: [{dataField: '[Product].[Category]'}, {dataField: '[Customer].[Customer]'}],
        columns: [{dataField: '[Ship Date].[Calendar Year]'}, {dataField: '[Ship Date].[Month of Year]'}],
        values: [{dataField: '[Measures].[Customer Count]'}],
        rowSkip: 0,
        rowTake: 3
    ...
    QUnit.test('Skip and take columns with searchValue', function(assert) {
      ...
      this.store.load({
        ...
        assert.deepEqual(data.columns.map(getHeaderItemValue), [undefined, undefined, 'Men\'s Sports Shorts, L', 'Men\'s Sports Shorts, M', undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined]);
      }).fail(getFailCallBack(assert)).always(done);
    });
    ...
    QUnit.test('Skip and take rows and columns', function(assert) {
      ...
      this.store.load({
        columns: [{
          dataField: '[Product].[Subcategory]',
          filterValues: ['Bike Racks', 'Bike Stands', 'Bottles and Cages', 'Caps', 'Cleaners']
        }],
        rows: [{dataField: '[Product].[Category]'}],
        values: [{dataField: '[Measures].[Customer Count]'}],
        columnSkip: 2,
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/browser","core/devices","__internal/grids/pivot_grid/module_widget_utils","__internal/grids/pivot_grid/data_source/module_utils","__internal/grids/pivot_grid/xmla_store/module","../../helpers/pivotGridTestSettings.js"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/browser"), require("core/devices"), require("__internal/grids/pivot_grid/module_widget_utils"), require("__internal/grids/pivot_grid/data_source/module_utils"), require("__internal/grids/pivot_grid/xmla_store/module"), require("../../helpers/pivotGridTestSettings.js"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=store.xmla.tests.js.map