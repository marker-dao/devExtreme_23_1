...
(["testing/tests/DevExpress.viz.rangeSelector/slidersController.tests.js"], ["jquery","../../helpers/vizMocks.js","viz/range_selector/sliders_controller","viz/core/renderers/renderer","viz/translators/translator2d","core/utils/support"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.viz.rangeSelector/slidersController.tests.js', ['jquery', '../../helpers/vizMocks.js', 'viz/range_selector/sliders_controller', 'viz/core/renderers/renderer', 'viz/translators/translator2d', 'core/utils/support'], true, function ($__require, exports, module) {
    var global = this || self,
        GLOBAL = global;
    const $ = $__require('jquery');
    const vizMocks = $__require('../../helpers/vizMocks.js');
    const SlidersController = $__require('viz/range_selector/sliders_controller').SlidersController;
    ...
    const environment = {
        ...
        update: function (options) {
            options = options || {};
            this.controller.update(options.verticalRange || [15, 25], $.extend({ animationEnabled: true }, options.behavior), options.isCompactMode, $.extend({ color: 'handle-color', opacity: 'handle-opacity', width: 4 }, options.sliderHandle), $.extend({ color: 'marker-color', invalidRangeColor: 'invalid-color', paddingLeftRight: 4, paddingTopBottom: 1 }, options.sliderMarker), $.extend({ color: 'shutter-color', opacity: 'shutter-opacity' }, options.shutter), options.rangeBounds || {}, options.fullTicks, options.selectedRangeColor || 'selected-range-color');
        },
    ...
    QUnit.module('Selected range', $.extend({}, environment, {
        check: function (assert, expected, expectedRange) {
            assert.deepEqual([this.sliderRoot(0).animate.lastCall.args[0].translateX, this.sliderRoot(1).animate.lastCall.args[0].translateX], expected, 'sliders');
    ...
    QUnit.test('translator range is empty', function (assert) {
        ...
        assert.deepEqual([this.sliderRoot(0).attr.lastCall.args[0].translateX, this.sliderRoot(1).attr.lastCall.args[0].translateX], [1000, 3000], 'sliders');
    ...
    QUnit.test('Normal case', function (assert) {
        ...
        this.check(assert, [1900, 2300], [[-207, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -207, 20], [0, 0, 158, 0, 158, 20, 4, 20, 0, 24, 0, 20, 0, 20]], [-203, 4], [-207, 0]);
    });
    ...
    QUnit.test('Boundary case', function (assert) {
        ...
        this.check(assert, [1000, 3000], [[0, 0, 208, 0, 208, 20, 4, 20, 0, 24, 0, 20, 0, 20], [-157, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -157, 20]], [4, -153], [0, -157]);
    });
    ...
    QUnit.test('Values are equal', function (assert) {
        ...
        this.check(assert, [2200, 2200], [[-207, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -207, 20], [0, 0, 158, 0, 158, 20, 4, 20, 0, 24, 0, 20, 0, 20]], [-203, 4], [-207, 0]);
    });
    ...
    QUnit.test('Values are equal (categories)', function (assert) {
        ...
        this.check(assert, [1400, 1800], [[-207, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -207, 20], [0, 0, 158, 0, 158, 20, 4, 20, 0, 24, 0, 20, 0, 20]], [-203, 4], [-207, 0]);
    });
    ...
    QUnit.test('Both values are at start', function (assert) {
        ...
        this.check(assert, [1000, 1000], [[0, 0, 208, 0, 208, 20, 4, 20, 0, 24, 0, 20, 0, 20], [0, 0, 158, 0, 158, 20, 4, 20, 0, 24, 0, 20, 0, 20]], [4, 4], [0, 0]);
    });
    ...
    QUnit.test('Both values are at end', function (assert) {
        ...
        this.check(assert, [3000, 3000], [[-207, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -207, 20], [-157, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -157, 20]], [-203, -153], [-207, -157]);
    });
    ...
    QUnit.test('Overlapping near start', function (assert) {
        ...
        this.check(assert, [1100, 1200], [[-100, 0, 108, 0, 108, 20, 4, 20, 0, 24, -4, 20, -100, 20], [0, 0, 158, 0, 158, 20, 4, 20, 0, 24, 0, 20, 0, 20]], [-96, 4], [-100, 0]);
    ...
    QUnit.test('Overlapping near end', function (assert) {
        ...
        this.check(assert, [2900, 2950], [[-207, 0, 1, 0, 1, 20, 1, 20, 1, 24, -3, 20, -207, 20], [-107, 0, 51, 0, 51, 20, 5, 20, 1, 24, -3, 20, -107, 20]], [-203, -103], [-207, -107]);
        assert.deepEqual(this.sliderBorder(0).append.lastCall.args, [this.sliderLabelRoot(0)], 'border is appended 1');
        assert.deepEqual(this.sliderBorder(1).append.lastCall.args, [this.sliderLabelRoot(1)], 'border is appended 2');
        // TODO: Investigate why heights are different
    ...
    QUnit.test('Docking with DateTime (irregular scale, from big range to small)', function (assert) {
        ...
        this.update({
            behavior: { snapToTicks: true },
            fullTicks: [new Date(2011, 0, 1), new Date(2011, 0, 2), new Date(2011, 0, 3), new Date(2011, 0, 4), new Date(2011, 0, 5), new Date(2011, 0, 6), new Date(2011, 0, 7), new Date(2011, 0, 8), new Date(2011, 0, 9), new Date(2011, 0, 10), new Date(2011, 0, 11), new Date(2011, 0, 12), new Date(2011, 0, 13)]
        });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","../../helpers/vizMocks.js","viz/range_selector/sliders_controller","viz/core/renderers/renderer","viz/translators/translator2d","core/utils/support"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("../../helpers/vizMocks.js"), require("viz/range_selector/sliders_controller"), require("viz/core/renderers/renderer"), require("viz/translators/translator2d"), require("core/utils/support"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=slidersController.tests.js.map