...
(["testing/tests/DevExpress.viz.vectorMap/tracker.tests.js"], ["jquery","core/utils/common","../../helpers/vizMocks.js","viz/vector_map/tracker","viz/vector_map/event_emitter","animation/frame"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.viz.vectorMap/tracker.tests.js", ["jquery", "core/utils/common", "../../helpers/vizMocks.js", "viz/vector_map/tracker", "viz/vector_map/event_emitter", "animation/frame"], function($__export) {
  "use strict";
  var $,
      noop,
      vizMocks,
  ...
  return {
    setters: [function($__m) {
      ...
      eventEmitterModule = $__m.default;
    }, function($__m) {
    ...
    execute: function() {
      ...
      environment = {
        beforeEach: function() {
          ...
          this.$target.get(0)['vectormap-data'] = 'test-data';
        ...
        triggerEvent: function($target, type, x, y, originalEventData) {
          ...
          var pointerType = /^pointer/.test(type) ? 'touch' : (/^MSPointer/.test(type) ? 2 : undefined);
      ...
      QUnit.test('Subscription to projection', function(assert) {
        ...
        assert.strictEqual($traceurRuntime.typeof(arg.center), 'function', 'center handler');
        assert.strictEqual($traceurRuntime.typeof(arg.zoom), 'function', 'zoom handler');
      ...
      QUnit.module('Default prevention', $.extend({}, environment, {check: function(assert, eventMode, eventName, withData, preventDefaultCount, stopPropagationCount) {
          ...
          this[withData ? 'trigger' : 'triggerNoData'](eventName, {
            x: 10,
            y: 20
          }, {
      ...
      QUnit.test('mouse - wheel', function(assert) {
        this.check(assert, 'mouse', 'wheel', true, 1, 2);
      });
      QUnit.test('mouse - wheel / no data', function(assert) {
        this.check(assert, 'mouse', 'wheel', false, 0, 1);
      });
      ...
      $.each(['mouse', 'touch', 'MSPointer', 'pointer'], function(_, type) {
        ...
        QUnit.test('"start" is raised on start', function(assert) {
          ...
          assert.deepEqual(this.onStart.lastCall.args, [{
            data: 'test-data',
            x: 10,
            y: 20
          }]);
        ...
        QUnit.test('"move" is raised on move after start', function(assert) {
          ...
          assert.deepEqual(this.onMove.lastCall.args, [{
            data: 'test-data',
            x: 22,
            y: 31
          }]);
        ...
        QUnit.test('"end" is raised on end after start', function(assert) {
          ...
          assert.deepEqual(this.onEnd.lastCall.args, [{
            data: 'test-data',
            x: 10,
            y: 20
          }]);
      ...
      QUnit.test('raised on wheel', function(assert) {
        ...
        assert.deepEqual(this.onZoom.lastCall.args, [{
          delta: 1,
          x: 10,
          y: 20
        }]);
      ...
      $.each(['touch', 'MSPointer', 'pointer'], function(_, type) {
        ...
        QUnit.test('raised on end after two starts', function(assert) {
          this.triggerMultitouch('start', {
            x: 10,
            y: 20
          }, null, 0).triggerMultitouch('start', {
            x: 30,
            y: 40
          }, null, 1).triggerMultitouch('end', {}, null, 1);
          assert.deepEqual(this.onZoom.lastCall.args, [{
            ratio: 1,
            x: 20,
            y: 30
        ...
        QUnit.test('raised after moves', function(assert) {
          this.triggerMultitouch('start', {
            ...
            x: 10,
            y: 40
          }, null, 1).triggerMultitouch('move', {
            x: 30,
            y: 10
          }, null, 1).triggerMultitouch('move', {
            x: 70,
            y: 10
          }, null, 0).triggerMultitouch('end', {}, null, 0);
          assert.deepEqual(this.onZoom.lastCall.args, [{
            ratio: 2,
            x: 10,
            y: 30
      ...
      StubFocus = vizMocks.stubClass(new trackerModule.Focus(), null, {$constructor: function() {
      ...
      QUnit.test('focus is turned off then turned on on move over element with data', function(assert) {
        ...
        assert.deepEqual(this.focus.turnOn.lastCall.args, ['test-data', {
          x: 10,
          y: 20
        }], 'turn on');
      });
      ...
      QUnit.test('turnOn', function(assert) {
        ...
        assert.strictEqual($traceurRuntime.typeof(arg.done), 'function');
        delete arg.done;
        assert.deepEqual(this.focusOn.lastCall.args[0], {
          data: 'data-1',
          x: 10,
          y: 20
        });
      ...
      QUnit.test('turnOn when on', function(assert) {
        ...
        assert.deepEqual(this.focusMove.lastCall.args[0], {
          data: 'data-1',
          x: 20,
          y: 40
        });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","../../helpers/vizMocks.js","viz/vector_map/tracker","viz/vector_map/event_emitter","animation/frame"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("../../helpers/vizMocks.js"), require("viz/vector_map/tracker"), require("viz/vector_map/event_emitter"), require("animation/frame"));
  else
