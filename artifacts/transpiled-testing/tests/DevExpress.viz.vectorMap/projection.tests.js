...
(["testing/tests/DevExpress.viz.vectorMap/projection.tests.js"], ["jquery","core/utils/common","viz/vector_map/event_emitter","viz/vector_map/projection.main","viz/vector_map/projection"], true, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.registerDynamic('testing/tests/DevExpress.viz.vectorMap/projection.tests.js', ['jquery', 'core/utils/common', 'viz/vector_map/event_emitter', 'viz/vector_map/projection.main', 'viz/vector_map/projection'], true, function ($__require, exports, module) {
    var global = this || self,
        GLOBAL = global;
    const $ = $__require('jquery');
    const noop = $__require('core/utils/common').noop;
    const eventEmitterModule = $__require('viz/vector_map/event_emitter');
    ...
    QUnit.test('setEngine / center is changed', function (assert) {
        ...
        const engine = projection({
            to: function (coords) {
                return [(coords[0] + 60) / 20, (coords[1] - 40) / 10];
            },
            from: function (coords) {
                return [(coords[0] - 3) * 20, (coords[1] + 4) * 10];
            }
        });
    ...
    QUnit.test('setEngine / center is not changed', function (assert) {
        ...
        const engine = projection({
            to: function (coords) {
                return [(coords[0] - 40) / 40, (coords[1] + 20) / 10];
            },
            from: function (coords) {
                return [(coords[0] + 1) * 40, (coords[1] - 2) * 10];
            }
        });
        this.projection.setSize({ left: 200, top: 100, width: 800, height: 700 });
    ...
    QUnit.test('Transform. Engine is pseudocylindrical - bounds should be correct', function (assert) {
        ...
        this.projection.setEngine({
            ...
            from: function (coordinates) {
                ...
                const t = y / 2;
                return [x * Math.PI / Math.sqrt(1 - 3 * t * t) / RADIANS, y * Math.PI / 2 / RADIANS];
            }
        });
    ...
    $.each([{ bounds: [-1.5, -3, 0.5, 1], center: [-0.5, -1], min: [-1.5, -3], max: [0.5, 1], project: [-1, 2], unproject: [-0.5, -1.5], message: 'x equals y' }, { bounds: [-1.5, -0.5, 0.5, 1.5], center: [-0.5, 0.5], min: [-1.5, -0.5], max: [0.5, 1.5], project: [-0.25, 0.75], unproject: [0.5, -0.25], message: 'x is wider than y' }, { bounds: [0.5, -3, 1.5, 1], center: [1, -1], min: [0.5, -3], max: [1.5, 1], project: [1.25, -0.5], unproject: [0.5, -0.5], message: 'y is wider than x' }, { bounds: [0.5, -3, 0.5, 1], center: [0.5, -1], min: [0.5, -3], max: [0.5, 1], project: [1, -2], unproject: [0.5, 0.5], message: 'x is zero' }, { bounds: [-1.5, -0.5, 0.5, -0.5], center: [-0.5, -0.5], min: [-1.5, -0.5], max: [0.5, -0.5], project: [-1, 1], unproject: [-0.5, -0.75], message: 'y is zero' }], function (_, data) {
    ...
    QUnit.test('mercator', function (assert) {
        const X = 85.0511;
        this.check(assert, [{ project: [-180, -X], unproject: [-1, +1] }, { project: [-180, +X], unproject: [-1, -1] }, { project: [+180, -X], unproject: [+1, +1] }, { project: [+180, +X], unproject: [+1, -1] }, { project: [-90, +45], unproject: [-0.5, -0.2805] }, { project: [+45, -22.5], unproject: [+0.25, +0.1283] }]);
    });

    QUnit.test('equirectangular', function (assert) {
        this.check(assert, [{ project: [-180, -90], unproject: [-1, +1] }, { project: [-180, +90], unproject: [-1, -1] }, { project: [+180, -90], unproject: [+1, +1] }, { project: [+180, +90], unproject: [+1, -1] }, { project: [+270, -180], unproject: [+1.5, +2] }, { project: [-90, +45], unproject: [-0.5, -0.5] }, { project: [+45, -22.5], unproject: [+0.25, +0.25] }]);
    });

    QUnit.test('lambert', function (assert) {
        this.check(assert, [{ project: [-180, -90], unproject: [-1, +1] }, { project: [-180, +90], unproject: [-1, -1] }, { project: [+180, -90], unproject: [+1, +1] }, { project: [+180, +90], unproject: [+1, -1] }, { project: [+270, -60], unproject: [+1.5, 0.866] }, { project: [-90, +45], unproject: [-0.5, -0.7071] }, { project: [+45, -22.5], unproject: [+0.25, +0.3827] }]);
    });

    QUnit.test('miller', function (assert) {
        this.check(assert, [{ project: [-180, -90], unproject: [-1, +0.7332] }, { project: [-180, +90], unproject: [-1, -0.7332] }, { project: [+180, -90], unproject: [+1, +0.7332] }, { project: [+180, +90], unproject: [+1, -0.7332] }, { project: [+270, -70], unproject: [+1.5, +0.4715] }, { project: [-90, +45], unproject: [-0.5, -0.2683] }, { project: [+45, -22.5], unproject: [+0.25, +0.1271] }]);
    });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","viz/vector_map/event_emitter","viz/vector_map/projection.main","viz/vector_map/projection"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("viz/vector_map/event_emitter"), require("viz/vector_map/projection.main"), require("viz/vector_map/projection"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=projection.tests.js.map