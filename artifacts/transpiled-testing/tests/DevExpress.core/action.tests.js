...
(["testing/tests/DevExpress.core/action.tests.js"], ["jquery","core/utils/common","core/action"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.core/action.tests.js", ["jquery", "core/utils/common", "core/action"], function($__export) {
  "use strict";
  var $,
      noop,
      Action,
  ...
  return {
    ...
    execute: function() {
      QUnit.testStart(function() {
        var markup = '<div class="dx-state-disabled">\
      ...
      actionExecutors = Action.executors;
      register = Action.registerExecutor;
      unregister = Action.unregisterExecutor;
      mockStringActionExecutor = {execute: function(e) {
      ...
      QUnit.test('registerActionExecutor/unregisterActionExecutor by object', function(assert) {
        ...
        var testExecutor2 = {execute: noop};
        register({
          'test1': testExecutor1,
          'test2': testExecutor2
        });
      ...
      QUnit.test('canceled action is not executed', function(assert) {
        var action = new Action('new location');
        register('canceller', {validate: function(e) {
            e.cancel = true;
          }});
        var result = action.execute();
      ...
      QUnit.test('action is handled once', function(assert) {
        ...
        var action = new Action(new CustomActionClass());
        var handledTwice = false;
        register('test1', {execute: function(e) {
            if (e.action instanceof CustomActionClass) {
              e.handled = true;
            }
          }});
        register('test2', {execute: function(e) {
            if (e.action instanceof CustomActionClass) {
              handledTwice = true;
              e.handled = true;
            }
      ...
      QUnit.test('beforeExecute callback can cancel action execution', function(assert) {
        ...
        var action = new Action(handlerSpy, {beforeExecute: function(e) {
            e.cancel = true;
          }});
        action.execute();
      ...
      QUnit.test('ui interaction validator should prevent all action handlers by validatingTargetName', function(assert) {
        var handlerSpy = sinon.spy(noop);
        var action = new Action(handlerSpy, {validatingTargetName: 'customElement'});
        action.execute({
      ...
      QUnit.test('ui interaction validator should prevent all ui action handlers by \'dx-state-readonly\' class', function(assert) {
        var handlerSpy = sinon.spy(noop);
        var $targetElement = $('.dx-state-readonly .dx-click-target');
      ...
      QUnit.test('not passed, all validators should be performed', function(assert) {
        ...
        var testExecutorWithExecuteCalled = 0;
        var testExecutorWithValidateAndExecuteValidateCalled = 0;
        var testExecutorWithValidateAndExecuteExecuteCalled = 0;
        register({
          'testExecutorWithValidate': {validate: function() {
              testExecutorWithValidateValidateCalled++;
            }},
          'testExecutorWithExecute': {execute: function() {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/common","core/action"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/common"), require("core/action"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=action.tests.js.map