...
(["testing/tests/DevExpress.ui/hierarchicalCollectionWidgetParts/hierarchicalDataAdapter.js"], ["jquery","ui/hierarchical_collection/ui.data_adapter","data/utils","./hierarchicalCollectionTestHelper.js","ui/widget/ui.errors"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui/hierarchicalCollectionWidgetParts/hierarchicalDataAdapter.js", ["jquery", "ui/hierarchical_collection/ui.data_adapter", "data/utils", "./hierarchicalCollectionTestHelper.js", "ui/widget/ui.errors"], function($__export) {
  "use strict";
  var $,
      HierarchicalDataAdapter,
      processRequestResultLock,
      HierarchicalCollectionTestHelper,
      errors,
      module,
      test,
      helper,
  ...
  return {
    setters: [function($__m) {
      ...
      HierarchicalDataAdapter = $__m.default;
    }, function($__m) {
      processRequestResultLock = $__m.processRequestResultLock;
    }, function($__m) {
      HierarchicalCollectionTestHelper = $__m.default;
    }, function($__m) {
      errors = $__m.default;
    }],
    execute: function() {
      ...
      module('plain structure', moduleConfig, function() {
        test('all items should be converted', function(assert) {
          var items = helper.plainData;
          var dataAdapter = helper.initDataAdapter({
            items: items,
            dataType: 'plain'
          });
          var data = dataAdapter.getData();
        ...
        test('data adapter should throw an error when id is not unique', function(assert) {
          ...
          }, errors.Error('E1040', '1'), 'raised error is correct');
        });
        ...
        test('public node should exist in internalFields', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithKeys});
          var data = dataAdapter.getData();
          assert.ok(Object.keys(data[0].internalFields.publicNode).length, 'publicNode is not empty');
        });
      });
      module('tree structure with keys', moduleConfig, function() {
        test('all items should be converted', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithKeys});
          var data = dataAdapter.getData();
          assert.equal(data.length, 7, 'all items was converted');
        ...
        test('dataAdapter should work correct with circular data', function(assert) {
          ...
          var dataAdapter = helper.initDataAdapter({items: [parent]});
          var data = dataAdapter.getData();
          assert.equal(data.length, 2, 'circular items were converted');
        });
      });
      module('tree structure without keys', moduleConfig, function() {
        test('all items should be converted', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithoutKeys});
          var data = dataAdapter.getData();
          assert.equal(data.length, 7, 'all items was converted');
        });
        test('parent keys should be correct', function(assert) {
      ...
      module('tree structure with object instances', moduleConfig, function() {
        test('all items should be converted', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.getTreeDataWithObjects()});
          var data = dataAdapter.getData();
        ...
        test('items should be correct', function(assert) {
          var items = helper.getTreeDataWithObjects();
          var dataAdapter = helper.initDataAdapter({items: items});
          var data = dataAdapter.getData();
      ...
      module('public methods', moduleConfig, function() {
        test('getTreeNodes', function(assert) {
          ...
          var accessors = {
            ...
            setters: {
              ...
              expanded: function(item, value) {
                item.isExpanded = value;
              },
              disabled: function(item, value) {
                item.isDisabled = value;
              },
              parentKey: function(item, value) {
                item.parentId = value;
              },
              key: function(item, value) {
                item.id = value;
              },
              items: function(item, value) {
                item.items = value;
              }
            }
          };
          ...
          var nodes = dataAdapter.getTreeNodes();
        ...
        test('getTreeNodes if data was without keys', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithoutKeys});
          var treeNodes = dataAdapter.getTreeNodes();
        ...
        test('getNodeByItem', function(assert) {
          ...
          var node = dataAdapter.getNodeByItem(helper.plainData[1]);
        ...
        test('getNodesByItems', function(assert) {
          ...
          var nodes = dataAdapter.getNodesByItems(helper.plainData);
          for (var i = 0,
              n = helper.plainData.length; i < n; i++) {
        ...
        test('getNodeByKey', function(assert) {
          ...
          var node = dataAdapter.getNodeByKey(2);
          assert.deepEqual(node.text, 'Item 11', 'node is correct');
        });
        ...
        test('getVisibleItemsCount', function(assert) {
          ...
          var dataAdapter = helper.initDataAdapter({items: helper.plainData});
        ...
        test('isAllSelected', function(assert) {
          ...
          $.each(helper.plainData, function(_, item) {
            item.selected = true;
          });
        ...
        test('getChildrenNodes', function(assert) {
          ...
          var children = dataAdapter.getChildrenNodes(1);
      ...
      module('getPublicNode method', moduleConfig, function() {
        test('public node should have correct accessor fields', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithoutKeys});
          var data = dataAdapter.getData();
          var publicNode = dataAdapter.getPublicNode(data[2]);
        ...
        test('public node should have correct item field', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: helper.treeDataWithoutKeys});
          var data = dataAdapter.getData();
          var publicNode = dataAdapter.getPublicNode(data[5]);
        ...
        test('public node should depend on original node', function(assert) {
          ...
          var dataAdapter = helper.initDataAdapter({items: helper.plainData});
          var data = dataAdapter.getData();
          var publicNode = dataAdapter.getPublicNode(data[0]);
      ...
      module('selection', moduleConfig, function() {
      ...
      module('expand', moduleConfig, function() {
      ...
      module('Item\'s dependence from nodes', moduleConfig, function() {
      ...
      module('Expansion changing', moduleConfig, function() {
      ...
      module('Search operation', moduleConfig, function() {
        test('It should be possible to find items that contain some substring', function(assert) {
          var dataAdapter = helper.initDataAdapter({items: [{
              id: 1,
              parentId: 0,
              text: 'Cars'
            }, {
        ...
        test('It should be possible to find items if text is set via expression', function(assert) {
          ...
          var dataAdapter = new HierarchicalDataAdapter({
            dataAccessors: accessors,
            items: items,
            dataType: 'plain'
          });
        ...
        test('searchExpr is array', function(assert) {
          ...
          var result = dataAdapter.search('1');
        ...
        test('search should consider simple sorting', function(assert) {
          ...
          var result = dataAdapter.search('1');
          var expectedValues = ['BMW', 'Cars', 'Motobikes', 'X1', 'Yamaha', 'YX 1'];
        ...
        test('search should consider sorting expression', function(assert) {
          ...
          var result = dataAdapter.search('1');
          var expectedValues = ['YX 1', 'Yamaha', 'X1', 'Motobikes', 'Cars', 'BMW', 'Audi', 'A1'];
          $.each(result, function(index, item) {
            assert.equal(item.text, expectedValues[index], 'Correct item');
          });
        });
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","ui/hierarchical_collection/ui.data_adapter","data/utils","./hierarchicalCollectionTestHelper.js","ui/widget/ui.errors"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("ui/hierarchical_collection/ui.data_adapter"), require("data/utils"), require("./hierarchicalCollectionTestHelper.js"), require("ui/widget/ui.errors"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=hierarchicalDataAdapter.js.map