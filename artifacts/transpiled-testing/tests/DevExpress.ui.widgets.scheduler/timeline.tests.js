System.register(["core/utils/size", "core/utils/date", "core/utils/resize_callbacks", "events/visibility_change", "generic_light.css!", "jquery", "ui/scheduler/workspaces/ui.scheduler.timeline", "ui/scheduler/workspaces/ui.scheduler.timeline_day", "ui/scheduler/workspaces/ui.scheduler.timeline_month", "ui/scheduler/workspaces/ui.scheduler.timeline_week", "ui/scheduler/workspaces/ui.scheduler.timeline_work_week", "../../helpers/keyboardMock.js", "../../helpers/pointerMock.js"], function (_export, _context) {
  "use strict";

  var getOuterWidth, getOuterHeight, dateUtils, resizeCallbacks, triggerHidingEvent, triggerShownEvent, $, keyboardMock, pointerMock, CELL_CLASS, HOVER_CLASS, TIMELINE_DAY, TIMELINE_WEEK, TIMELINE_MONTH;
  return {
    setters: [function (_coreUtilsSize) {
      ...
      triggerHidingEvent = _eventsVisibility_change.triggerHidingEvent;
      triggerShownEvent = _eventsVisibility_change.triggerShownEvent;
    }, function (_generic_lightCss) {}, function (_jquery) {
      $ = _jquery.default;
    }, function (_uiSchedulerWorkspacesUiSchedulerTimeline) {}, function (_uiSchedulerWorkspacesUiSchedulerTimeline_day) {}, function (_uiSchedulerWorkspacesUiSchedulerTimeline_month) {}, function (_uiSchedulerWorkspacesUiSchedulerTimeline_week) {}, function (_uiSchedulerWorkspacesUiSchedulerTimeline_work_week) {}, function (_helpersKeyboardMockJs) {
    ...
    execute: function () {
      ...
      QUnit.skip('the "getCoordinatesByDate" method should return right coordinates for grouped timeline', function (assert) {
        ...
        const expectedPosition = instance.$element().find('.dx-scheduler-date-table-row').eq(1).find('.dx-scheduler-date-table-cell').eq(2).position();
      ...
      QUnit.test('the \'getCellIndexByCoordinates\' method should return right coordinates', function (assert) {
        ...
        const cellIndex = this.instance.getCellIndexByCoordinates({
          left: cellWidth * 15,
          top: 1
        });
      ...
      QUnit.test('the \'getCellIndexByCoordinates\' method should return right coordinates for fractional value', function (assert) {
        ...
        let cellIndex = this.instance.getCellIndexByCoordinates({
          left: cellWidth * 15 + 0.656,
          top: cellHeight * 2 - 0.656
        });
        assert.equal(cellIndex, 63, 'Cell index is OK');
        cellIndex = this.instance.getCellIndexByCoordinates({
          left: cellWidth + 0.656,
          top: cellHeight - 0.656
        });
      ...
      QUnit.skip('the \'getCoordinatesByDate\' method should return right coordinates for grouped timeline, groupOrientation = horizontal', function (assert) {
        ...
        const expectedPosition = this.instance.$element().find('.dx-scheduler-date-table-row').eq(0).find('.dx-scheduler-date-table-cell').eq(4).position();
        assert.equal(coordinates.left, expectedPosition.left, 'Coordinates are OK');
        assert.equal(coordinates.top, expectedPosition.top, 'Coordinates are OK');
      });
      ...
      QUnit.skip('Scheduler timeline month getPositionShift should return null shift', function (assert) {
        ...
        assert.deepEqual(this.instance.getPositionShift(), {
          top: 0,
          left: 0,
          cellPosition: 0
        }, 'First view date is OK');
      });
      ...
      QUnit.module('Timeline Keyboard Navigation', () => {
        ['standard', 'virtual'].forEach(scrollingMode => {
          const moduleDescription = scrollingMode === 'virtual' ? 'Virtual Scrolling' : 'Standard Scrolling';
          QUnit.module(moduleDescription, {
            ...
            QUnit.test('Timeline should select/unselect cells with mouse', function (assert) {
              ...
              $($table).trigger($.Event('dxpointerdown', {
                target: cells.eq(3).get(0),
                which: 1,
                pointerType: 'mouse'
              }));
              $($table).trigger($.Event('dxpointermove', {
                target: cell,
                which: 1
              }));
              ...
              $($table).trigger($.Event('dxpointerup', {
                target: cell,
                which: 1
              }));
            });
            ...
            QUnit.module('Mouse Multiselection with Vertical Grouping and Grouping by Date', () => {
              [{
                ...
                QUnit.test(description, function (assert) {
                  ...
                  $($table).trigger($.Event('dxpointerdown', {
                    target: cells.eq(startCell).get(0),
                    which: 1,
                    pointerType: 'mouse'
                  }));
                  $($table).trigger($.Event('dxpointermove', {
      ...
      QUnit.module('Mouse Interaction', () => {
        [TIMELINE_DAY, TIMELINE_WEEK, TIMELINE_MONTH].forEach(workSpace => {
          QUnit.test(`Cell hover should work correctly in ${workSpace.name}`, function (assert) {
            ...
            const cells = $element.find(`.${CELL_CLASS}`);
            $element.trigger($.Event('dxpointerenter', {
              target: cells.eq(2).get(0),
              which: 1
            }));
      ...
      QUnit.module('Renovated Render', {
        ...
        [TIMELINE_DAY, TIMELINE_WEEK, TIMELINE_MONTH].forEach(({
          class: viewClass,
          name
        }) => {
