System.register(["animation/fx", "generic_light.css!", "jquery", "ui/widget/ui.errors", "../../helpers/scheduler/helpers.js"], function (_export, _context) {
  "use strict";

  var fx, $, errors, createWrapper, test, module, testStart;
  return {
      ...
      module('ScrollTo', {
        ...
        [{
          ...
          const checkScrollTo = (assert, scheduler, topCellCount, leftCellCount, date, groups, allDay) => {
            ...
            const rtlInitialPosition = scrollableInstance.option('rtlEnabled') ? scrollableInstance.scrollLeft() : 0;
            ...
            assert.roughEqual(scrollByStub.getCall(0).args[0].top, topCellCount * cellHeight - (scrollableHeight - cellHeight) / 2, 2.01, 'Correct top parameter');
            assert.roughEqual(rtlInitialPosition + scrollByStub.getCall(0).args[0].left, leftCellCount * cellWidth - (scrollableWidth - cellWidth) / 2, 3.01, 'Correct left parameter');
          };
          ...
          [{
            ...
            test(`ScrollTo to all-day cells should work in ${view} view when ${scrolling.text} is used`, function (assert) {
              ...
              const cellHeight = $schedulerCell.get(0).getBoundingClientRect().height;
              const cellWidth = $schedulerCell.get(0).getBoundingClientRect().width;
              const top = view === 'week' ? 0 : topCellCount * cellHeight - (scrollableHeight - cellHeight) / 2;
              assert.ok(scrollBy.calledOnce, 'ScrollBy was called');
              assert.roughEqual(scrollBy.getCall(0).args[0].top, top, 2.01, 'Correct top parameter');
              assert.equal(scrollBy.getCall(0).args[0].left, leftCellCount * cellWidth - (scrollableWidth - cellWidth) / 2, 'Correct left parameter');
            });
          });
          [{
            view: 'week',
