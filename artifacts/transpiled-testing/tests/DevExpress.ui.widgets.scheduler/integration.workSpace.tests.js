System.register(["jquery", "localization/date", "../../helpers/scheduler/helpers.js", "../../helpers/keyboardMock.js", "localization", "events/core/events_engine", "animation/fx", "../../helpers/pointerMock.js", "events/drag", "data/custom_store", "core/utils/type", "core/config", "animation/translator", "core/utils/size"], function (_export, _context) {
  "use strict";

  var $, dateLocalization, createWrapper, CLASSES, initTestMarkup, isDesktopEnvironment, keyboardMock, localization, eventsEngine, fx, pointerMock, dragEvents, CustomStore, isRenderer, config, translator, getOuterHeight, getInnerHeight, getOuterWidth, SELECTED_CELL_CLASS, FOCUSED_CELL_CLASS, testStart, module, test, moduleConfig;
  return {
    ...
    execute: function () {
      ...
      FOCUSED_CELL_CLASS = CLASSES.focusedCell.slice(1);
      ({
        testStart,
        module,
        test
      ...
      module('Integration: Work space', {
        ...
        test('Scheduler work space should have a single type class', function (assert) {
          ...
          const check = function (className) {
            let checked = true;
            $.each(['dx-scheduler-work-space-day', 'dx-scheduler-work-space-week', 'dx-scheduler-work-space-work-week', 'dx-scheduler-work-space-month'], function (_, item) {
              if (className === item) {
        ...
        ['standard', 'virtual'].forEach(scrollingMode => {
          ...
          test(`Focused cells cash should be correct (T640466) when scrolling is ${scrollingMode}`, function (assert) {
            ...
            const workSpace = scheduler.instance.getWorkSpace();
            assert.deepEqual(workSpace.cellsSelectionState.getSelectedCells()[0], {
              startDate: new Date(2018, 3, 8),
              endDate: new Date(2018, 3, 8, 0, 30),
              allDay: false,
              groupIndex: 0
        ...
        isDesktopEnvironment() && test('SelectedCellData option should be correct when virtual scrolling is enabled', function (assert) {
          ...
          $($table).trigger($.Event('dxpointerdown', {
            target: $cells.eq(0).get(0),
            which: 1,
            pointerType: 'mouse'
          }));
          $($table).trigger($.Event('dxpointermove', {
            target: $cells.eq(1).get(0),
            which: 1
          }));
          const firstCell = {
            allDay: false,
        ...
        test('"onOptionChanged" should not be called on scroll when virtual scrolling is enabled', function (assert) {
          ...
          setTimeout(() => {
            assert.equal(onOptionChangedCalls, 1, '"onOptionChanged" was not triggered again because selected cells have not been changed');
            done();
          });
        });
        isDesktopEnvironment() && test('Appointment popup should be opened with correct parameters if virtual scrolling is enabled', function (assert) {
          ...
          setTimeout(() => {
            ...
            assert.deepEqual(showAppointmentPopupSpy.getCall(0).args[0], {
              allDay: false,
              endDate: new Date(2020, 8, 21, 0, 30),
              startDate: new Date(2020, 8, 20, 0, 0)
      ...
      isDesktopEnvironment() && module('Cells selection', {
        ...moduleConfig
      }, () => {
        const resources = [{
          fieldExpr: 'ownerId',
        ...
        module(' Multiple selection when dragging is not enabled', () => {
          [{
            ...
            test(`Multiple selection should work in ${view} when dragging is not enabled`, function (assert) {
              ...
              const $table = instance.workSpace.getDateTable();
              $($table).trigger($.Event('dxpointerdown', {
                target: $cells.eq(startCell.index).get(0),
                which: 1,
                pointerType: 'mouse'
              }));
              $($table).trigger($.Event('dxpointermove', {
                target: $cells.eq(endCell.index).get(0),
                which: 1
              }));
              assert.deepEqual(instance.option('selectedCellData'), [startCell.cellData, endCell.cellData], 'correct cells have been selected');
            });
            if (view !== 'month') {
      ...
      module('Resource Cell Template', () => {
        [true, false].forEach(renovateRender => {
          const moduleDescription = renovateRender ? 'Renovated Render' : 'Old Render';
          module(moduleDescription, {
            ...
            test('resourceCellTemplate should have correct options', function (assert) {
              ...
              assert.deepEqual(templateOptions.data, {
                text: 'John',
                id: 1
              }, 'data option is OK');
            });
