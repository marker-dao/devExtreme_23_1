System.register(["animation/fx", "jquery", "../../helpers/pointerMock.js", "core/utils/browser", "data/data_source/data_source", "animation/translator", "core/config", "events/drag", "core/element_data", "color", "../../helpers/scheduler/helpers.js", "generic_light.css!", "ui/scheduler/ui.scheduler"], function (_export, _context) {
  "use strict";

  var fx, $, pointerMock, browser, DataSource, translator, config, dragEvents, dataUtils, Color, SchedulerTestWrapper, createWrapper, initTestMarkup, isDesktopEnvironment, CLASSES, supportedScrollingModes, testStart, test, module, DATE_TABLE_CELL_CLASS, APPOINTMENT_CLASS, realSetTimeout, priorityData, getAbsolutePosition, commonViews, timeLineViews, groupViews, zoomModuleConfig, DROPPABLE_CELL_CLASS, DRAG_SOURCE_CLASS, commonModuleConfig, draggingFromTooltipConfig, moveAsMouseConfig;
  return {
    ...
    }, function (_eventsDrag) {
      dragEvents = _eventsDrag.default;
    }, function (_coreElement_data) {
      dataUtils = _coreElement_data.default;
    }, function (_color) {
      Color = _color.default;
    }, function (_helpersSchedulerHelpersJs) {
      SchedulerTestWrapper = _helpersSchedulerHelpersJs.SchedulerTestWrapper;
      createWrapper = _helpersSchedulerHelpersJs.createWrapper;
      initTestMarkup = _helpersSchedulerHelpersJs.initTestMarkup;
      isDesktopEnvironment = _helpersSchedulerHelpersJs.isDesktopEnvironment;
      CLASSES = _helpersSchedulerHelpersJs.CLASSES;
      supportedScrollingModes = _helpersSchedulerHelpersJs.supportedScrollingModes;
    }, function (_generic_lightCss) {}, function (_uiSchedulerUiScheduler) {}],
    execute: function () {
      ({
        testStart,
        test,
      ...
      getAbsolutePosition = function (appointment) {
      ...
      DRAG_SOURCE_CLASS = CLASSES.appointmentDragSource.slice(1);
      module('Browser zoom', zoomModuleConfig, () => {
        ...
        QUnit.test('Appointment should drag to above cell in browser zoom case(T833310)', function (assert) {
          ...
          views.forEach(view => {
            ...
            pointer.down(offset.left, offset.top).move(0, -30);
            pointer.up();
      ...
      draggingFromTooltipConfig = $.extend({}, {
        data: [{
          text: 'app1',
        ...
        testFakeAppointmentPosition: function (scheduler, button, index, viewName, assert) {
          ...
          pointer.down(mousePosition.x, mousePosition.y).move(dragOffset.left, dragOffset.top);
          const fakeAppointmentPosition = this.getFakeAppointmentPosition(scheduler);
          assert.roughEqual(Math.round(fakeAppointmentPosition.left - dragOffset.left), Math.round(mousePosition.x), 1.1, `appointment should have correct left position in ${viewName}`);
          assert.roughEqual(Math.round(fakeAppointmentPosition.top - dragOffset.top), Math.round(mousePosition.y), 1.1, `appointment should have correct top position in ${viewName}`);
          pointer.up();
        }
      }, commonModuleConfig);
        ...
        [commonViews, timeLineViews, groupViews].forEach(views => {
          views.forEach(view => {
            test(`Views: ${view.name}`, function (assert) {
              ...
              pointer.down(mousePosition.x, mousePosition.y).move(dragOffset.left, dragOffset.top);
              const fakeAppointmentPosition = this.getFakeAppointmentPosition(scheduler);
              assert.roughEqual(Math.round(fakeAppointmentPosition.left - dragOffset.left), Math.round(mousePosition.x), 1.1, `appointment should have correct left position in ${view.name}`);
              assert.roughEqual(Math.round(fakeAppointmentPosition.top - dragOffset.top), Math.round(mousePosition.y), 1.1, `appointment should have correct top position in ${view.name}`);
              pointer.up();
            });
          });
        });
      ...
      moveAsMouseConfig = $.extend({}, {
        ...
        testAppointmentPosition: function (scheduler, text, dragCase, viewName, assert) {
          ...
          const pointer = pointerMock(appointment).start();
          pointer.down(positionBeforeDrag.left, positionBeforeDrag.top).move(dragCase.left, dragCase.top);
          ...
          pointer.up();
          assert.equal(positionAfterDrag.left - positionBeforeDrag.left, dragCase.left, `appointment '${text}' should have correct left position in ${viewName}`);
          assert.equal(positionAfterDrag.top - positionBeforeDrag.top, dragCase.top, `appointment '${text}' should have correct top position in ${viewName}`);
        },
        testViews: function (views, assert, rtlEnabled) {
          ...
          views.forEach(view => {
            ...
            this.dragCases.forEach(dragCase => this.data.forEach(({
              text
            }) => this.testAppointmentPosition(scheduler, text, dragCase, view.name, assert)));
          });
        },
      ...
      module('Appointment should move a same distance as mouse, rtlEnabled = false', moveAsMouseConfig, () => {
        ...
        test('Common Views', function (assert) {
          this.testViews(commonViews, assert, false);
        });
        test('Time Line Views', function (assert) {
          this.testViews(timeLineViews, assert, false);
        });
        test('Group Views', function (assert) {
          this.testViews(groupViews, assert, false);
        });
      });
      module('Appointment should move a same distance as mouse, rtlEnabled = true', moveAsMouseConfig, () => {
        ...
        test('Common Views', function (assert) {
          this.testViews(commonViews, assert, true);
        });
        test('Time Line Views', function (assert) {
          this.testViews(timeLineViews, assert, true);
        });
        test('Group Views', function (assert) {
          this.testViews(groupViews, assert, true);
        });
      });
      module('Common', commonModuleConfig, () => {
        ...
        test('Phantom appointment should have correct template', function (assert) {
          ...
          const pointer = pointerMock(scheduler.tooltip.getItemElement()).start().dragStart();
        ...
        test('The recurring appointment should have correct position when dragging', function (assert) {
          ...
          const pointer = pointerMock($appointment).start();
          try {
            pointer.down(positionBeforeDrag.left, positionBeforeDrag.top).move(150, 0);
            const $draggedAppointment = scheduler.appointments.getFakeAppointmentWrapper();
            const positionAfterDrag = getAbsolutePosition($draggedAppointment);
            pointer.up();
        ...
        QUnit.test('The appointment should have correct position after a drop when the store is asynchronous', function (assert) {
          ...
          pointer.down(positionBeforeDrag.left, positionBeforeDrag.top).move(cellWidth, 0);
        ...
        QUnit.test('The appointment should be dragged into the all-day panel when there is a scroll offset(T851985)', function (assert) {
          ...
          pointerMock($appointment).start().down(positionBeforeDrag.left, positionBeforeDrag.top).move(0, -(positionBeforeDrag.top - allDayPanelPosition.top - $allDayPanel.outerHeight() / 2)).up();
          const data = scheduler.instance.option('dataSource')[1];
        ...
        test('Appointment dragged from tooltip should have correct css', function (assert) {
          ...
          const pointer = pointerMock(appointment).start();
          pointer.down(appointmentPosition.left, appointmentPosition.top).move(50, 50);
          const draggedAppointment = scheduler.appointments.getFakeAppointmentWrapper();
        ...
        test('Dragged appointment should not have a right border', function (assert) {
          ...
          const $appointment = scheduler.appointments.getAppointment();
          const positionBeforeDrag = getAbsolutePosition($appointment);
          const pointer = pointerMock($appointment).start().down(positionBeforeDrag.left, positionBeforeDrag.top).move(50, 50);
          const fakeAppointment = scheduler.appointments.getFakeAppointment();
      ...
      module('appointmentDragging customization', $.extend({}, {
        ...
        createDraggable: (options, data) => {
          return $('<div>').css({
            width: 100,
            height: 100
          }).appendTo('#qunit-fixture').dxDraggable($.extend({
            clone: true,
        ...
        test('Event calls on appointment drag', function (assert) {
          ...
          const positionBeforeDrag = getAbsolutePosition(appointment);
          const pointer = pointerMock(appointment).start();
          pointer.down(positionBeforeDrag.left, positionBeforeDrag.top).move(0, 50);
          const $draggedAppointment = scheduler.appointments.getFakeAppointmentWrapper();
          const positionAfterDrag = getAbsolutePosition($draggedAppointment);
          pointer.up();
        ...
        test('Move appointment from Draggable', function (assert) {
          ...
          pointer.down(draggablePosition.left, draggablePosition.top).move(appointmentPosition.left - draggablePosition.left, appointmentPosition.top - draggablePosition.top).up();
          ...
          assert.deepEqual(draggableData, {
            text: 'App from Draggable'
          }, 'draggable data is not changed');
        });
        test('Move appointment to Draggable', function (assert) {
          ...
          const dataSource = scheduler.instance.option('dataSource');
          const pointer = pointerMock(appointment).start();
          pointer.down(appointmentPosition.left, appointmentPosition.top).move(draggablePosition.left - appointmentPosition.left, draggablePosition.top - appointmentPosition.top).up();
        ...
        [true, false].forEach(createDraggableFirst => {
          [true, false].forEach(allDay => {
            const testAppointmentDraggingFromSchedulerWithScroll = (assert, that, data, pointerMove, draggablePos, schedulerPos, scrollPos) => {
              ...
              pointer.down(appointmentPosition.left, appointmentPosition.top).move(pointerMoveCoordinates.x, pointerMoveCoordinates.y).up();
        ...
        test('The external appointment should move in the month view', function (assert) {
          ...
          const $dragElement = this.createDraggable({
            clone: false,
            group: group
          }, itemData);
          const scheduler = this.createScheduler({
        ...
        test('Move item from Draggable to Scheduler and back - droppable class should be removed', function (assert) {
          ...
          pointer.move(5, 5).move(appointmentPosition.left - draggablePosition.left - 5, appointmentPosition.top - draggablePosition.top - 5).move(draggablePosition.left - appointmentPosition.left, draggablePosition.top - appointmentPosition.top).up();
          assert.notOk($cellElement.hasClass(DROPPABLE_CELL_CLASS), 'cell has not droppable class');
        });
        ...
        [true, false].forEach(isDragFromTooltip => {
          const testText = isDragFromTooltip ? 'tooltip' : 'grid';
          const data = [{
            text: 'App 1',
            startDate: new Date(2018, 4, 21, 9, 30),
            endDate: new Date(2018, 4, 21, 11, 30)
          ...
          test(`itemData should be correct in appointmentDragging events when dragging from ${testText}`, function (assert) {
            ...
            const appointment = isDragFromTooltip ? scheduler.appointments.compact.getAppointment() : scheduler.appointments.find('App 1');
            const appointmentPosition = getAbsolutePosition(appointment);
            const pointer = pointerMock(appointment).start();
            pointer.down(appointmentPosition.left, appointmentPosition.top).move(50, 50).up();
          });
        });
      });
      module('Phantom Appointment Dragging', commonModuleConfig, () => {
        ...
        const checkAppointmentDragging = (assert, scheduler, appointmentTitle, dX, dY, appointmentCount = 1, dragSourceCount) => {
          ...
          const appointmentPosition = getAbsolutePosition(appointment);
          const pointer = pointerMock(appointment).start();
          pointer.down(appointmentPosition.left, appointmentPosition.top).move(dX, dY);
        ...
        const checkVirtualAppointmentDragging = (assert, scheduler, appointmentTitle, appointmentCount, appointmentCountAfterDND, isCloseDialog = false) => {
          ...
          assert.equal(appointments.length, appointmentCount, 'Phantom appointment does not exist');
          assert.equal(dragSource.length, 0, 'Drag source does not exist');
          pointerMock($appointment).start().down(positionBeforeDrag.left, positionBeforeDrag.top).move(0, 50);
        ...
        test('Phantom appointment should be removed after DnD from tooltip', function (assert) {
          ...
          pointer.down(appointmentPosition.left, appointmentPosition.top).move(0, 60);
        ...
        test('Dragging should work correctly with recurrent appointments', function (assert) {
          ...
          const appointmentPosition = getAbsolutePosition(appointment);
          const pointer = pointerMock(appointment).start();
          pointer.down(appointmentPosition.left, appointmentPosition.top).move(30, 30);
          appointments = scheduler.appointments.find(appointmentTitle);
          dragSource = scheduler.appointments.getDragSource();
        ...
        test('DnD should work correctly with virtual scrolling while scrolling', function (assert) {
          ...
          assert.equal(dragSource.length, 0, 'Drag source does not exist');
          const pointer = pointerMock($appointment).start().down(positionBeforeDrag.left, positionBeforeDrag.top).move(0, 50);
        ...
        test('Appointment should be updated correctly after DnD with virtual scrolling', function (assert) {
          ...
          pointerMock($appointment).start().down(positionBeforeDrag.left, positionBeforeDrag.top).move(cellCenter.left - positionBeforeDrag.left, cellCenter.top - positionBeforeDrag.top);
      ...
      module('Appointment dragging', {
        ...
        test('Draggable rendering option "immediate" should be turned off', function (assert) {
          const tasks = [{
            text: 'Task',
            startDate: new Date(2015, 2, 17),
            endDate: new Date(2015, 2, 17, 0, 30)
          }];
        ...
        supportedScrollingModes.forEach(scrollingMode => {
          module(`Scrolling mode ${scrollingMode}`, {
            ...
            test('Appointment should have correct position while dragging from group', function (assert) {
              ...
              assert.deepEqual(appointmentData.ownerId, {
                id: 2
