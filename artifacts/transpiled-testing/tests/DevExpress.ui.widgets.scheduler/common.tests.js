System.register(["core/utils/size", "animation/fx", "core/config", "core/element_data", "core/utils/type", "data/custom_store", "data/data_source/data_source", "events/visibility_change", "generic_light.css!", "jquery", "time_zone_utils", "ui/themes", "../../helpers/scheduler/helpers.js", "ui/scroll_view/ui.scrollable.js"], function (_export, _context) {
  "use strict";

  var getOuterHeight, getHeight, fx, config, dataUtils, isRenderer, CustomStore, DataSource, triggerHidingEvent, triggerShownEvent, $, getTimeZones, themes, createWrapper, initTestMarkup, Scrollable, isRenovatedScrollable;
  return {
    setters: [function (_coreUtilsSize) {
      ...
      $ = _jquery.default;
    }, function (_time_zone_utils) {
      getTimeZones = _time_zone_utils.getTimeZones;
    }, function (_uiThemes) {
    ...
    execute: function () {
      ...
      QUnit.module('Filtering', () => {
        ...
        QUnit.test('Scheduler should filter data on client side if the remoteFiltering option is false', function (assert) {
          const dataSource = new DataSource([{
            StartDate: new Date(2015, 11, 23).toString(),
            EndDate: new Date(2015, 11, 23, 0, 30).toString()
          }, {
            StartDate: new Date(2015, 11, 19).toString(),
            EndDate: new Date(2015, 11, 19, 0, 30).toString()
          }]);
          const scheduler = createWrapper({
          ...
          assert.deepEqual(dataUtils.data($appointments[0], 'dxItemData'), {
            StartDate: new Date(2015, 11, 23).toString(),
            EndDate: new Date(2015, 11, 23, 0, 30).toString()
        ...
        QUnit.test('Scheduler should filter data on server side if the remoteFiltering option is true', function (assert) {
          const dataSource = new DataSource([{
            StartDate: '2015-12-23T00:00',
            EndDate: '2015-12-23T00:30'
          }, {
            StartDate: '2015-12-19T00:00',
            EndDate: '2015-12-19T00:30'
          }]);
          const scheduler = createWrapper({
            currentDate: new Date(2015, 11, 24),
            firstDayOfWeek: 1,
        ...
        QUnit.test('Scheduler should filter data on client side depends on user filter', function (assert) {
          const dataSource = new DataSource({
            ...
            store: [{
              ...
              StartDate: new Date(2015, 11, 24).toString(),
              EndDate: new Date(2015, 11, 24, 0, 30).toString(),
              UserId: 2
            }]
          });
          const scheduler = createWrapper({
          ...
          assert.deepEqual(scheduler.instance.option('dataSource').items(), [{
            StartDate: new Date(2015, 11, 23).toString(),
            EndDate: new Date(2015, 11, 23, 0, 30).toString(),
            UserId: 1
            ...
            store: new CustomStore({
              load: function () {
                return [{
                  ...
                  StartDate: new Date(2015, 9, 24).toString(),
                  EndDate: new Date(2015, 9, 24, 0, 30).toString()
                }];
              }
            })
          });
