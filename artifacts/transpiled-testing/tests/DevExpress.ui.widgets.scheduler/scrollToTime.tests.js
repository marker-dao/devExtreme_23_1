System.register(["core/utils/size", "animation/fx", "ui/widget/ui.errors", "../../helpers/scheduler/helpers.js"], function (_export, _context) {
  "use strict";

  var getOuterHeight, getOuterWidth, fx, errors, createWrapper, initTestMarkup;
  return {
    ...
    execute: function () {
      ...
      QUnit.module('Scrolling to time', () => {
        ['standard', 'virtual'].forEach(scrollingMode => {
          const moduleName = scrollingMode === 'virtual' ? 'Virtual Scrolling' : 'Standard Scrolling';
          QUnit.module(moduleName, {
            ...
            QUnit.test('Check scrolling to time, if startDayHour is not 0', function (assert) {
              ...
              assert.roughEqual(scrollBy.getCall(1).args[0].top, expectedTop, 1.001, 'scrollBy was called with right distance');
            });
            ...
            QUnit.test('Check scrolling to time for timeline view', function (assert) {
              ...
              const scrollBy = sinon.spy(scrollable, 'scrollBy');
              scheduler.instance.scrollToTime(9, 5);
              assert.roughEqual(scrollBy.getCall(0).args[0].left, scheduler.instance._workSpace._getScrollCoordinates(9, 5, new Date(2015, 1, 9)).left, 1.001, 'scrollBy was called with right distance');
            });
            QUnit.test('Check scrolling to time for timeline view, rtl mode', function (assert) {
              ...
              scheduler.instance.scrollToTime(9, 5);
              assert.roughEqual(scrollBy.getCall(0).args[0].left, scheduler.instance._workSpace._getScrollCoordinates(9, 5, new Date(2015, 1, 9)).left - scrollLeft - offset, 1.001, 'scrollBy was called with right distance');
            });
            QUnit.test('Check scrolling to time for timeline view if date was set', function (assert) {
              ...
              assert.roughEqual(scrollBy.getCall(0).args[0].left, scheduler.instance._workSpace._getScrollCoordinates(9, 5, new Date(2015, 1, 11)).left, 1.001, 'scrollBy was called with right distance');
            });
            QUnit.test('Check scrolling to time for timeline view if date was set, rtl mode', function (assert) {
              ...
              scheduler.instance.scrollToTime(9, 5, new Date(2015, 1, 11, 10, 30));
              assert.roughEqual(scrollBy.getCall(0).args[0].left, scheduler.instance._workSpace._getScrollCoordinates(9, 5, new Date(2015, 1, 11)).left - scrollLeft - offset, 1.001, 'scrollBy was called with right distance');
            });
            QUnit.test('scrollToTime should throw a deprecation warning', function (assert) {
              ...
              assert.deepEqual(errors.log.getCall(0).args, ['W0002', 'dxScheduler', 'scrollToTime', '21.1', 'Use the "scrollTo" method instead'], 'Deprecation warning is correct');
            });
          });
        });
      });
