System.register(["core/utils/size", "jquery", "animation/fx", "../../helpers/pointerMock.js", "data/data_source/data_source", "data/array_store", "data/custom_store", "data/query", "core/element_data", "../../helpers/scheduler/helpers.js", "generic_light.css!", "ui/scheduler/ui.scheduler"], function (_export, _context) {
  "use strict";

  var getOuterHeight, getOuterWidth, $, fx, pointerMock, DataSource, ArrayStore, CustomStore, Query, dataUtils, CLASSES, supportedScrollingModes, createWrapper, initTestMarkup, module, test, testStart, createInstanceBase, config;
  return {
          ...
          module(`Scrolling mode ${scrollingMode}`, () => {
            ...
            test('AllDay tasks should not be filtered by end day hour', function (assert) {
              const tasks = [{
                ...
                text: 'Two',
                startDate: new Date(2015, 2, 16, 10),
                allDay: true,
                endDate: new Date(2015, 2, 16, 10, 30)
              }];
              const dataSource = new DataSource({
                store: tasks
              });
            ...
            test('Tail of long appointment should have a right width', function (assert) {
              const scheduler = createInstance({
                dataSource: [{
                  text: 'Task 1',
                  startDate: new Date(2015, 8, 12),
                  endDate: new Date(2015, 8, 22, 10)
                }],
            ...
            test('Multi-day appointment parts should be displayed correctly in allDay panel', function (assert) {
              const appointment = {
                startDate: new Date(2015, 3, 5, 0),
                endDate: new Date(2015, 3, 6, 7),
                text: 'caption'
              };
              const scheduler = createInstance({
            ...
            test('AllDay appointment should have correct height after changing view', function (assert) {
              const appointment = {
                startDate: new Date(2015, 2, 5, 1),
                endDate: new Date(2015, 2, 5, 1, 30),
                allDay: true,
                text: 'caption'
              };
              const scheduler = createInstance({
            ...
            test('allDay panel should be expanded when there are long appointments without allDay', function (assert) {
              const appointment = {
                startDate: new Date(2015, 2, 5, 1),
                endDate: new Date(2015, 2, 5, 3),
                text: 'caption'
              };
              const newAppointment = {
                startDate: new Date(2015, 2, 5, 1),
                endDate: new Date(2015, 2, 8, 3),
                text: 'caption'
              };
              const scheduler = createInstance({
                currentDate: new Date(2015, 2, 4),
