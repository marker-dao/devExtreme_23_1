System.register(["core/utils/data", "core/config", "data/data_source/data_source", "ui/scheduler/appointments/dataProvider/appointmentDataProvider", "renovation/ui/scheduler/utils/data"], function (_export, _context) {
  "use strict";

  var compileGetter, compileSetter, config, DataSource, AppointmentDataProvider, getPreparedDataItems, module, test, defaultDataAccessors, createAppointmentDataProvider;
  return {
    setters: [function (_coreUtilsData) {
      compileGetter = _coreUtilsData.compileGetter;
      compileSetter = _coreUtilsData.compileSetter;
    ...
    }, function (_uiSchedulerAppointmentsDataProviderAppointmentDataProvider) {
      AppointmentDataProvider = _uiSchedulerAppointmentsDataProviderAppointmentDataProvider.AppointmentDataProvider;
    }, function (_renovationUiSchedulerUtilsData) {
      getPreparedDataItems = _renovationUiSchedulerUtilsData.getPreparedDataItems;
    }],
    execute: function () {
      ({
        module,
      ...
      createAppointmentDataProvider = options => {
        return {
          ...
          prepareDataItems: () => {
            ...
            return getPreparedDataItems(options.dataSource.items(), options.dataAccessors, appointmentDuration, {
              createDate: date => date
            });
          }
        };
      ...
      module('Server side filtering', () => {
        ...
        test('Appointment filterByDate should filter dataSource correctly without copying dateFilter', function (assert) {
          ...
          appointmentDataProvider.filterByDate(new Date(2015, 1, 9, 0), new Date(2015, 1, 10, 13), true);
          const expectedFilter = [dateFilter, ['text', '=', 'Appointment 2']];
        ...
        test('Appointment filterByDate should return filter with dateSerializationFormat and without forceIsoDateParsing', function (assert) {
          ...
          try {
            ...
            const expectedFilter = [[[['endDate', '>=', new Date(2015, 1, 10)], ['startDate', '<', new Date(2015, 1, 11)]], 'or', [['endDate', new Date(2015, 1, 10)], ['startDate', new Date(2015, 1, 10)]]]];
            const actualFilter = dataSource.filter();
        ...
        test('Appointment filterByDate should return filter with dateSerializationFormat and forceIsoDateParsing', function (assert) {
          ...
          try {
            ...
            appointmentDataProvider.filterByDate(new Date(2015, 1, 10, 10), new Date(2015, 1, 10, 13), true, 'yyyy-MM-ddTHH:mm:ss');
            const expectedFilter = [[[['endDate', '>=', '2015-02-10T00:00:00'], ['startDate', '<', '2015-02-11T00:00:00']], 'or', [['endDate', '2015-02-10T00:00:00'], ['startDate', '2015-02-10T00:00:00']]]];
            const actualFilter = dataSource.filter();
            assert.deepEqual(actualFilter, expectedFilter, 'filter is right');
          } finally {
            config().forceIsoDateParsing = defaultForceIsoDateParsing;
      ...
      module('Client side after filtering', () => {
        ...
        test('Loaded appointments on the borders should be filtered by start & end day hours', function (assert) {
          ...
          assert.deepEqual(appts, [{
            ...
            StartDate: new Date(2015, 0, 1, 3, 45).toString(),
            EndDate: new Date(2015, 0, 1, 3, 50).toString()
          }], 'Appointments are OK. Appointment \'a\' was filtered');
        });
