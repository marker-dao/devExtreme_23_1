System.register(["jquery", "animation/translator", "animation/fx", "../../helpers/scheduler/helpers.js", "ui/themes", "ui/scheduler/compactAppointmentsHelper", "ui/widget/ui.widget", "color", "data/data_source/data_source", "data/custom_store", "ui/scheduler/appointmentAdapter", "ui/scheduler/ui.scheduler", "generic_light.css!"], function (_export, _context) {
  "use strict";

  var $, translator, fx, createWrapper, themes, CompactAppointmentsHelper, Widget, Color, DataSource, CustomStore, AppointmentAdapter, test, module, testStart, ADAPTIVE_COLLECTOR_DEFAULT_SIZE, ADAPTIVE_COLLECTOR_BOTTOM_OFFSET, ADAPTIVE_COLLECTOR_RIGHT_OFFSET, COMPACT_THEME_ADAPTIVE_COLLECTOR_RIGHT_OFFSET, baseConfig;
  return {
    ...
    }, function (_uiSchedulerCompactAppointmentsHelper) {
      CompactAppointmentsHelper = _uiSchedulerCompactAppointmentsHelper.CompactAppointmentsHelper;
    }, function (_uiWidgetUiWidget) {
      Widget = _uiWidgetUiWidget.default;
    }, function (_color) {
    ...
    }, function (_uiSchedulerAppointmentAdapter) {
      AppointmentAdapter = _uiSchedulerAppointmentAdapter.default;
    ...
    execute: function () {
      ...
      ADAPTIVE_COLLECTOR_DEFAULT_SIZE = 28;
      ADAPTIVE_COLLECTOR_BOTTOM_OFFSET = 40;
      ADAPTIVE_COLLECTOR_RIGHT_OFFSET = 5;
      COMPACT_THEME_ADAPTIVE_COLLECTOR_RIGHT_OFFSET = 1;
      baseConfig = {
        beforeEach: function () {
          fx.off = true;
      ...
      module('Integration: collector', baseConfig, () => {
        test('Start date should be equal targetedAppointmentData.startDate in appointment popup form in case recurrent appointment (T882652)', function (assert) {
          ...
          assert.equal(scheduler.tooltip.getDateText(), 'May 16 9:30 AM - 11:00 AM', 'Recurrence appointment date should be display equal targetedAppointmentData date in tooltip');
      ...
      module('Integration: Appointments Collector Base Tests', baseConfig, () => {
        ...
        const renderAppointmentsCollectorContainer = ({
          widgetMock,
          items,
          options,
          color
        ...
        test('Appointment collector should have a correct markup', function (assert) {
          const widgetMock = createWidget();
          const $button = renderAppointmentsCollectorContainer({
            widgetMock
          });
      ...
      module('Integration: Appointments Collector, adaptivityEnabled = false', baseConfig, () => {
        const tasks = [{
          startDate: new Date(2019, 2, 4),
          text: 'a',
          endDate: new Date(2019, 2, 4, 0, 30)
        }, {
          startDate: new Date(2019, 2, 4),
          text: 'b',
          endDate: new Date(2019, 2, 4, 0, 30)
        ...
        test('DropDown appointment button should have correct coordinates on weekView, not in allDay panel', function (assert) {
          ...
          const scheduler = createInstance({
            ...
            maxAppointmentsPerCell: 1,
            dataSource: [{
              startDate: new Date(2019, 2, 6),
              text: 'a',
              endDate: new Date(2019, 2, 6, 0, 30)
            }, {
              startDate: new Date(2019, 2, 6),
              text: 'b',
              endDate: new Date(2019, 2, 6, 0, 30)
        ...
        test('Many collapsed appts should be grouped correctly with one multi day task which started before collector (T525443)', function (assert) {
          ...
          scheduler.instance.option('dataSource', [{
            text: 'long appt',
            startDate: new Date(2019, 5, 8, 9, 0),
            endDate: new Date(2019, 5, 20, 9, 15)
          }, {
            text: '1',
            startDate: new Date(2019, 5, 11, 9, 30),
            endDate: new Date(2019, 5, 11, 11, 30)
          }, {
            text: '2',
            startDate: new Date(2019, 5, 11, 12, 0),
            endDate: new Date(2019, 5, 11, 13, 0)
          }, {
            text: '3',
            startDate: new Date(2019, 5, 11, 12, 0),
            endDate: new Date(2019, 5, 11, 13, 0)
            ...
            text: '4',
            startDate: new Date(2019, 5, 11, 8, 0),
            endDate: new Date(2019, 5, 11, 23, 59)
          }, {
            text: '5',
            startDate: new Date(2019, 5, 11, 9, 45),
            endDate: new Date(2019, 5, 11, 11, 15)
          }, {
            text: '6',
            startDate: new Date(2019, 5, 11, 11, 0),
            endDate: new Date(2019, 5, 11, 12, 0)
          }, {
            text: '7',
            startDate: new Date(2019, 5, 11, 11, 0),
            endDate: new Date(2019, 5, 11, 13, 30)
          }, {
            text: '8',
            startDate: new Date(2019, 5, 11, 14, 0),
            endDate: new Date(2019, 5, 11, 15, 30)
          }, {
            text: '9',
            startDate: new Date(2019, 5, 11, 14, 0),
            endDate: new Date(2019, 5, 11, 15, 30)
            ...
            text: '13',
            startDate: new Date(2019, 5, 11, 14, 30),
            endDate: new Date(2019, 5, 11, 16, 0)
          }]);
          scheduler.appointments.compact.click(0);
          this.clock.tick(300);
        ...
        test('Appointment collector should be painted depend on resource color', function (assert) {
          const colors = ['#ff0000', '#ff0000', '#ff0000', '#0000ff', '#0000ff', '#0000ff'];
          const appointments = [{
            startDate: new Date(2015, 2, 4),
            text: 'a',
            endDate: new Date(2015, 2, 4, 0, 30),
            ...
            integrationOptions: {
              templates: {
                'dropDownAppointmentTemplate': {
                  render(args) {
                    const $element = $('<span>').addClass('dx-template-wrapper').text('text');
        ...
        [{
          ...
          expectedText: '1 more',
          collectorIndex: 0,
          description: 'Scheduler should render correct number of collectors' + 'and pass correct number of appointments to them in week view\'s all-day panel (T965267)'
        }].forEach(({
          dataSource,
          view,
