...
(["testing/tests/DevExpress.ui.widgets.scheduler/dataSource.tests.js"], ["core/utils/size","jquery","../../helpers/scheduler/helpers.js","data/data_source/data_source","data/array_store","animation/fx","animation/translator","ui/scheduler/utils.timeZone","data/custom_store","core/utils/common","../../helpers/pointerMock.js","events/drag"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/tests/DevExpress.ui.widgets.scheduler/dataSource.tests.js", ["core/utils/size", "jquery", "../../helpers/scheduler/helpers.js", "data/data_source/data_source", "data/array_store", "animation/fx", "animation/translator", "ui/scheduler/utils.timeZone", "data/custom_store", "core/utils/common", "../../helpers/pointerMock.js", "events/drag"], function($__export) {
  "use strict";
  var getOuterWidth,
      getOuterHeight,
      $,
  ...
  return {
    ...
    execute: function() {
      ...
      module('Events', {
        ...
        module('Update', function() {
          isDesktopEnvironment() && test('oldData shouldn\'t has recurrenceException property', function(assert) {
            ...
            var scheduler = createWrapper({
              ...
              onAppointmentUpdating: function(e) {
                ...
                assert.equal($traceurRuntime.typeof((e.newData.recurrenceException)), 'string', 'newData argument should has new recurrenceException property');
              }
            });
          ...
          test('onAppointmentUpdating', function(assert) {
            ...
            var newData = {
              startDate: new Date(2015, 1, 10, 16),
              endDate: new Date(2015, 1, 10, 17),
              text: 'title'
            };
            var scheduler = createWrapper({
          ...
          test('Appointment should not be updated if \'cancel\' flag is defined as true', function(assert) {
            ...
            scheduler.instance.updateAppointment(appointments[0], {
              startDate: new Date(),
              text: 'Appointment 1'
            });
            this.clock.tick(10);
          ...
          test('Appointment form should not be updated if \'cancel\' flag is defined as true (T653358)', function(assert) {
            ...
            try {
              ...
              this.clock.tick(10);
              var appointmentForm = scheduler.instance._appointmentPopup.form;
          ...
          test('Appointment should be returned to the initial state if \'cancel\' flag is defined as true during async operation', function(assert) {
            ...
            var $appointment = $(scheduler.instance.$element().find('.dx-scheduler-appointment').eq(0));
            var initialLeftPosition = translator.locate($appointment).left;
            var cellWidth = getOuterWidth(scheduler.instance.$element().find('.dx-scheduler-all-day-table-cell').eq(0));
            var pointer = pointerMock(scheduler.instance.$element().find('.dx-resizable-handle-left').eq(0)).start();
          ...
          test('Appointment should have initial position if \'cancel\' flag is defined as true during update operation', function(assert) {
            ...
            var $appointment = $(scheduler.instance.$element().find('.dx-scheduler-appointment').eq(0));
            var initialPosition = translator.locate($appointment);
            $(scheduler.instance.$element().find('.dx-scheduler-date-table-cell').eq(5)).trigger(dragEvents.enter);
            pointerMock($appointment).start().down(initialPosition.left + 10, initialPosition.top + 10).move(initialPosition.left + 10, initialPosition.top + 100).up();
          ...
          test('Appointment should have initial size if \'cancel\' flag is defined as true during update operation (day view)', function(assert) {
            ...
            var $appointment = $(scheduler.instance.$element().find('.dx-scheduler-appointment').eq(0));
            var initialHeight = getOuterHeight($appointment);
            var cellHeight = getOuterHeight(scheduler.instance.$element().find('.dx-scheduler-date-table-cell').eq(0));
            var pointer = pointerMock(scheduler.instance.$element().find('.dx-resizable-handle-bottom').eq(0)).start();
          ...
          test('Appointment should have initial size if "cancel" flag is defined as true during update operation (month view)', function(assert) {
            ...
            var $appointment = $(scheduler.instance.$element().find('.dx-scheduler-appointment').eq(0));
            var initialWidth = getOuterWidth($appointment);
            var cellWidth = getOuterWidth(scheduler.instance.$element().find('.dx-scheduler-date-table-cell').eq(0));
            var pointer = pointerMock(scheduler.instance.$element().find('.dx-resizable-handle-right').eq(0)).start();
            pointer.dragStart().drag(cellWidth * 2, 0).dragEnd();
          ...
          test('Appointment should have initial top coordinate if \'cancel\' flag is defined as true during resize operation', function(assert) {
            var scheduler = createWrapper({
              ...
              currentView: 'week',
              dataSource: [{
                startDate: 1423620000000,
                endDate: 1423627200000,
                text: 'caption'
              }],
              firstDayOfWeek: 1,
              currentDate: new Date(2015, 1, 9)
            ...
            var $appointment = $(scheduler.instance.$element().find('.dx-scheduler-appointment').eq(0));
            var initialTopPosition = translator.locate($appointment).top;
            var cellHeight = getOuterHeight(scheduler.instance.$element().find('.dx-scheduler-all-day-table-cell').eq(0));
            var pointer = pointerMock(scheduler.instance.$element().find('.dx-resizable-handle-top').eq(0)).start();
            pointer.dragStart().drag(0, -cellHeight * 2).dragEnd();
        ...
        module('Delete', function() {
          ...
          test('onAppointmentDeleting', function(assert) {
            ...
            var appointments = [{
              startDate: new Date(2015, 3, 29, 5),
              text: 'Appointment 1',
              endDate: new Date(2015, 3, 29, 6)
            }];
            var scheduler = createWrapper({
              onAppointmentDeleting: deletingSpy,
              currentDate: new Date(2015, 3, 29),
              dataSource: new DataSource({store: appointments})
            });
            ...
            assert.equal(args.element, scheduler.instance.element(), 'Element field is OK');
            assert.equal(args.component, scheduler.instance, 'Component field is OK');
            assert.deepEqual(args.appointmentData, {
              startDate: new Date(2015, 3, 29, 5),
              text: 'Appointment 1',
              endDate: new Date(2015, 3, 29, 6)
            }, 'Appointment field is OK');
            assert.strictEqual(args.cancel, false, '\'Cancel\' flag is OK');
          });
      ...
      module('ArraySore(auto generated id)', moduleConfig, function() {
        ...
        test('onAppointmentUpdate[*] events should be work right', function(assert) {
          ...
          var scheduler = createWrapper({
            ...
            onAppointmentUpdating: function(e) {
              ...
              assert.deepEqual(e.newData, {text: 'Done'}, 'e.newData should be equal with part updated data on onAppointmentDeleting event');
            },
        ...
        if (isDesktopEnvironment()) {
          test('Excluded appointment\'s key property shouldn\'t equal to parent appointment(T929772)', function(assert) {
            ...
            var appointments = scheduler.instance.getDataSource().items();
            var recurrenceAppointment = appointments[0];
            var excludedAppointment = appointments[1];
            var expectedDate = new Date(excludedAppointment.startDate);
      ...
      module('Timezones', moduleConfig, function() {
        var timeZones = {
          LosAngeles: 'America/Los_Angeles',
          UTC: 'Etc/UTC'
        };
        [undefined, timeZones.UTC, timeZones.LosAngeles].forEach(function(timeZone) {
      ...
      module('Push API', function() {
        QUnit.test('Push new item to the store (remoteFiltering: true) (T900529)', function(assert) {
          ...
          var dataSource = scheduler.instance.getDataSource();
        ...
        QUnit.test('Push API should work correctly for the wrong data item (T986087)', function(assert) {
          ...
          [null, undefined].forEach(function(wrongData) {
            try {
              dataSource.store().push([{
                type: 'update',
                key: 123,
              ...
              var dataSourceItems = instance.getDataSource().items();
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["core/utils/size","jquery","../../helpers/scheduler/helpers.js","data/data_source/data_source","data/array_store","animation/fx","animation/translator","ui/scheduler/utils.timeZone","data/custom_store","core/utils/common","../../helpers/pointerMock.js","events/drag"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("core/utils/size"), require("jquery"), require("../../helpers/scheduler/helpers.js"), require("data/data_source/data_source"), require("data/array_store"), require("animation/fx"), require("animation/translator"), require("ui/scheduler/utils.timeZone"), require("data/custom_store"), require("core/utils/common"), require("../../helpers/pointerMock.js"), require("events/drag"));
  else
