System.register(["core/utils/size", "jquery", "animation/translator", "events/dblclick", "animation/fx", "data/data_source/data_source", "data/custom_store", "../../helpers/scheduler/helpers.js", "generic_light.css!", "ui/scheduler/ui.scheduler"], function (_export, _context) {
  "use strict";

  var getOuterHeight, getOuterWidth, $, translator, dblclickEvent, fx, DataSource, CustomStore, supportedScrollingModes, createWrapper, initTestMarkup, module, test, testStart, APPOINTMENT_DEFAULT_LEFT_OFFSET, createInstanceBase, config;
  return {
    ...
    execute: function () {
      ...
      createInstanceBase = options => createWrapper({
        ...
        supportedScrollingModes.forEach(scrollingMode => {
          module(`Scrolling mode ${scrollingMode}`, () => {
            ...
            test('AllDay appointments should have correct height, groupOrientation = vertical', function (assert) {
              ...
              const allDayPanelHeight = $(scheduler.instance.$element()).find('.dx-scheduler-all-day-table-cell').eq(0).get(0).getBoundingClientRect().height;
              assert.roughEqual(getOuterHeight($(scheduler.instance.$element()).find('.dx-scheduler-all-day-appointment').eq(0)), allDayPanelHeight, 0.501, 'First appointment height is correct on init');
              assert.roughEqual(getOuterHeight($(scheduler.instance.$element()).find('.dx-scheduler-all-day-appointment').eq(1)), allDayPanelHeight, 0.501, 'Second appointment height is correct on init');
            });
