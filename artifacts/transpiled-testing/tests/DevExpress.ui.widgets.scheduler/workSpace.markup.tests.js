System.register(["jquery", "ui/scheduler/workspaces/ui.scheduler.work_space", "ui/scheduler/workspaces/ui.scheduler.work_space.grouped.strategy.horizontal", "ui/scheduler/workspaces/ui.scheduler.work_space.grouped.strategy.vertical", "localization/date", "core/devices", "ui/scheduler/ui.scheduler", "generic_light.css!"], function (_export, _context) {
  "use strict";

  var $, SchedulerWorkSpace, SchedulerWorkSpaceHorizontalStrategy, SchedulerWorkSpaceVerticalStrategy, dateLocalization, devices, WORKSPACE_CLASS, WORKSPACE_WITH_COUNT_CLASS, WORKSPACE_WITH_GROUP_BY_DATE_CLASS, HEADER_PANEL_CLASS, ALL_DAY_PANEL_CLASS, ALL_DAY_TABLE_CELL_CLASS, ALL_DAY_ROW_CLASS, TIME_PANEL_CLASS, DATE_TABLE_CLASS, ALL_DAY_TITLE_CLASS, VERTICAL_GROUP_TABLE_CLASS, CELL_CLASS, CELL_TEXT_CLASS, TIME_PANEL_CELL_CLASS, HEADER_PANEL_CELL_CLASS, HORIZONTAL_SIZES_CLASS, VERTICAL_SIZES_CLASS, FIRST_GROUP_CELL_CLASS, LAST_GROUP_CELL_CLASS, WORKSPACE_DAY, WORKSPACE_WEEK, WORKSPACE_MONTH, toSelector, checkRowsAndCells, dayModuleConfig, dayWithGroupingModuleConfig, weekModuleConfig, weekWithGroupingModuleConfig, workWeekModuleConfig, monthModuleConfig, monthWithGroupingModuleConfig, scrollingModuleConfig;
  return {
    setters: [function (_jquery) {
      $ = _jquery.default;
    }, function (_uiSchedulerWorkspacesUiSchedulerWork_space) {
      SchedulerWorkSpace = _uiSchedulerWorkspacesUiSchedulerWork_space.default;
    }, function (_uiSchedulerWorkspacesUiSchedulerWork_spaceGroupedStrategyHorizontal) {
      SchedulerWorkSpaceHorizontalStrategy = _uiSchedulerWorkspacesUiSchedulerWork_spaceGroupedStrategyHorizontal.default;
    }, function (_uiSchedulerWorkspacesUiSchedulerWork_spaceGroupedStrategyVertical) {
      SchedulerWorkSpaceVerticalStrategy = _uiSchedulerWorkspacesUiSchedulerWork_spaceGroupedStrategyVertical.default;
    }, function (_localizationDate) {
      dateLocalization = _localizationDate.default;
    }, function (_coreDevices) {
      devices = _coreDevices.default;
    }, function (_uiSchedulerUiScheduler) {}, function (_generic_lightCss) {}],
    execute: function () {
      QUnit.testStart(() => {
        const markup = '<div id="scheduler-work-space">\
        <div id="scheduler-work-space-grouped">';
        $('#qunit-fixture').html(markup);
      });
      WORKSPACE_CLASS = 'dx-scheduler-work-space';
      ...
      CELL_TEXT_CLASS = 'dx-scheduler-date-table-cell-text';
      ...
      toSelector = cssClass => '.' + cssClass;
      checkRowsAndCells = function ($element, assert, interval, start, end, groupCount) {
      ...
      [{
        ...
        QUnit.module(`Base Workspace markup for ${viewName}`, moduleConfig, () => {
          ...
          QUnit.test('Scheduler workspace parts should be wrapped by scrollable', function (assert) {
            ...
            assert.ok($element.find('.dx-scheduler-date-table-container').parent().parent().hasClass('dx-scrollable-content'), 'Scrollable contains date table');
          });
      ...
      QUnit.module('Workspace Day markup', dayModuleConfig, () => {
        ...
        QUnit.test('Grouped cells should have a right group field in dxCellData', function (assert) {
          ...
          assert.deepEqual($element.find('.dx-scheduler-date-table tbody tr>td').eq(1).data('dxCellData').groups, {
            one: 2
          }, 'Cell group is OK');
        });
        ...
        [true, false].forEach(isRenovatedRender => {
          QUnit.test('Scheduler workspace day grouped view should contain a few headers', function (assert) {
            ...
            const lowerHeaderColspan = this.instance.option('renovateRender') ? '1' : undefined;
            assert.equal($element.find('.dx-scheduler-header-row th').length, 0, 'Date table has not header cell');
      ...
      QUnit.module('Workspace Day markup with vertical grouping', dayWithGroupingModuleConfig, () => {
        ...
        QUnit.test('Grouped cells should have a right group field in dxCellData', function (assert) {
          ...
          assert.deepEqual($element.find('.dx-scheduler-date-table tbody tr>td').eq(25).data('dxCellData').groups, {
            a: 2
          }, 'Cell group is OK');
        });
        QUnit.test('Grouped allDay cells should have a right group field in dxCellData', function (assert) {
          ...
          assert.deepEqual($allDayCells.eq(0).data('dxCellData').groups, {
            a: 1
          }, 'Cell group is OK');
          assert.deepEqual($allDayCells.eq(1).data('dxCellData').groups, {
            a: 2
          }, 'Cell group is OK');
        });
      });
      ...
      QUnit.module('Workspace Week markup', weekModuleConfig, () => {
        ...
        QUnit.test('Scheduler workspace week view should contain a 7 headers', function (assert) {
          ...
          $headerCells.each(function (index, cell) {
            ...
            const cellText = [dateLocalization.getDayNames('abbreviated')[index % 7].toLowerCase(), date.getDate()].join(' ');
      ...
      QUnit.module('Workspace Week markup with vertical grouping', weekWithGroupingModuleConfig, () => {
        ...
        QUnit.test('Grouped cells should have a right group field in dxCellData', function (assert) {
          ...
          assert.deepEqual($cells.eq(cellCount / 2).data('dxCellData').groups, {
            a: 2
          }, 'Cell group is OK');
        });
      });
      workWeekModuleConfig = {
        beforeEach: function () {
      ...
      QUnit.module('FirstGroupCell and LastGroupCell classes', () => {
        ...
        [true, false].forEach(isRenovatedRender => {
          const moduleName = isRenovatedRender ? 'Renovated Render' : 'Non-Renovated Render';
