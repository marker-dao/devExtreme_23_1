System.register(["core/config", "core/utils/common", "core/utils/type", "generic_light.css!", "jquery", "ui/scheduler/workspaces/ui.scheduler.work_space_day", "ui/scheduler/workspaces/ui.scheduler.work_space_month", "ui/scheduler/workspaces/ui.scheduler.work_space_work_week", "../../helpers/keyboardMock.js", "../../helpers/memoryLeaksHelper.js", "../../helpers/pointerMock.js"], function (_export, _context) {
  "use strict";

  var config, noop, isRenderer, $, keyboardMock, memoryLeaksHelper, pointerMock, CELL_CLASS, ALL_DAY_TABLE_CELL_CLASS, HOVER_CLASS, WORKSPACE_DAY, WORKSPACE_WEEK, WORKSPACE_MONTH, test, module, testStart;
  return {
    setters: [function (_coreConfig) {
      ...
      $ = _jquery.default;
    }, function (_uiSchedulerWorkspacesUiSchedulerWork_space_day) {}, function (_uiSchedulerWorkspacesUiSchedulerWork_space_month) {}, function (_uiSchedulerWorkspacesUiSchedulerWork_space_work_week) {}, function (_helpersKeyboardMockJs) {
      keyboardMock = _helpersKeyboardMockJs.default;
    }, function (_helpersMemoryLeaksHelperJs) {
      memoryLeaksHelper = _helpersMemoryLeaksHelperJs.default;
          ...
          ['standard', 'virtual'].forEach(scrollingMode => {
            module(`${scrollingMode} scrolling`, {
              ...
              test('It should not be possible to select cells via keyboard if the allowMultipleCellSelection option is false', function (assert) {
                ...
                const cells = $element.find('.' + CELL_CLASS);
                pointerMock(cells.eq(2)).start().click();
                keyboardMock($element).keyDown('down', {
                  shiftKey: true
                });
                assert.equal(cells.filter('.dx-state-focused').length, 1, 'right quantity of focused cells');
              });
            ...
            module(`${scrollingMode} scrolling`, {
              ...
              test('Pointer move propagation should be stopped', function (assert) {
                ...
                $element.trigger($.Event('dxpointerdown', {
                  target: cells.eq(15).get(0),
                  which: 1,
                  pointerType: 'mouse'
                }));
                $element.trigger($.Event('dxpointermove', {
                  target: cells.eq(16).get(0),
                  which: 1
                }));
              });
              ...
              test('Workspace should handle pointerdown by only left mouse key', function (assert) {
                ...
                $element.dxSchedulerWorkSpaceMonth('instance');
                const cells = $element.find('.' + CELL_CLASS);
                cells.eq(0).trigger($.Event('dxpointerdown', {
                  which: 1,
                  pointerType: 'mouse'
                }));
                assert.ok(cells.eq(0).hasClass('dx-state-focused'), 'cell is focused');
                cells.eq(1).trigger($.Event('dxpointerdown', {
                  which: 2,
                  pointerType: 'mouse'
                }));
              ...
              test('Workspace Day should not select cells that belong to another group', function (assert) {
                ...
                assert.equal(cells.filter('.dx-state-focused').length, 3, 'new cells have not been focused because the mouse pointer is in another group');
              ...
              test('Mouse Multiselection should work correctly when appointments' + 'are grouped vertically by more than one resource and allDayPanel is enabled', function (assert) {
                const $element = this.createInstance({
                  focusStateEnabled: true,
                  onContentReady: function (e) {
                    const scrollable = e.component.getScrollable();
