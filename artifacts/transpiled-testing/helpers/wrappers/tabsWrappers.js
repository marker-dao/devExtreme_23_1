...
(["testing/helpers/wrappers/tabsWrappers.js"], ["jquery","core/utils/extend"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/helpers/wrappers/tabsWrappers.js", ["jquery", "core/utils/extend"], function($__export) {
  "use strict";
  var $,
      extend,
      TestTabsWrapper,
  ...
  return {
    ...
    execute: function() {
      TestTabsWrapper = function() {
        function TestTabsWrapper($element, options) {
          var itemsCount = options.itemsCount;
          var tabsOptions = extend({items: this._generateItems(itemsCount)}, options);
          this._$tabs = $($element).dxTabs(tabsOptions);
          this._tabs = this._$tabs.dxTabs('instance');
        }
        return ($traceurRuntime.createClass)(TestTabsWrapper, {
          _generateItems: function() {
            var count = arguments[0] !== (void 0) ? arguments[0] : 5;
            return $traceurRuntime.spread(new Array(count)).map(function(value, index) {
              return ({text: ("item " + index)});
            });
          },
          _checkTabsContent: function($tabs) {
            var items = this._tabs.option('items');
            $tabs.toArray().forEach(function(tabElement, index) {
              return QUnit.assert.equal($(tabElement).text(), items[index].text, ("text of the " + index + " tab"));
            });
          },
          set width(value) {
            this._tabs.option('width', value);
          },
          setItemsByCount: function(count) {
            this._tabs.option('items', this._generateItems(count));
          },
          checkTabsWithoutScrollable: function() {
            var $tabs = this._$tabs.find('.dx-tabs-wrapper > .dx-tab');
            QUnit.assert.equal($tabs.length, this._tabs.option('items').length, 'tabs are rendered');
            this._checkTabsContent($tabs);
          },
          checkTabsWithScrollable: function() {
            var $tabs = this._$tabs.find('> .dx-tabs-scrollable .dx-scrollable-content .dx-tabs-wrapper > .dx-tab');
            QUnit.assert.equal($tabs.length, this._tabs.option('items').length, 'tabs are rendered in the Scrollable');
            this._checkTabsContent($tabs);
          },
          checkNavigationButtons: function(expected) {
            var visibilityStateText = expected ? 'shown' : 'hidden';
      ...
      $__export("TestTabsWrapper", TestTabsWrapper);
      TestAsyncTabsWrapper = function($__super) {
        function TestAsyncTabsWrapper($element, options) {
          var tabsOptions = extend(options, {
            templatesRenderAsynchronously: true,
            integrationOptions: {templates: {'testItem': {render: function(args) {
                    setTimeout(function() {
          ...
          $traceurRuntime.superConstructor(TestAsyncTabsWrapper).call(this, $element, tabsOptions);
        }
        return ($traceurRuntime.createClass)(TestAsyncTabsWrapper, {}, {}, $__super);
      }(TestTabsWrapper);
      $__export("TestAsyncTabsWrapper", TestAsyncTabsWrapper);
    }
  };
});
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/extend"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/extend"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=tabsWrappers.js.map