...
(["testing/helpers/fileManager/file_provider.test.js"], ["core/utils/deferred","file_management/provider_base","file_management/error_codes"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/helpers/fileManager/file_provider.test.js", ["core/utils/deferred", "file_management/provider_base", "file_management/error_codes"], function($__export) {
  "use strict";
  var Deferred,
  ...
  return {
    setters: [function($__m) {
      ...
      FileSystemProviderBase = $__m.default;
    }, function($__m) {
      ErrorCode = $__m.default;
    }],
    execute: function() {
      DEFAULT_DELAY = 2000;
      $__export('default', function($__super) {
        function TestFileSystemProvider(options) {
          $traceurRuntime.superConstructor(TestFileSystemProvider).call(this, options);
        ...
        return ($traceurRuntime.createClass)(TestFileSystemProvider, {
          getItems: function(path) {
            return this._provider.getItems(path);
          },
          renameItem: function(item, name) {
            var $__3 = this;
            return this._doDelay(function() {
              return $__3._renameItemCore(item, name);
            });
          },
          _renameItemCore: function(item, name) {
          ...
          createDirectory: function(parentDir, name) {
            var $__3 = this;
            return this._doDelay(function() {
              return $__3._createDirectoryCore(parentDir, name);
            });
          },
          _createDirectoryCore: function(parentDir, name) {
            this._raiseError();
          ...
          deleteItems: function(items) {
            ...
            if (this._useMultipleRequestMode()) {
              return this._getDelayedDeferreds(items, function(item) {
                return $__3._provider.deleteItems([item]);
              }, 'deletion');
            }
            if (this._useSingleRequestMode()) {
              return this._doDelay(function() {
                return $__3._provider.deleteItems(items);
              });
            }
            return this._provider.deleteItems(items);
          },
          moveItems: function(items, destinationDir) {
            var $__3 = this;
            if (this._useMultipleRequestMode()) {
              return this._getDelayedDeferreds(items, function(item) {
                return $__3._provider.moveItems([item], destinationDir);
              }, 'moving');
            }
            if (this._useSingleRequestMode()) {
              return this._doDelay(function() {
                return $__3._provider.moveItems(items, destinationDir);
              });
            }
            return this._provider.moveItems(items, destinationDir);
          },
          copyItems: function(items, destinationDir) {
            var $__3 = this;
            if (this._useMultipleRequestMode()) {
              return this._getDelayedDeferreds(items, function(item) {
                return $__3._provider.copyItems([item], destinationDir);
              }, 'copying');
            }
            if (this._useSingleRequestMode()) {
              return this._doDelay(function() {
                return $__3._provider.copyItems(items, destinationDir);
              });
            }
            return this._provider.copyItems(items, destinationDir);
          },
          uploadFileChunk: function(fileData, chunksInfo, destinationDirectory) {
            var $__3 = this;
            var deferred = chunksInfo.chunkIndex === 0 ? this._initiateFileUpload() : new Deferred().resolve().promise();
            var delay = chunksInfo.chunkIndex === 0 ? (DEFAULT_DELAY * (1 + 0.15 * chunksInfo.fileIndex)) : DEFAULT_DELAY;
            deferred = deferred.then(function() {
              return $__3._doDelay(function() {
                $__3._raiseChunkError(chunksInfo.chunkIndex, chunksInfo.fileIndex);
                return $__3._provider.uploadFileChunk(fileData, chunksInfo, destinationDirectory);
              }, delay, true);
            });
            if (chunksInfo.chunkIndex === chunksInfo.chunkCount - 1) {
              deferred = deferred.then(function() {
                return $__3._finalizeFileUpload();
              });
            }
            return deferred;
          },
          abortFileUpload: function(fileData, chunksInfo, destinationDirectory) {
            var $__3 = this;
            return this._doDelay(function() {
              return $__3._provider.abortFileUpload(fileData, chunksInfo, destinationDirectory);
            }, DEFAULT_DELAY, true);
          },
          downloadItems: function(items) {
            var $__3 = this;
            return this._getDelayedDeferreds(items, function() {
              return $__3._provider.downloadItems(items);
            }, 'downloading');
          },
          _initiateFileUpload: function() {
            return this._doDelay(null, 500, true);
          },
          _finalizeFileUpload: function() {
            return this._doDelay(null, DEFAULT_DELAY, true);
          },
          _getDelayedDeferreds: function(items, itemAction, counterName) {
            var $__3 = this;
            var index = this._counters[counterName];
            this._counters[counterName]++;
            return items.map(function(item) {
              return $__3._doDelay(function() {
                $__3._raiseError(item, index);
                return itemAction(item, index);
              }, (1 + 0.5 * index) * DEFAULT_DELAY, true);
            });
          },
          _raiseChunkError: function(chunkIndex, fileIndex) {
            var divider = 2;
            if ((chunkIndex + 1) % divider === 0) {
              this._raiseError(null, fileIndex);
            }
          },
          _raiseError: function(item, index) {
          ...
          _doDelay: function(action, timeout, skipError) {
            var $__3 = this;
            ...
            setTimeout(function() {
              try {
                if (!skipError && $__3._needRaiseError()) {
                  $__3._raiseErrorCore();
                }
                var result = action();
                deferred.resolve(result);
          ...
          _raiseErrorCore: function(fileItem) {
          ...
          _needRaiseError: function() {
            return this._raiseErrorMode !== 'none';
          },
          _useDefaultRequestMode: function() {
            return this._requestModeIs('default');
          },
          _useSingleRequestMode: function() {
            return this._requestModeIs('single');
          },
          _useMultipleRequestMode: function() {
            return this._requestModeIs('multiple');
          },
          _requestModeIs: function(value) {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["core/utils/deferred","file_management/provider_base","file_management/error_codes"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("core/utils/deferred"), require("file_management/provider_base"), require("file_management/error_codes"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=file_provider.test.js.map