...
(["testing/helpers/ariaAccessibilityTestHelper.js"], ["jquery","core/utils/type","ui/check_box"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/helpers/ariaAccessibilityTestHelper.js", ["jquery", "core/utils/type", "ui/check_box"], function($__export) {
  "use strict";
  var $,
      isDefined,
      CheckBox,
  ...
  return {
    ...
    execute: function() {
      ...
      ariaAccessibilityTestHelper = function() {
        function ariaAccessibilityTestHelper(args) {
          var $__3 = this;
          var createWidget = args.createWidget;
          this.createWidget = function() {
            var options = arguments[0] !== (void 0) ? arguments[0] : {};
            $__3.$widget = $('<div>').appendTo('#qunit-fixture');
            $__3.widget = createWidget($__3.$widget, options);
            $__3.isCheckBoxMode = $__3.widget.option('showCheckBoxesMode') === 'normal';
            $__3.$items = $__3.getItems();
            if ($__3.widget.getFocusedItemId) {
              $__3.focusedItemId = $__3.widget.getFocusedItemId();
            }
          };
        }
        return ($traceurRuntime.createClass)(ariaAccessibilityTestHelper, {
          getItems: function() {
          ...
          checkAttributes: function($target, expectedAttributes, prefix) {
            var element = $target.get(0);
            var skipAttributes = ['class', 'style', 'onclick'];
            var attributeNames = element.getAttributeNames().filter(function(name) {
              return skipAttributes.indexOf(name) === -1;
            }).map(function(name) {
          ...
          _checkItemAttributes: function(options, index, defaultValue) {
            var $__5 = options,
                attributes = $__5.attributes,
                focusedItemIndex = $__5.focusedItemIndex,
                role = $__5.role,
                tabindex = $__5.tabindex;
            var itemAttributes = {};
            attributes && attributes.forEach(function(attrName) {
          ...
          _checkCheckboxAttributes: function(options, index, defaultValue) {
            var attributes = options.attributes;
            var $checkBox = this.getItems().eq(index).prev();
          ...
          _checkNodeAttributes: function(options, $node, index) {
            var focusedNodeIndex = options.focusedNodeIndex;
            var $item = this.getItems().eq(index);
            var node = this.widget._getNode($item);
            var nodeAttributes = {
              role: 'treeitem',
          ...
          _checkGroupNodeAttributes: function(index) {
            var $nodeContainer = this.getItems().eq(index).closest('.dx-treeview-node-container').eq(0);
            this.checkAttributes($nodeContainer, {role: 'group'}, ("nodeContainer[" + index + "]"));
          },
          _checkAttribute: function(options, index, defaultValue) {
            var $node = this.getItems().eq(index).closest(("." + TREEVIEW_NODE_CLASS));
            if ($node.length) {
          ...
          checkItemsAttributes: function(selectedIndexes) {
            var options = arguments[1] !== (void 0) ? arguments[1] : {};
            var $__3 = this;
            selectedIndexes.forEach(function(index) {
              $__3._checkAttribute(options, index, 'true');
            });
            this.getItems().each(function(index) {
              if (selectedIndexes.indexOf(index) === -1) {
                $__3._checkAttribute(options, index, 'false');
              }
            });
          }
        }, {});
      }();
      $__export('default', ariaAccessibilityTestHelper);
    }
  };
});
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/type","ui/check_box"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/type"), require("ui/check_box"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=ariaAccessibilityTestHelper.js.map