...
(["testing/helpers/sizeMocks.js"], ["core/utils/size"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/helpers/sizeMocks.js", ["core/utils/size"], function($__export) {
  "use strict";
  var implementationsMap,
      defaultCommonCallbacks;
  function enumerateProperties(callback) {
    ['get', 'set'].forEach(function(kind) {
      ['Width', 'Height'].forEach(function(dimension) {
        ['', 'Inner', 'Outer'].forEach(function(location) {
          callback(kind + location + dimension);
        });
      });
    });
  }
  function initializeSizeMocks() {
    if (implementationsMap.for) {
      return;
    }
    implementationsMap.map = new Map();
    enumerateProperties(function(propertyName) {
      defaultCommonCallbacks[propertyName] = implementationsMap[propertyName];
      var newCallback = function(element, value) {
        var $__1,
            $__2;
        var target = this.map.get(element);
        if (target) {
          return ($__1 = target)[propertyName].apply($__1, $traceurRuntime.spread(arguments));
        }
        return ($__2 = defaultCommonCallbacks)[propertyName].apply($__2, $traceurRuntime.spread(arguments));
      };
      implementationsMap[propertyName] = newCallback.bind(implementationsMap);
    });
    implementationsMap.for = function(target) {
      var targetCallbacks = this.map.get(target);
      if (!targetCallbacks) {
        targetCallbacks = {};
        enumerateProperties(function(propertyName) {
          targetCallbacks[propertyName] = function(args) {
            var $__1;
            ($__1 = defaultCommonCallbacks)[propertyName].apply($__1, $traceurRuntime.spread(arguments));
          };
        });
  ...
  $__export("initializeSizeMocks", initializeSizeMocks);
  $__export("destroySizeMocks", destroySizeMocks);
  return {
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["core/utils/size"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("core/utils/size"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=sizeMocks.js.map