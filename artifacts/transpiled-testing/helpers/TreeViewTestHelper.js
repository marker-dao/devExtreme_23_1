...
(["testing/helpers/TreeViewTestHelper.js"], ["jquery","core/utils/type"], false, function($__System) {
var require = this.require, exports = this.exports, module = this.module;
$__System.register("testing/helpers/TreeViewTestHelper.js", ["jquery", "core/utils/type"], function($__export) {
  "use strict";
  var $,
      isDefined,
      CONTAINER_ID,
      ...
      SELECTED_ITEM_CLASS,
      INVISIBLE_ITEM_CLASS,
      CHECK_BOX_CLASS,
      CHECK_BOX_CHECKED_CLASS,
      assert,
      TreeViewTestWrapper;
  return {
    ...
    execute: function() {
      var $__3;
      CONTAINER_ID = 'treeView';
      WIDGET_CLASS = 'dx-treeview';
      NODE_CLASS = (WIDGET_CLASS + "-node");
      ITEM_CLASS = (WIDGET_CLASS + "-item");
      ITEM_CONTENT_CLASS = (WIDGET_CLASS + "-item-content");
      TOGGLE_ITEM_VISIBILITY_CLASS = (WIDGET_CLASS + "-toggle-item-visibility");
      NODE_LOAD_INDICATOR_CLASS = (NODE_CLASS + "-loadindicator");
      SELECTED_ITEM_CLASS = 'dx-state-selected';
      INVISIBLE_ITEM_CLASS = 'dx-state-invisible';
      CHECK_BOX_CLASS = 'dx-checkbox';
      CHECK_BOX_CHECKED_CLASS = 'dx-checkbox-checked';
      (($__3 = QUnit, assert = $__3.assert, $__3));
      TreeViewTestWrapper = function() {
        function TreeViewTestWrapper(options) {
          var $__2 = this;
          if (!options.onItemSelectionChanged) {
            options.onItemSelectionChanged = function() {
              return $__2.eventLog.push('itemSelectionChanged');
            };
          }
          if (!options.onSelectionChanged) {
            options.onSelectionChanged = function() {
              return $__2.eventLog.push('selectionChanged');
            };
          }
          if (!options.onSelectAllValueChanged) {
            options.onSelectAllValueChanged = function() {
              return $__2.eventLog.push('selectAllValueChanged');
            };
          }
          this.eventLog = [];
        ...
        return ($traceurRuntime.createClass)(TreeViewTestWrapper, {
          getElement: function() {
            return $(("#" + CONTAINER_ID));
          },
          getInstance: function(options) {
            return this.getElement().dxTreeView(options).dxTreeView('instance');
          },
          getNodes: function() {
            return this.getElement().find(("." + NODE_CLASS));
          },
          getItems: function($node) {
            return isDefined($node) ? $node.find(("." + ITEM_CLASS)) : this.getElement().find(("." + ITEM_CLASS));
          },
          ...
          getToggleItemVisibility: function($node) {
            return isDefined($node) ? $node.find(("." + TOGGLE_ITEM_VISIBILITY_CLASS)) : this.getElement().find(("." + TOGGLE_ITEM_VISIBILITY_CLASS));
          },
          getNodeLoadIndicator: function($node) {
            return isDefined($node) ? $node.find(("." + NODE_LOAD_INDICATOR_CLASS)) : this.getElement().find(("." + NODE_LOAD_INDICATOR_CLASS));
          },
          hasWidgetClass: function($item) {
            return $item.hasClass(WIDGET_CLASS);
          },
          hasItemClass: function($item) {
            return $item.hasClass(ITEM_CLASS);
          },
          hasCheckboxCheckedClass: function($item) {
            return $item.hasClass(CHECK_BOX_CHECKED_CLASS);
          },
          hasSelectedClass: function($item) {
            return $item.hasClass(SELECTED_ITEM_CLASS);
          },
          hasInvisibleClass: function($item) {
            return $item.hasClass(INVISIBLE_ITEM_CLASS);
          },
          checkSelectedNodes: function(selectedIndexes, additionalErrorMessage) {
            var $__2 = this;
            var $node = this.getNodes();
            selectedIndexes.forEach(function(index) {
              assert.equal($__2.hasSelectedClass($node.eq(index)), true, ("item " + index + " has selected class"));
              if ($__2.isCheckBoxMode)
                assert.equal($__2.hasCheckboxCheckedClass($node.eq(index).children()), true, ("checkbox " + index + " has checked class") + (additionalErrorMessage || ''));
            });
            $node.each(function(index) {
              if (selectedIndexes.indexOf(index) === -1) {
                assert.equal($__2.hasSelectedClass($node.eq(index)), false, ("item " + index + " has no selected class"));
                if ($__2.isCheckBoxMode)
                  assert.equal(!!$__2.hasCheckboxCheckedClass($node.eq(index).children()), false, ("checkbox " + index + " has not checked class") + (additionalErrorMessage || ''));
              }
            });
          },
          checkSelectedItemsWithTreeStructure: function(selectedIndexes, items) {
            var itemsArray = this.convertTreeToFlatList(items);
            this.checkSelectedItemsWithPlainStructure(selectedIndexes, itemsArray);
          },
          checkSelectedItemsWithPlainStructure: function(selectedIndexes, itemsArray) {
            itemsArray.forEach(function(_, index) {
              if (selectedIndexes.indexOf(index) === -1) {
          ...
          checkSelection: function(expectedKeys, expectedNodes, additionalErrorMessage) {
            var items = this.instance._dataAdapter.getData().map(function(node) {
              return node.internalFields.publicNode;
            });
            var keysArray = items.map(function(item) {
              return item.key;
            });
            var selectedIndexes = expectedKeys.map(function(key) {
          ...
          checkSelected: function(expectedSelectedIndexes, items) {
            this.checkSelectedItemsWithTreeStructure(expectedSelectedIndexes, items);
            this.checkSelectedNodes(expectedSelectedIndexes);
          },
          checkSelectedKeys: function(expectedSelectedKeys, additionalErrorMessage) {
            var actualKeys = this.instance.getSelectedNodeKeys();
            assert.deepEqual(actualKeys.sort(), expectedSelectedKeys.sort(), 'getSelectedNodeKeys method ' + additionalErrorMessage);
            var keysByAdapter = this.instance._dataAdapter.getSelectedNodesKeys();
            assert.deepEqual(keysByAdapter.sort(), expectedSelectedKeys.sort(), 'selectedKeys from dataAdapter' + additionalErrorMessage);
            var selectedKeysByNodes = this.instance.getSelectedNodes().map(function(node) {
              return node.key;
            });
            assert.deepEqual(selectedKeysByNodes.sort(), expectedSelectedKeys.sort(), 'getSelectedNodes method ' + additionalErrorMessage);
          },
          checkEventLog: function(expectedEventLog, additionalErrorMessage) {
            assert.deepEqual(this.eventLog, expectedEventLog, 'eventLog ' + additionalErrorMessage);
          },
          checkNodeIsInVisibleArea: function(itemKey) {
            var $treeView = this.getElement();
            var $item = $treeView.find(("[data-item-id=\"" + itemKey + "\"] ." + ITEM_CLASS)).eq(0);
            $item.find(("." + ITEM_CONTENT_CLASS)).wrapInner('<span/>');
            var treeViewRect = $treeView.get(0).getBoundingClientRect();
            var itemTextRect = $item.find('span').get(0).getBoundingClientRect();
            var scrollDirection = this.instance.option('scrollDirection');
          ...
          convertTreeToFlatList: function(items) {
            var itemsArray = [];
            var inOrder = function(items) {
      ...
      $__export('default', TreeViewTestWrapper);
    }
  };
});
...
(function(factory) {
  if (typeof define == 'function' && define.amd)
    define(["jquery","core/utils/type"], factory);
  else if (typeof module == 'object' && module.exports && typeof require == 'function')
    module.exports = factory(require("jquery"), require("core/utils/type"));
  else
    throw new Error("Module must be loaded as AMD or CommonJS");
});
//# sourceMappingURL=TreeViewTestHelper.js.map