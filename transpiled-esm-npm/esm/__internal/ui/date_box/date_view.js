...
class DateView extends Editor {
  ...
  _renderRollers() {
    ...
    Object.keys(this._rollerConfigs).forEach(name => {
      ...
      const $roller = $('<div>').appendTo(this._$rollersContainer).addClass(`${DATEVIEW_ROLLER_CLASS}-${rollerType}`);
  ...
  _createRollerConfigs(type) {
    ...
    .getFormatParts(uiDateUtils.FORMATS_MAP[selectedType]).forEach(partName => {
