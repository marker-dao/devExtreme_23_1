...
import { getOffset, getOuterWidth, getWidth } from '../../../core/utils/size';
...
import { CustomChunksFileUploadStrategy } from '../../ui/file_uploader/file_upload_strategy.chunks.custom';
import { DefaultChunksFileUploadStrategy } from '../../ui/file_uploader/file_upload_strategy.chunks.default';
import { CustomWholeFileUploadStrategy } from '../../ui/file_uploader/file_upload_strategy.whole.custom';
import { DefaultWholeFileUploadStrategy } from '../../ui/file_uploader/file_upload_strategy.whole.default';
...
class FileUploader extends Editor {
  ...
  _getFile(fileData) {
    ...
    const targetFileValue = isNumeric(fileData) ? value === null || value === void 0 ? void 0 : value[fileData] : fileData;
  ...
  _renderFile(file) {
    ...
    const $fileContainer = $('<div>').addClass(FILEUPLOADER_FILE_CONTAINER_CLASS).appendTo(this._$filesContainer).attr('role', 'listitem');
  ...
  _updateFileNameMaxWidth() {
    ...
    const filesContainerWidth = getWidth((_this$_$filesContaine2 = this._$filesContainer) === null || _this$_$filesContaine2 === void 0 ? void 0 : _this$_$filesContaine2.find(`.${FILEUPLOADER_FILE_CONTAINER_CLASS}`).first()) || getWidth(this._$filesContainer);
  ...
  _renderFileIcon(fileName, $container) {
    ...
    $('<div>').addClass(`${FILEUPLOADER_FILE_ICON_CLASS} ${ICON_CLASS} ${ICON_CLASS}-${getFileIconName(fileName)}`).appendTo($container);
  }
  ...
  _getUploadButton(file) {
    ...
    file.uploadButton = this._createComponent($('<div>').addClass(`${FILEUPLOADER_BUTTON_CLASS} ${FILEUPLOADER_UPLOAD_BUTTON_CLASS}`), Button, {
      ...
      elementAttr: {
        // @ts-expect-error format params should be extended
        'aria-label': messageLocalization.format('dxFileUploader-uploadFileButtonLabel', (file === null || file === void 0 || (_file$value2 = file.value) === null || _file$value2 === void 0 ? void 0 : _file$value2.name) ?? '')
      }
    });
  ...
  _renderUploadButton() {
    ...
    const $uploadButton = $('<div>').addClass(FILEUPLOADER_BUTTON_CLASS).addClass(FILEUPLOADER_UPLOAD_BUTTON_CLASS).appendTo(this._$content);
