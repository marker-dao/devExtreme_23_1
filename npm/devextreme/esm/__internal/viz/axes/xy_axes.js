...
import { generateDateBreaks } from '../../viz/axes/datetime_breaks';
import { getCosAndSin, getLog, patchFontOptions } from '../../viz/core/utils';
...
function generateAutoBreaks(_ref, series, _ref2) {
  ...
  const breaks = [];
  const getRange = type === 'logarithmic' ? (min, max) => getLog(max / min, logarithmBase) : (min, max) => max - min;
...
export default {
  linear: {
    ...
    _initializeMarkersTrackers(offset) {
      ...
      that._markerTrackers = this._dateMarkers.filter(marker => !marker.hidden).map((marker, i, markers) => {
