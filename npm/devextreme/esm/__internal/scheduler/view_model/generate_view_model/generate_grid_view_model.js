...
export const generateGridViewModel = (schedulerStore, items) => {
  ...
  const step2 = maybeSplit(items, hasAllDayPanel, (entities, panelName) => {
    ...
    const positionInsideGroup = byGroup.map(group => {
      ...
      const innerStep0 = isMonthView || panelName === 'allDayPanel' ? expandAllDayAllDayPanel(group, endDayHour, viewOffset) : expandAllDayRegularPanel(group);
      ...
      const innerStep3 = isMonthView || panelName === 'allDayPanel' ? snapToCells(innerStep2, optionManager.getCells(panelName)) : innerStep2;
