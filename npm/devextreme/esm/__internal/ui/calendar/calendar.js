...
import { inRange, sign } from '../../../core/utils/math';
...
class Calendar extends Editor {
  ...
  _setDateOption(optionName, optionValue) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-return
    ? optionValue.map(value => dateSerialization.serializeDate(value, serializationFormat)) : dateSerialization.serializeDate(optionValue, serializationFormat);
  ...
  _shouldSkipFocusEvent(event) {
    ...
    return Boolean($(target).parents(`.${CALENDAR_CLASS}`).length) && Boolean($(relatedTarget).parents(`.${CALENDAR_CLASS}`).length);
  }
