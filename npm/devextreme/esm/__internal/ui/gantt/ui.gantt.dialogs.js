...
class TaskEditDialogInfo extends DialogInfoBase {
  ...
  _getFormItems() {
    ...
    return [{
      ...
      validationRules: [{
        type: 'required',
        message: messageLocalization.format('validation-required-formatted',
        // @ts-expect-error ts-error
  ...
  _getValidationMessage(isStartDependencies, correctDate) {
    if (isStartDependencies) {
      return messageLocalization.format('dxGantt-dialogStartDateValidation',
      // @ts-expect-error ts-error
      this._getFormattedDateText(correctDate));
    }
    return messageLocalization.format('dxGantt-dialogEndDateValidation',
    // @ts-expect-error ts-error
    this._getFormattedDateText(correctDate));
  }
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
...
class ConfirmDialogInfo extends DialogInfoBase {
  ...
  _getConfirmMessage() {
    switch (this._parameters.type) {
      ...
      case 2:
        return messageLocalization.format('dxGantt-dialogResourcesDeleteConfirmation',
        // @ts-expect-error ts-error
