...
class Calendar extends _editor.default {
  ...
  _getSerializationFormat() {
    let optionName = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'value';
    const {
      [optionName]: value
    } = this.option();
    const {
  ...
  _setDateOption(optionName, optionValue) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-return
    ? optionValue.map(value => _date_serialization.default.serializeDate(value, serializationFormat)) : _date_serialization.default.serializeDate(optionValue, serializationFormat);
  ...
  _viewConfig(date) {
    const {
      firstDayOfWeek = _date.default.firstDayOfWeekIndex(),
      showWeekNumbers = false,
      selectWeekOnClick,
      weekNumberRule,
      zoomLevel = ZOOM_LEVEL.MONTH,
  ...
  _shouldSkipFocusEvent(event) {
    ...
    return Boolean((0, _renderer.default)(target).parents(`.${CALENDAR_CLASS}`).length) && Boolean((0, _renderer.default)(relatedTarget).parents(`.${CALENDAR_CLASS}`).length);
  }
  ...
  _updateAriaSelected(value, previousValue) {
    previousValue === null || previousValue === void 0 || previousValue.forEach(item => {
      if (!item) {
        return;
      }
      this.setAria('selected', false, this._view._getCellByDate(item));
    });
    value === null || value === void 0 || value.forEach(item => {
      if (!item) {
        return;
      }
